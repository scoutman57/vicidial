...
# 
# AST_WINphoneAPP_1.0.0.pl version 1.0.0      for Perl/Tk
# by Matt Florell mattf@vicimarketing.com started 2003/10/29
#
# Description:
# - Grabs live call info from a DB updated every second
# - Displays live status of users phones and Zap/SIP/Local channels
...
# - Administrative Hangup of any live Zap/SIP/Local channel
...
# perl -d:ptkdb C:\AST_VICI\AST_WINphoneAPP_1.0.0.pl  
#  
#
...
########################################################
### Frame for current live phones, live Zap Channels and live SIP/Local Channels

		my $live_phones_frame = $main_frame->Frame()->pack(-expand => '1', -fill => 'both', -side => 'right');
		$live_phones_frame->Label(-text => "Busy:               Outside Lines:                SIP Lines:             ")->pack(-side => 'top');
	...
	### list box for current Zaps that are on live calls right now
		...
		if ($admin_hangup_enabled) 
			...
			my $zap_hangup_button = $hangup_buttons_frame->Button(-text => 'Zap Hangup', -width => -1,
						-command => sub 
						{
							$zap_frame_list_refresh = 1;
						 Zap_Hangup;
			...
			my $sip_hangup_button = $hangup_buttons_frame->Button(-text => 'SIP Hangup', -width => -1,
						-command => sub 
						{
	...
	### list box for current live busy extensions that are on Zap calls
...
### Frame for middle buttons and entries
		...
		### button to put the current Zap channel you are on in Park
...
### Frame for Conferencing <initially hidden>
	...
	$conf_frame->place(-in=>$MW, -width=>1, -height=>1, -x=>1, -y=>1); # hide the frame in the upper-left corner pixel
		...
		### conf buttons at the bottom of the conf frame
			...
			$conf_sip_channel_frame->Label(-text => " Your SIP channel: ")->pack(-side => 'left');
...
### Frame for SIP/Local Hangup <initially hidden>
	...
	### list box for current live SIP/Local channels
		...
		### conf buttons at the bottom of the conf frame
			...
			my $sip_hangup_button = $button_sip_frame->Button(-text => 'HANGUP SIP LINE', -width => -1,
						-command => sub 
						{
...
if($zap_frame_list_refresh)		{$live_ext->insert('0', "$DBphonelist[$channel_counter]");}
			   ...
			   $LIVE_SIP_TEST = "SIP/$record->[1]";
			   if ( ($LIVE_SIP_TEST =~ /$SIP_user$|$SIP_user\&/) or ( ($CONF_DIAL) && ($record->[1] =~ /$NEW_conf/) ) )
				{
...
if ($Default_window)
	...
	$maxlist_reached=0;
	   ...
	   $dbh->query("SELECT distinct number_dialed,start_epoch FROM call_log where server_ip = '$server_ip' and channel = '$SIP_user' order by start_epoch desc limit 50;");
	   if ($dbh->has_selected_record)
		{
	   $iterA=$dbh->create_record_iterator;
		 $rec_countA=0;
...
##########################################
### transfer a live zap call to a local extension
##########################################
sub Local_Xfer
{
...
##########################################
### transfer a live zap call to voicemailbox
##########################################
sub Voicemail_Xfer
{
...
##########################################
### SIP Hangup subroutine, launches zap hangup window
##########################################
sub SIP_Hangup
{
...
$main_frame->place(-in=>$MW, -width=>1, -height=>1, -x=>1, -y=>1);
	...
	or 	die "Couldn't connect to database: \n";
		...
		$stmt = "SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and channel LIKE \"$SIP_user$DASH\%\" limit 9";
		$dbh->query("$stmt");
		if ($dbh->has_selected_record)
			{
			$iter=$dbh->create_record_iterator;
	   ...
	   $dbh->query("SELECT conf_exten FROM conferences where server_ip='$server_ip' and extension='' limit 1");
	   if ($dbh->has_selected_record)
	   {
	   $iter=$dbh->create_record_iterator;
	     $rec_count=0;
