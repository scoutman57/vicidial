				...
				if (-e "$webroot/$file_path/$file_name")
					...
					if ($file_passthru < 1)
						...
						foreach(@TRANSLATIONS)
							...
							foreach(@file)
								{
								if ($file[$LINESct] !~ /^\#|^\s*function |'INCALL'|'PAUSED'|'READY'|'NEW'|xmlhttp|ILPV |ILPA |SELECT cmd_line_f|=\"SELECT|WWW-Authenticate/)
									{
									...
									if ($file[$LINESct] =~ /INTERNATIONALIZATION-LINKS-PLACEHOLDER-VICIDIAL/)
										...
										if ($file[$LINESct] =~ /INTERNATIONALIZATION-LINKS-PLACEHOLDER-AGC/)
											...
											if ( ($lang_abb =~ /^en/) && ($Tct > 2) )
												...
												$file[$LINESct] =~ s/\"help.gif/\"..\/vicidial\/help.gif/g;
