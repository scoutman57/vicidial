	...
	if ($row[0] > 0)
	...
	else
		{
		 if ( (strlen($campaign_id) < 2) or (strlen($campaign_name) < 2) )
			...
			$stmt="INSERT INTO vicidial_campaigns values('$campaign_id','$campaign_name','$active','','','','','','DOWN','$park_ext','$park_file_name','$web_form_address','$allow_closers');";
			$rslt=mysql_query($stmt, $link);
			}
		}
$ADD=10;
...
if ($ADD==41)
{
	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
	 ...
	 if ( (strlen($campaign_name) < 6) or (strlen($active) < 1) )
		...
		$stmt="UPDATE vicidial_campaigns set campaign_name='$campaign_name',active='$active',dial_status_a='$dial_status_a',dial_status_b='$dial_status_b',dial_status_c='$dial_status_c',dial_status_d='$dial_status_d',dial_status_e='$dial_status_e',lead_order='$lead_order',allow_closers='$allow_closers' where campaign_id='$campaign_id';";
		$rslt=mysql_query($stmt, $link);
		}
