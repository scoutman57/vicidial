...
# 
# AST_WINphoneAPP_0.9.pl version 0.9.2      for Perl/Tk
...
### Frame for middle buttons and entries
	...
	### button for a popup HELP screen with user directions
		my $LaunchBrowser_New = $middle_bottons_frame->Button(-text => 'HELP', -width => -1,
					-command => sub 
					{
					 LaunchBrowser_New;
					});
		$LaunchBrowser_New->pack(-side => 'bottom', -expand => 'no', -fill => 'both');
		$LaunchBrowser_New->configure(-state => 'normal');
...
if($zap_frame_list_refresh)		{$live_ext->insert('0', "$DBphonelist[$channel_counter]");}
			   ...
			   if ( ($SIP_user eq "SIP/$record->[1]") or ( ($CONF_DIAL) && ($record->[1] =~ /$NEW_conf/) ) )
				{
