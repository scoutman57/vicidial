...
if ($Default_window)
	...
	if (!$enable_persistant_mysql)	{&connect_to_db;}
		...
		$stmtA = "SELECT distinct number_dialed,start_epoch FROM call_log where server_ip = '$server_ip' and (channel = '$SIP_user' or channel LIKE \"$SIP_user%\" or channel LIKE \"$SIP_user\@$SIP_abb%\") order by start_epoch desc limit 50;";
			if($DB){print STDERR "\n|$stmtA|\n";}
		print STDERR "\n|$stmtA|\n";
