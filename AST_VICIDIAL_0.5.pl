...
# 
# AST_VICIDIAL_0.5.pl version 0.5      for Perl/Tk
...
# perl -d:ptkdb C:\AST_VICI\AST_VICIDIAL_0.1.pl  
#  
#
...
#		$main_frame->Label(-text => "  ")->pack(-side => 'top');
	...
	### buttons at the left side of the main dialer frame
		...
		my $start_dialing_button = $dial_buttons_frame->Button(-text => 'DIAL NEXT NUMBER', -width => -1, -background => '#CCFFCC', -activebackground => '#99FF99',
					-command => sub 
					{
...
#		$dial_info_frame->Label(-text => " ")->pack(-side => 'top');
	...
	### send-to-closer frame
		...
		my $dial_closer_code_frame = $dial_closer_frame->Frame()->pack(-expand => '1', -fill => 'both', -side => 'top', -anchor   => "ne");
		$dial_closer_code_frame->Label(-text => "\nCode:", -background => '#CCCCFF')->pack(-side => 'left');
		my $closer_send_code_value = $dial_closer_code_frame->Entry(-width => '2', -relief => 'sunken')->pack(-side => 'right');
...
#	$MW->after (1500, \&get_dispo_list);
#	$MW->after (2500, \&call_dispo_window);
...
if (length($PARK)<5)
	...
	### insert a NEW record to the vicidial_manager table to be processed
		$stmtA = "INSERT INTO vicidial_manager values('','','$SQLdate','NEW','N','$server_ip','','Redirect','$RDqueryCID','Channel: $PARK','Context: $ext_context','Exten: $VICIDIAL_park_on_extension','Priority: 1','Callerid: $TCqueryCID','','','','','')";
		$dbhA->query($stmtA)  or die  "Couldn't execute query:\n";
...
sub call_dispo_window
{
	...
	if ($call_dispo_result =~ /OK/i)
		...
		if (length($DISP_val) > 4)
			...
			if ( ($stopdial_value_get =~ /NO/i) && ($auto_dial_next_number) )
				{
