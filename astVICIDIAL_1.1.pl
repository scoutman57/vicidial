...
# 
# astVICIDIAL_1.1.pl version 1.1.4      for Perl/Tk
...
# perl -d:ptkdb C:\AST_VICI\astVICIDIAL_1.1.4.pl  
#  
#
...
# 50624-1045 - altered line-alive detection to accept Local because of Asterisk changes
# 50907-0941 - altered conf dial filtering to allow anything over 10 digits through exactly
#
...
if ( (length($cust_channel)>4) && ($call_length_in_seconds > 5) )
	...
	if ($rec_countH)	### if channel active set inactivity counter to zero
		...
		{
		$dbhA->query("SELECT count(*) FROM $live_sip_channels where server_ip = '$server_ip' and channel='$cust_channel' and extension IN('$session_id','conf','ring','$VICIDIAL_park_on_filename','park','universal_')");
		   if ($dbhA->has_selected_record)
		   {
		   $iter=$dbhA->create_record_iterator;
			 $rec_countH=0;
...
if ( (length($xfer_channel)>4) && ($xfer_length_in_seconds > 5) )
	...
	if ($rec_countH)	### if channel active set inactivity counter to zero
		...
		{
		$dbhA->query("SELECT count(*) FROM $live_sip_channels where server_ip = '$server_ip' and channel='$xfer_channel' and extension IN('$session_id','conf','ring','$VICIDIAL_park_on_filename','park','universal_')");
		   if ($dbhA->has_selected_record)
		   {
		   $iter=$dbhA->create_record_iterator;
			 $rec_countH=0;
...
sub conf_dial_number {
	...
	if ( (length($conf_number_to_dial) ne 3) && (length($conf_number_to_dial) ne 4) && (length($conf_number_to_dial) ne 9) && (length($conf_number_to_dial) ne 10) && (length($conf_number_to_dial) ne 11) && (length($conf_number_to_dial) < 12) )
	{
