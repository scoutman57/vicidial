...
use DBI;
       $dbhX = DBI->connect('DBI:mysql:asterisk:10.0.0.10', 'cron','test')
...
#3857822|31022|105|01144|1621859299|MRS|B||BURTON|249 MUNDON ROAD|MALDON|ESSEX||||CM9 6PW|UK||||||COMMENTS
		...
		if (length($phone_number)>8)
			...
			if ($multi_insert_counter > 8)
				...
				$stmtZ = "INSERT INTO vicidial_list values$multistmt('','$entry_date','$modify_date','$status','$user','$vendor_lead_code','$source_id','$list_id','$campaign_id','$called_since_last_reset','$phone_code','$phone_number','$title','$first_name','$middle_initial','$last_name','$address1','$address2','$address3','$city','$state','$province','$postal_code','$country_code','$gender','$date_of_birth','$alt_phone','$email','$security_phrase','$comments');";
					$sthX = $dbhX->prepare($stmtZ) or die "preparing: ",$dbhX->errstr;
					$sthX->execute or die "executing: ", $dbhX->errstr;
