#!/usr/bin/perl
#
# AST_manager_send.pl version 0.1
#
...
#	&event_logger;
	...
	$endless_loop=86400;		# 1 day at .10 seconds per loop
...
#			$tn->buffer_empty;
	...
	$endless_loop--;
		...
		#	$input_buf = $tn->get(Errmode    => Return, Timeout    => 1,);
		#	$input_buf_length = length($input_buf);

		#	if($DB){print "input buffer: $input_buf_length\n";}
...
#			if($DB){print "input lines: $#list_lines\n";}
			...
			@psoutput = `ps -f -C AST_manager_li --no-headers`;
