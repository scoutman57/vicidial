...
while($one_day_interval > 0)
{
	...
	if ($dbhA->has_selected_record)
	   {
		...
		while ( $record = $iter->each)
			{
			if($V){print STDERR $record->[0]," NEW Actions to send on server $server_ip      $endless_loop\n";}
	...
	if (!$NEW_actions)
	   ...
	   $dbhA->query("$stmtA");
		...
		if ($V) {print STDERR "rows updated to QUEUE: |$affected_rows|\n";}
		}
		...
		if ($dbhA->has_selected_record)
		   {
			...
			while ( $record = $iter->each)
				{
				if($V){print STDERR $record->[0],"|", $record->[1],"|", $record->[6],"|", $record->[7],"|", $record->[8],"\n";}
				$man_id		 = "$record->[0]";
				$uniqueid	 = "$record->[1]";
				$channel	 = "$record->[6]";
				$action		 = "$record->[7]";
	...
	$endless_loop--;
		if( ($COUNTER_OUTPUT) or ($V) ){print STDERR "loop counter: |$endless_loop|\r";}
		...
		if ($endless_loop =~ /00$/)
			...
			if( ($COUNTER_OUTPUT) or ($V) ){print "checking to see if listener is dead |$sendonlyone|$running_listen|\n";}
			...
			if (!$running_listen) 
				{
				$sendonlyone++;
				if( ($COUNTER_OUTPUT) or ($V) ){print "LISTENER DEAD STOPPING PROGRAM... ATTEMPTING TO START keepalive SCRIPT\n";}
		...
		if( ($COUNTER_OUTPUT) or ($V) ){print "DONE... Exiting... Goodbye... See you later... Not really, initiating next loop...$one_day_interval left\n";}
	...
	if( ($COUNTER_OUTPUT) or ($V) ){print "DONE... Exiting... Goodbye... See you later... Really I mean it this time\n";}
