...
# 
# astVICIDIAL_0.8.pl version 0.8      for Perl/Tk
...
# perl -d:ptkdb C:\AST_VICI\astVICIDIAL_0.8.pl  
#  
#
...
#		$dial_info_frame->Label(-text => " ")->pack(-side => 'top');
		...
		$dial_info_frame->Label(-text => "Comments:")->pack(-side => 'top', -anchor   => "nw");
...
#	or 	die "Couldn't connect to database: \n";
	...
	foreach(@sip_chan_hangup)
		...
		$stmtA = "DELETE from vicidial_live_agents where user ='$Luser'";
		print STDERR "|$stmtA|\n";
	   $dbhA->query("$stmtA");
		my $affected_rows = $dbhA->get_affected_rows_length;
		print STDERR "old vicidial_live_agents records cleared: |$affected_rows|$Luser|\n";
...
sub conf_dial_number {
	...
	while ( ($get_channel_of_new_xcall_loop < 50) && (length($xferconf_zap_channel)<4) )
		...
		### sleep for 9 tenths of a second
		usleep(1*900*1000);
		print STDERR "Called $CNTD_prefix$conf_number_to_dial - $CNqueryCID   waiting for ring: $get_channel_of_new_xcall_loop seconds\r";
...
if (length($PARK)<5)
	...
	### use manager middleware-app to connect the next call to the meetme room
		...
		# 90009*CL_uk3survy_*8301*10000123*universal***
		$TXextension = "990009*CL_$campaign$park_exten_suffix**$lead_id*$VICIDIAL_park_on_filename***";
