...
# 
# astGUIclient_1.0.5.pl version 1.0.5      for Perl/Tk
...
# perl -d:ptkdb C:\AST_VICI\astGUIclient_1.0.5.pl  
#  
#
...
########################################################
### Frame for current live phones, live Zap/IAX Channels and live SIP/Local Channels
	...
	### list box for current Zaps/IAXs that are on live calls right now
		...
		if ($admin_hangup_enabled) 
			...
			my $zap_hangup_button = $hangup_buttons_frame->Button(-text => 'Zap/IAX Hangup', -width => -1,
						-command => sub 
						{
							$zap_frame_list_refresh = 1;
						 Zap_Hangup;
...
################################################################################################################
### Frame for Zap/IAX Hangup <initially hidden>
	...
	### list box for current phones that are on live calls right now
		...
		### conf buttons at the bottom of the conf frame
			...
			my $zap_hangup_button = $button_zap_frame->Button(-text => 'HANGUP ZAP/IAX LINE', -width => -1,
						-command => sub 
						{
...
##########################################
### get the list of active online Zap T1 channels and IAX VOIP channels from the database
##########################################
sub get_online_channels
{
