#!/usr/bin/perl
#
# AST_update.pl version 1.1.4  (build 50621-1307)
#
# DESCRIPTION:
	...
	foreach(@test_channels)
		...
		if ( ( ($PERCENT_static < 10) && ( ($channel_counter > 3) or ($sip_counter > 4) ) ) or
			...
			if ( ($endless_loop =~ /0$/) && ($SYSPERF) )
				...
				$stmtA = "INSERT INTO server_performance values('$now_date','$server_ip','$serverLOAD','$MEMfree','$MEMused','$serverPROCESSES','$#list_channels','$channel_counter','$sip_counter','$test_zap_count','$test_iax_count','$test_local_count','$test_sip_count','$recording_count')";
