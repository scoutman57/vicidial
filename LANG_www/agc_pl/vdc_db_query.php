		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST Rozmowa TIME",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
					if (eregi("DOWN LAST Rozmowa TIME",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
...
if ($ACTION == 'AGENTSview')
	...
	while ($agents_count > $loop_count)
		...
		if ($comments=='AgentXferViewSelect') 
			{
			$AXVSuserPORZĄDEK[$j] =	"$full_name$US$j";
			$AXVSuser[$j] =			$user;
	...
	if ($comments=='AgentXferViewSelect') 
		...
		sort($AXVSuserPORZĄDEK);
		while ($j > $k)
			{
			$order_split = explode("_",$AXVSuserPORZĄDEK[$k]);
			$i = $order_split[1];
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		if ($counts_to_print > 0) 
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; PHONE &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; STATUS &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; LAST Rozmowa &nbsp; </font></TD>";
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Zadzwoniono Since Nazwisko Reset: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Zadzwoniono Count: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Nazwisko Local Call Time: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
