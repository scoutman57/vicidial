...
if ($ACTION=="RedirectToPark")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($extenName)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($parkedby)<1) )
		...
		$stmt = "INSERT INTO park_log SET uniqueid='$uniqueid',status='MISE EN ATTED',channel='$channel',channel_group='$campaign',server_ip='$server_ip',parked_time='$NOW_TIME',parked_sec=0,extension='$CalLCID',user='$user',lead_id='$lead_id';";
			if ($format=='debug') {echo "\n<!-- $stmt -->";}
		$rslt=mysql_query($stmt, $link);
		...
		if ($enable_queuemetrics_logging > 0)
			...
			if ($VAC_eq_ct > 0)
				{
				$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='$CalLCID',queue='$queue',agent='Agent/$user',verb='CALLERONHOLD',data1='MISE EN ATT',serverid='$queuemetrics_log_id';";
				$rslt=mysql_query($stmt, $linkB);
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) )
		...
		if ($VAC_pl_ct > 0)
			...
			if ($enable_queuemetrics_logging > 0)
				...
				if ($VAC_eq_ct > 0)
					{
					$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='$CalLCID',queue='$queue',agent='Agent/$user',verb='CALLEROFFHOLD',data1='$parked_sec',data2='MISE EN ATT',serverid='$queuemetrics_log_id';";
					$rslt=mysql_query($stmt, $linkB);
...
if ($ACTION=="RedirectToParkIVR")
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($extenName)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) or (strlen($parkedby)<1) )
		...
		$stmt = "UPDATE vicidial_auto_calls SET extension='MISE EN ATT_IVR' where callerid='$CalLCID' limit 1;";
			if ($format=='debug') {echo "\n<!-- $stmt -->";}
		$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'02088',$user,$server_ip,$session_name,$one_mysql_log);}

		$stmt = "INSERT INTO park_log SET uniqueid='$uniqueid',status='IVRMISE EN ATTED',channel='$channel',channel_group='$campaign',server_ip='$server_ip',parked_time='$NOW_TIME',parked_sec=0,extension='$CalLCID',user='$user',lead_id='$lead_id';";
			if ($format=='debug') {echo "\n<!-- $stmt -->";}
		$rslt=mysql_query($stmt, $link);
		...
		if ($enable_queuemetrics_logging > 0)
			...
			if ($VAC_eq_ct > 0)
				{
				$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='$CalLCID',queue='$queue',agent='Agent/$user',verb='CALLERONHOLD',data1='IVRMISE EN ATT',serverid='$queuemetrics_log_id';";
				$rslt=mysql_query($stmt, $linkB);
	...
	if ( (strlen($channel)<3) or (strlen($queryCID)<15) or (strlen($exten)<1) or (strlen($ext_context)<1) or (strlen($ext_priority)<1) )
		...
		if ($VAC_pl_ct > 0)
			...
			if ($enable_queuemetrics_logging > 0)
				...
				if ($VAC_eq_ct > 0)
					{
					$stmt = "INSERT INTO queue_log SET partition='P01',time_id='$StarTtime',call_id='$CalLCID',queue='$queue',agent='Agent/$user',verb='CALLEROFFHOLD',data1='$parked_sec',data2='IVRMISE EN ATT',serverid='$queuemetrics_log_id';";
					$rslt=mysql_query($stmt, $linkB);
