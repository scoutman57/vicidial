...
if ($template_id) {
	...
	if (strlen($row["custom_table"])>0) {echo "USES CUSTOM TABELA: YES\n";} else {echo "USES CUSTOM TABELA: NO\n";}
	if (strlen($row["template_statuses"])>0) {echo "WILL ONLY DEDUPE AGAINST STATUS: ".preg_replace('/\|/', ',', $row["template_statuses"])."\n";}
	exit;
}
