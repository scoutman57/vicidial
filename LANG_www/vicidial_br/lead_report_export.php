...
$report_name = 'Exportação lidera Relatório';
$db_source = 'M';
...
#	Header("HTTP/1.0 401 Unauthorized");
    echo "Nome ou Senha inválidos or no export report permission: |$PHP_AUTH_USER|\n";
    exit;
	}
...
if ($run_export > 0)
	...
	if ($RUNcampaign > 0)
		...
		if ($outbound_to_print < 1)
			{
			echo "Existem no outbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ($RUNgroup > 0)
		...
		if ( ($inbound_to_print < 1) and ($outbound_calls < 1) )
			{
			echo "Existem no inbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ( ($outbound_to_print > 0) or ($inbound_to_print > 0) )
		...
		while ($k > $i)
			...
			if ( ($custom_fields_enabled > 0) and ($custom_fields=='YES') )
				...
				$stmt="MOSTRAR TABELAS LIKE \"custom_$CF_list_id\";";
				if ($DB>0) {echo "$stmt";}
				$rslt=mysql_query($stmt, $link);
		...
		{
		echo "Existem no calls during this time period for these parameters\n";
		exit;
		}
	}
##### END RUN THE EXPORT AND OUTPUT FLAT DATA FILE #####
	...
	echo "<CENTER><BR>\n";
	echo "<FONT SIZE=3 FACE=\"Arial,Helvetica\"><B>Exportação lidera Relatório</B></FONT><BR>\n";
	echo "<FONT SIZE=2 FACE=\"Arial,Helvetica\">Este relatório extrai informações da ligação para números discados no período selecionado. A liderança só é exportada uma vez, não importa quantas chamadas foram manipulados. O estado actual liderança é usado.</FONT><BR><BR>\n";
	...
	if ($custom_fields_enabled > 0)
		...
		echo "<B>campo Personalizados:</B><BR>\n";
