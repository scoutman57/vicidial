...
<title>ADMINISTRATION: Campanha Listas Reset
...
if ( ($LOGuser_level >= 9) and $LOGmodify_campaigns>0 and $LOGmodify_lists>0 and ( (preg_match("/Administration Change Log/",$LOGallowed_reports)) or (preg_match("/ALL RELATÓRIOS/",$LOGallowed_reports)) ) )
	{
	...
	echo "<br><B>Repor Chamado-Lead-Status for Campanhas:</B><BR><BR>\n";
	...
	echo "<option value='Y'>Ativo lists only</option>";
	echo "<option value=''>All lists</option>";
	echo "</select>";
	echo "</td>";
	echo "<td align='right'><input type='submit' name='submit_campaign_reset' value='ENVIAR'></td></tr>";
	...
	if ($submit_campaign_reset && $reset_lead_called_campaigns) {
		...
		if (mysqli_num_rows($list_id_rslt)>0) {
			echo "CAMPANHA <B>$reset_lead_called_campaigns</B> LISTS RESETTING $verbiage:<BR>\n<UL>";
			...
			$stmt="INSERT INTO vicidial_admin_log set event_date='$SQLdate', user='$PHP_AUTH_USER', ip_address='$ip', event_section='CAMPANHAS', event_type='RESET', record_id='$reset_lead_called_campaigns', event_code='ADMIN RESET CAMPANHA LISTS', event_sql=\"$SQL_log\", event_notes='';";
			if ($DB) {echo "|$stmt|\n";}
			$rslt=mysql_to_mysqli($stmt, $link);
		}
	...
	echo "VERSÃO: $admin_version<BR>";
	echo "BUILD: $build\n";
	if (!preg_match("/_BUILD_/",$SShosted_settings))
		{echo "<BR><a href=\"$PHP_SELF?ADD=999995\"><font color=white>&copy; 2013 ViciDial Group</font></a><BR><img src=\"../vicidial/images/pixel.gif\">";}
	echo "</font>\n";
?>
