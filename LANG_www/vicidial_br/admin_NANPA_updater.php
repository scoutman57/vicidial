...
if (strlen($active_voicemail_server)<7)
	{
	echo "ERROR: Admin -> Config. de Sistema -> Correio de Voz AtivoServer não está definido\n";
	exit;
	}
...
$list_ct=count($lists);
if ($submit_form=="ENVIAR" && $list_ct>0 && (strlen($vl_field_update)>0 || strlen($cellphone_list_id)>0 || strlen($landline_list_id)>0 || strlen($invalid_list_id)>0) ) 
	{
	for ($i=0; $i<$list_ct; $i++) 
		{
		if ($lists[$i]=="---ALL---") 
...
echo "<html>\n";
echo "<head>\n";
echo "<!-- VERSÃO: $admin_version   BUILD: $build   ADD: $ADD   PHP_SELF: $PHP_SELF-->\n";
...
if (mysqli_num_rows($schedule_rslt)>0 || (mysqli_num_rows($running_rslt)>0)) {
	...
	if (mysqli_num_rows($schedule_rslt)>0) {
		...
		echo "<td align='left' bgcolor='#015B91' width='300'><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE SIZE=2>Listas</th>";
		...
		while ($row=mysqli_fetch_array($schedule_rslt)) {
			...
			echo "<td align='center'><FONT FACE=\"ARIAL,HELVETICA\" size='1'><a href='$PHP_SELF?delete_trigger_id=$row[trigger_id]'>APAGAR</a></font></td>";
			echo "</tr>";
		}
		echo "</table>";
		echo "</td></tr>";
...
if ( ( (mysqli_num_rows($schedule_rslt)>0) or (mysqli_num_rows($running_rslt)>0) ) and ($block_scheduling_while_running==1) ) 
	...
	echo "";
	echo "<tr><td align='center' colspan='5'><input type='submit' value='ENVIAR' name='submit_form'></td></tr>";
	echo "</table>";
