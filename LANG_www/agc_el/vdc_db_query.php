...
if (isset($_GET["CallBackLeadΚατάσταση"]))				{$CallBackLeadΚατάσταση=$_GET["CallBackLeadΚατάσταση"];}
	elseif (isset($_POST["CallBackLeadΚατάσταση"]))	{$CallBackLeadΚατάσταση=$_POST["CallBackLeadΚατάσταση"];}
		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST ΚΛΗΣΗ TIME",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
					if (eregi("DOWN LAST ΚΛΗΣΗ TIME",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if (!$CBleadIDset)
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "Τηλ and Post Code Time Zone Mismatch! ";
...
if ($ACTION == 'updateDISPO')
	...
	#	fclose($fp);
		...
		if (strlen($CallBackLeadΚατάσταση) > 0) {$log_dispo_choice = $CallBackLeadΚατάσταση;}
	...
	if ( ($dispo_choice == 'CBHOLD') and (strlen($CallBackDatETimE)>10) )
		...
		$comments = eregi_replace(';','',$comments);
		$comments = eregi_replace("\\\\",' ',$comments);
		$stmt="INSERT INTO vicidial_callbacks (lead_id,list_id,campaign_id,status,entry_time,callback_time,user,recipient,comments,user_group,lead_status) values('$lead_id','$list_id','$campaign','ACTIVE','$NOW_TIME','$CallBackDatETimE','$user','$recipient','$comments','$user_group','$CallBackLeadΚατάσταση');";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00154',$user,$server_ip,$session_name,$one_mysql_log);}
		}

	### BEGIN Call Σημειώσεις Logging ###
...
if ($ACTION == 'AGENTSview')
	...
	while ($agents_count > $loop_count)
		...
		if ($comments=='AgentXferViewSelect') 
			{
			$AXVSuserΤΑΞΙΝ[$j] =	"$full_name$US$j";
			$AXVSuser[$j] =			$user;
	...
	if ($comments=='AgentXferViewSelect') 
		...
		while ($j > $k)
			{
			$order_split = explode("_",$AXVSuserΤΑΞΙΝ[$k]);
			$i = $order_split[1];
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		if ($counts_to_print > 0) 
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; PHONE &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; ΚΑΤΑΣΤΑΣΗ &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; LAST ΚΛΗΣΗ &nbsp; </font></TD>";
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Αποκαλούμενος Since Επίθετο Reset: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Αποκαλούμενος Count: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Επίθετο Local Call Time: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
