...
function user_authorization($user,$pass,$user_option,$user_update,$bcrypt,$return_hash)
	...
	$LOCK_over = ($STARTtime - 900); # failed login lockout time is 15 minutes(900 δευτερόλεπτα)
	...
	if ($auth < 1)
		...
		if ( (preg_match("/MXAG/",$SShosted_settings)) and ($mvla_total < 1) )
			...
			$vla_set = preg_replace("/.*MXAG|_ΔΗΜΙΟΥΡΓΙΑ_|DRA| /",'',$vla_set);
...
function custom_list_fields_values($lead_id,$list_id,$uniqueid,$user)
	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			$CFoutput .= "<input type=hidden name=stage id=stage value=\"ΥΠΟΒΑΛΕΤΕ\">\n";
	...
	if (preg_match("/--A--/",$CFoutput))
		...
		if (preg_match('/--A--TABLEper_call_notes--B--/i',$CFoutput))
			...
			if ($hide_call_log_info!='Y')
				{
				if ($search != 'logfirst')
					{$NOTESout .= "Κλήσεων ΓΙΑ ΑΥΤΟ ΤΟ ΜΟΛΥΒΔΟ:<br>\n";}
...
function lookup_gmt($phone_code,$USarea,$state,$LOCAL_GMT_OFF_STD,$Shour,$Smin,$Ssec,$Smon,$Smday,$Syear,$postalgmt,$postal_code)
	...
	if ( (!$AC_processed) and ($dst_range == 'SSM-FSN') )
		{
		if ($DBX) {print "     Second Sunday Μάρτιος to Πρώτο Sunday Νοέμβριος\n";}
		#**********************************************************************
		# SSM-FSN
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο Second Sunday Μάρτιος to Πρώτο Sunday Νοέμβριος at 2 am.
		#     INPUTS:
		#       mm              INTEGER       Month.
		...
		#                                      (DEFAULT is blank)
		#                                     make calculations based στο UTC time, 
		#                                     which means shift at 10:00 UTC in Απρίλιος
		#                                     and 9:00 UTC in Οκτώβριος
		#     OUTPUT: 
	...
	if ( (!$AC_processed) and ($dst_range == 'FSA-LSO') )
		{
		if ($DBX) {print "     Πρώτο Sunday Απρίλιος to Επίθετο Sunday Οκτώβριος\n";}
		#**********************************************************************
		# FSA-LSO
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο first Sunday in Απρίλιος and last Sunday in Οκτώβριος at 2 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSM-LSO') )
		{
		if ($DBX) {print "     Επίθετο Sunday Μάρτιος to Επίθετο Sunday Οκτώβριος\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο last Sunday in Μάρτιος and last Sunday in Οκτώβριος at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSO-LSM') )
		{
		if ($DBX) {print "     Επίθετο Sunday Οκτώβριος to Επίθετο Sunday Μάρτιος\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο last Sunday in Οκτώβριος and last Sunday in Μάρτιος at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-LSM') )
		{
		if ($DBX) {print "     Πρώτο Sunday Οκτώβριος to Επίθετο Sunday Μάρτιος\n";}
		#**********************************************************************
		#   TASMANIA ONLY
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο first Sunday in Οκτώβριος and last Sunday in Μάρτιος at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-FSA') )
		{
		if ($DBX) {print "     Sunday in Οκτώβριος to Πρώτο Sunday in Απρίλιος\n";}
		#**********************************************************************
		# FSO-FSA
		#   2008+ AUSTRALIA ONLY (country code 61)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο first Sunday in Οκτώβριος and first Sunday in Απρίλιος at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-TSM') )
		{
		if ($DBX) {print "     Πρώτο Sunday Οκτώβριος to Third Sunday Μάρτιος\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο first Sunday in Οκτώβριος and third Sunday in Μάρτιος at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSS-FSA') )
		{
		if ($DBX) {print "     Επίθετο Sunday in Σεπτέμβριος to Πρώτο Sunday in Απρίλιος\n";}
		#**********************************************************************
		# LSS-FSA
		#   2007+ ΝΕΟ ZEALAND (country code 64)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based στο last Sunday in Σεπτέμβριος and first Sunday in Απρίλιος at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'TSO-LSF') )
		{
		if ($DBX) {print "     Third Sunday Οκτώβριος to Επίθετο Sunday February\n";}
		#**********************************************************************
		# TSO-LSF
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect. Brazil
		#     Based στο Third Sunday Οκτώβριος to Επίθετο Sunday February at 1 am.
		#**********************************************************************
