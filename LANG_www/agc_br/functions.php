...
function user_authorization($user,$pass,$user_option,$user_update,$bcrypt,$return_hash)
	...
	$LOCK_over = ($STARTtime - 900); # failed login lockout time is 15 minutes(900 segundos)
	...
	if ($auth < 1)
		...
		if ( (preg_match("/MXAG/",$SShosted_settings)) and ($mvla_total < 1) )
			...
			$vla_set = preg_replace("/.*MXAG|_CONFIGURAÇÃO_|DRA| /",'',$vla_set);
...
function custom_list_fields_values($lead_id,$list_id,$uniqueid,$user)
	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			$CFoutput .= "<input type=hidden name=stage id=stage value=\"ENVIAR\">\n";
	...
	if (preg_match("/--A--/",$CFoutput))
		...
		if (preg_match('/--A--TABLEper_call_notes--B--/i',$CFoutput))
			...
			if ($hide_call_log_info!='Y')
				{
				if ($search != 'logfirst')
					{$NOTESout .= "CONVITE PARA ESTE LOG LEAD:<br>\n";}
...
function lookup_gmt($phone_code,$USarea,$state,$LOCAL_GMT_OFF_STD,$Shour,$Smin,$Ssec,$Smon,$Smday,$Syear,$postalgmt,$postal_code)
	...
	if ( (!$AC_processed) and ($dst_range == 'SSM-FSN') )
		{
		if ($DBX) {print "     Second Sunday Março to Nome Sunday Novembro\n";}
		#**********************************************************************
		# SSM-FSN
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     BasedligadoSecond Sunday Março to Nome Sunday Novembro at 2 am.
		#     INPUTS:
		#       mm              INTEGER       Month.
		...
		#                                      (DEFAULT is blank)
		#                                     make calculations basedligadoUTC time, 
		#                                     which means shift at 10:00 UTC in Abril
		#                                     and 9:00 UTC in Outubro
		#     OUTPUT: 
	...
	if ( (!$AC_processed) and ($dst_range == 'FSA-LSO') )
		{
		if ($DBX) {print "     Nome Sunday Abril to Sobrenome Sunday Outubro\n";}
		#**********************************************************************
		# FSA-LSO
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Basedligadofirst Sunday in Abril and last Sunday in Outubro at 2 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSM-LSO') )
		{
		if ($DBX) {print "     Sobrenome Sunday Março to Sobrenome Sunday Outubro\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Basedligadolast Sunday in Março and last Sunday in Outubro at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSO-LSM') )
		{
		if ($DBX) {print "     Sobrenome Sunday Outubro to Sobrenome Sunday Março\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Basedligadolast Sunday in Outubro and last Sunday in Março at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-LSM') )
		{
		if ($DBX) {print "     Nome Sunday Outubro to Sobrenome Sunday Março\n";}
		#**********************************************************************
		#   TASMANIA ONLY
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Basedligadofirst Sunday in Outubro and last Sunday in Março at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-FSA') )
		{
		if ($DBX) {print "     Sunday in Outubro to Nome Sunday in Abril\n";}
		#**********************************************************************
		# FSO-FSA
		#   2008+ AUSTRALIA ONLY (country code 61)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Basedligadofirst Sunday in Outubro and first Sunday in Abril at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-TSM') )
		{
		if ($DBX) {print "     Nome Sunday Outubro to Third Sunday Março\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Basedligadofirst Sunday in Outubro and third Sunday in Março at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSS-FSA') )
		{
		if ($DBX) {print "     Sobrenome Sunday in Setembro to Nome Sunday in Abril\n";}
		#**********************************************************************
		# LSS-FSA
		#   2007+ NOVO ZEALAND (country code 64)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Basedligadolast Sunday in Setembro and first Sunday in Abril at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'TSO-LSF') )
		{
		if ($DBX) {print "     Third Sunday Outubro to Sobrenome Sunday February\n";}
		#**********************************************************************
		# TSO-LSF
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect. Brazil
		#     BasedligadoThird Sunday Outubro to Sobrenome Sunday February at 1 am.
		#**********************************************************************
