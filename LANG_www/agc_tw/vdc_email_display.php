	...
	for ($i=1; $i<=5; $i++) 
		...
		if ($LF_orig) 
			...
			if (preg_match("/;|:|\/|\^|\[|\]|\"|\'|\*/",$LF_orig))
				{
				echo "ERROR: 無效 File Name: $LF_orig\n";
				exit;
				}
			else 
				{
...
if ($lead_id) {
	...
	if (mysqli_num_rows($rslt)>0) {
		...
		if (mysqli_num_rows($att_rslt)>0) {
			$EMAIL_form.="<tr bgcolor=white><td align='right' valign='top' width='150'>附件:</td><td align='left' valign='top' width='*'><pre>";
		...
		$EMAIL_form.="<tr bgcolor=white><td align='right' valign='top' width='150'>附件:</td><td align='left' valign='top' width='*'>";
