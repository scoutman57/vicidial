...
$DefaulTAlTDiaL			= '0';	# set to 1 to enable ALT RING by default if enabled for the campaign
...
echo "<html>\n";
echo "<head>\n";
echo "<!-- VERSION: $version     SKAPA: $build -->\n";
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						document.getElementById("LogiNReseT").innerHTML = "<INPUT TYPE=BUTTON VALUE=\"Uppdatera Kampanj List\" OnClick=\"login_allowable_campaigns()\">";
...
echo "<span id=\"LogiNReseT\"><INPUT TYPE=BUTTON VALUE=\"Uppdatera Kampanj List\" OnClick=\"login_allowable_campaigns()\"></span></TD></TR>\n";
	...
	if($auth>0)
		...
		if ( ( (ereg("START|ALL",$shift_enforcement)) and (!ereg("OFF",$VU_agent_shift_enforcement_override)) ) or (ereg("START|ALL",$VU_agent_shift_enforcement_override)) )
			...
			if ( ($shift_ok < 1) and ($VU_shift_override_flag < 1) and ($VDloginDISPLAY > 0) )
				...
				$VDdisplayMESSAGE.= "<INPUT TYPE=Submit NAME=SKICKA VALUE=SKICKA></FORM>\n";
				}
			}
...
### logged in and use that phone login account
	...
	if ( (strlen($park_ext)>0) && (strlen($park_file_name)>0) )
		...
		echo "<!-- KAMPANJ ANPASSAD PARKERAERING:  |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
		}
	echo "<!-- KAMPANJ STANDARD PARKERAERING: |$VICIDiaL_park_on_extension|$VICIDiaL_park_on_filename| -->\n";
	...
	if ( ( ($campaign_allow_inbound == 'Y') and ($dial_method != 'MANUAL') ) || ($campaign_leads_to_call > 0) || (ereg('Y',$no_hopper_leads_logins)) )
		...
		echo "<!-- gammal KÖ och INCALL omvänd lista:   |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!--  gammal KÖ och INCALL omvänd hopper: |$affected_rows| -->\n";
		...
		$affected_rows = mysql_affected_rows($link);
		echo "<!-- gamla vicidial liveagentposter rensade: |$affected_rows| -->\n";
...
$Cmonths = Array('Januari','February','Mars','April','Maj','Juni',
...
?>
	...
	var DiaLControl_auto_HTML = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF.gif\" border=0 alt=\" Paus \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/vdc_LB_resume.gif\" border=0 alt=\"Återuppta\"></a>";
	var DiaLControl_auto_HTML_ready = "<a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADpause');\"><IMG SRC=\"../agc/images/vdc_LB_pause.gif\" border=0 alt=\" Paus \"></a><IMG SRC=\"../agc/images/vdc_LB_resume_OFF.gif\" border=0 alt=\"Återuppta\">";
	var DiaLControl_auto_HTML_OFF = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF.gif\" border=0 alt=\" Paus \"><IMG SRC=\"../agc/images/vdc_LB_resume_OFF.gif\" border=0 alt=\"Återuppta\">";
	var DiaLControl_manual_HTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Ring nästa nummer\"></a>";
...
// filter manual dialstring and pass på to originate call
	function SendManualDial(taskFromConf)
		...
		if (taskFromConf == 'YES')
			...
			document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Ring med kund\" style=\"vertical-align:middle\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Parkera kunduppringning\" style=\"vertical-align:middle\"></a>";
	...
	function basic_originate_call(tasknum,taskprefix,taskreverse,taskdialvalue,tasknowait,taskconfxfer,taskcid,taskusegroupalias) 
		...
		if (xmlhttp) 
			...
			xmlhttp.onreadystatechange = function() 
				...
				//		document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Lägg på xfer linje\"></a>";
						}
					}
				}
			delete xmlhttp;
...
// ZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZZ  above section working på API functions
						...
						if ( (conf_channels_xtra_display == 1) || (conf_channels_xtra_display == 0) )
							...
							if (live_conf_calls > 0)
								...
								agentphonelive=0;
								var conv_start=-1;
								var live_conf_HTML = "<font face=\"Arial,Helvetica\"><B>AKTIVA SAMTAL I DIN SESSION:</B></font><BR><TABLE WIDTH=<?php echo $CQwidth ?>><TR BGCOLOR=<?php echo $SCRIPT_COLOR ?>><TD><font class=\"log_title\">#</TD><TD><font class=\"log_title\">FJÄRRKANAL</TD><TD><font class=\"log_title\">Lägg på</TD><TD><font class=\"log_title\">VOLYM</TD></TR>";
	...
	function conf_send_recording(taskconfrectype,taskconfrec,taskconffile) 
		...
		if (xmlhttp) 
			...
			//	filename = filedate + "_" + user_abb;
				...
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('StopMonitorConf','" + taskconfrec + "','" + filename + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_stoprecording.gif\" border=0 alt=\"Stoppa inspelning\"></a>";

				if (LIVE_campaign_recording == 'ALLFORCE')
					{
					document.getElementById("RecorDControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_startrecording_OFF.gif\" border=0 alt=\"Starta inspelning\">";
					}
				else
					{
			...
			if (taskconfrectype == 'StopMonitorConf')
				...
				var channelrec = "Local/" + conf_silent_prefix + '' + taskconfrec + "@" + ext_context;
				var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + taskconfrec + "','');return false;\"><IMG SRC=\"../agc/images/vdc_LB_startrecording.gif\" border=0 alt=\"Starta inspelning\"></a>";
				if (LIVE_campaign_recording == 'ALLFORCE')
					{
					document.getElementById("RecorDControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_startrecording_OFF.gif\" border=0 alt=\"Starta inspelning\">";
					}
...
// Covers the following types: XFER, VMAIL, ENTRY, CONF, PARKERA, FROMPARKERA, XfeRLOCAL, XfeRINTERNAL, XfeRBLIND, VfeRVMAIL
		...
		if (xmlhttpXF) 
			...
			if ( (taskvar == 'XfeRBLIND') || (taskvar == 'XfeRVMAIL') )
				...
				if (blindxferdialstring.length<'2')
					...
					xferredirect_query='';
					taskvar = 'NOTHING';
					alert("anknytningen måste ha mer än 1 siffra:" + blindxferdialstring);
					}
				else
					{
			...
			if (taskvar == 'ParK')
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('FROMParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_grabparkedcall.gif\" border=0 alt=\"Ta parkerat samtal\"></a>";
				customerparked=1;
				}
			if (taskvar == 'FROMParK')
				{
				...
				document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + redirectdestination + "','" + redirectdestserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_parkcall.gif\" border=0 alt=\"Parkera samtal\"></a>";
				customerparked=0;
				}
			...
			//		alert(xmlhttp.responseText);
				...
				//		alert("VICIDIAL Call log entered:\n" + document.vicidial_form.uniqueid.value);
						if ( (taskMDstage != "start") && (VDstop_rec_after_each_call == 1) )
							{
							var conf_rec_start_html = "<a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + session_id + "','');return false;\"><IMG SRC=\"../agc/images/vdc_LB_startrecording.gif\" border=0 alt=\"Starta inspelning\"></a>";
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (CBcounT == 0) {var CBprint = "NO";}
						...
						document.getElementById("CBstatusSpan").innerHTML ="<a href=\"#\" onclick=\"CalLBacKsLisTCheck();return false;\">" + CBprint + " AKTIVA ÅTERKOMSTER</a>";
...
// Request list of USERONLY callbacks for this agent
	function CalLBacKsLisTCheck()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
			{
			alert("DU MÅSTE VARA I PAUSLÄGE FÖR ATT KOLLA ÅTERKOMSTER NÄR RINGERN ÄR I AUTODIAL");
			}
		else
			{
			...
			if (xmlhttp) 
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						var loop_ct=0;
						var conv_start=0;
						var CB_HTML = "<table width=610><tr bgcolor=<?php echo $SCRIPT_COLOR ?>><td><font class=\"log_title\">#</td><td><font class=\"log_title\"> SAMTALBACK DATE/TIME</td><td><font class=\"log_title\">NUNNER</td><td><font class=\"log_title\">NAME</td><td><font class=\"log_title\">  STATUS</td><td align=right><font class=\"log_title\">CAMPAIGN</td><td><font class=\"log_title\">LAST SAMTALSDATUM\/TID</td><td align=left><font class=\"log_title\"> COMMENTS</td></tr>"
...
// Open page to enter details for a new manual dial lead
	function NeWManuaLDiaLCalL(TVfast)
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
			{
			alert("DU MÅSTE VARA I PAUSLÄGE FÖR ATT RINGA ETT NUMMER MANUELLT NÄR RINGERN ÄR I AUTODIAL");
			}
		else
			{
			if (TVfast=='FAST')
...
// Fast version of manual dial
		...
		if ( (MDDiaLCodEform.length < 1) || (MDPhonENumbeRform.length < 5) )
			{
			alert("DU MÅSTE KNAPPA IN TELEFONNUMMER OCH PREFIX FÖR ATT ANVÄNDA SNABBUPPRINGNING");
			}
		else
			{
			var MDLookuPLeaD = 'new';
...
// Request lookup of manual dial channel
	function ManualDialCheckChanneL(taskCheckOR)
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					//		alert(document.getElementById("MainStatuSSpan").innerHTML);
							document.getElementById("MainStatuSSpan").innerHTML = " Ringer: " + status_display_number + " UID: " + CIDcheck + " &nbsp; väntar på Ringsignal... " + MD_ring_secondS + " sekunder";
					//		alert("channel not found yet:\n" + campaign);
						...
						if (taskCheckOR == 'YES')
							...
							if ( (XDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') && (MD_ring_secondS < 10) )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Ringer upp 3-part: " + document.vicidial_form.xfernumber.value + " UID: " + CIDcheck;

								document.getElementById("Leave3WayCall").innerHTML ="<a href=\"#\" onclick=\"leave_3way_call('FIRST');return false;\"><IMG SRC=\"../agc/images/vdc_XB_leave3waycall.gif\" border=0 alt=\"Lämna 3-partssamtal\" style=\"vertical-align:middle\"></a>";

								document.getElementById("DialWithCustomer").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer_OFF.gif\" border=0 alt=\"Ring med kund\" style=\"vertical-align:middle\">";

								document.getElementById("ParkCustomerDial").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial_OFF.gif\" border=0 alt=\"Parkera kunduppringning\" style=\"vertical-align:middle\">";

								document.getElementById("HangupXferLine").innerHTML ="<a href=\"#\" onclick=\"xfercall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupxferline.gif\" border=0 alt=\"Lägg på xfer linje\" style=\"vertical-align:middle\"></a>";

								document.getElementById("HangupBothLines").innerHTML ="<a href=\"#\" onclick=\"bothcall_send_hangup();return false;\"><IMG SRC=\"../agc/images/vdc_XB_hangupbothlines.gif\" border=0 alt=\"Lägg på båda linjerna\" style=\"vertical-align:middle\"></a>";
							...
							if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Ringde: " + status_display_number + " UID: " + CIDcheck + " &nbsp;"; 

								document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('ParK','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_parkcall.gif\" border=0 alt=\"Parkera samtal\"></a>";

								document.getElementById("HangupControl").innerHTML = "<a href=\"#\" onclick=\"dialedcall_send_hangup();\"><IMG SRC=\"../agc/images/vdc_LB_hangupcustomer.gif\" border=0 alt=\"Lägg på kund\"></a>";

								document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('ON');\"><IMG SRC=\"../agc/images/vdc_LB_transferconf.gif\" border=0 alt=\"Vidarekoppla - Konferens\"></a>";

								document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_localcloser.gif\" border=0 alt=\"LOKAL CLOSER\" style=\"vertical-align:middle\"></a>";

								document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_XB_blindtransfer.gif\" border=0 alt=\"Ring blindnummer\" style=\"vertical-align:middle\"></a>";
								...
								if (quick_transfer_button == 'IN_GROUP')
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRLOCAL','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"SNABBKOPPLA\"></a>";
									}
								...
								if ( (quick_transfer_button == 'PRESET_1') || (quick_transfer_button == 'PRESET_2') || (quick_transfer_button == 'PRESET_3') || (quick_transfer_button == 'PRESET_4') || (quick_transfer_button == 'PRESET_5') )
									{
									document.getElementById("QuickXfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XfeRBLIND','" + lastcustchannel + "','" + lastcustserverip + "');return false;\"><IMG SRC=\"../agc/images/vdc_LB_quickxfer.gif\" border=0 alt=\"SNABBKOPPLA\"></a>";
		...
		if (MD_ring_secondS > 49) 
			...
			MD_channel_look=0;
			MD_ring_secondS=0;
			alert("Samtal kopplas ej, kontakta systemadministratören\n");
			}
				...
				//	alert(xmlhttp.responseText);
					...
					if (UDresponse_status == 'GOOD')
						...
						if (enable_second_webform > 0)
							{
							document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH();\"><IMG SRC=\"../agc/images/vdc_LB_webform_two.gif\" border=0 alt=\"Webformulär 2\"></a>\n";
							}
						}
					else
						{
...
// Send the Manual Ring nästa nummer request
	function ManualDialNext(mdnCBid,mdnBDleadid,mdnDiaLCodE,mdnPhonENumbeR,mdnStagE,mdVendorid,mdgroupalias)
		...
		if (dial_method == "INBOUND_MAN")
			...
			document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF.gif\" border=0 alt=\" Paus \"><IMG SRC=\"../agc/images/vdc_LB_resume_OFF.gif\" border=0 alt=\"Återuppta\"><BR><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber_OFF.gif\" border=0 alt=\"Ring nästa nummer\">";
			}
		else
			{
			document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_dialnextnumber_OFF.gif\" border=0 alt=\"Ring nästa nummer\">";
			}
		if (document.vicidial_form.LeadPreview.checked==true)
			{
			reselect_preview_dial = 1;
			var man_preview = 'YES';
			var man_status = "Förhandsgranska leadet innan <a href=\"#\" onclick=\"ManualDialOnly()\"><font class=\"preview_text\"> RING LEAD</font></a> or <a href=\"#\" onclick=\"ManualDialSkip()\"><font class=\"preview_text\">SKIPPA LEAD</font></a>"; 
			}
		else
			{
			...
			//		alert(xmlhttp.responseText);
					...
					if ( (MDnextCID.match(regMNCvar)) || (MDnextCID.match(regMDFvarDNC)) || (MDnextCID.match(regMDFvarCAMP)) )
						...
						if (MDnextCID.match(regMNCvar))
							{alert("Inga fler leads i hoppern för kampanj:\n" + campaign);   alert_displayed=1;}
						...
						if (MDnextCID.match(regMDFvarCAMP))
							{alert("Detta nummer finns ej i kampanjlistan:\n" + mdnPhonENumbeR);   alert_displayed=1;}
						if (alert_displayed==0)						
							{alert("Ospecifierat fel:\n" + mdnPhonENumbeR + "|" + MDnextCID);   alert_displayed=1;}

						if (starting_dial_level == 0)
							{
							document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Ring nästa nummer\"></a>";
							}
						else
							{
							if (dial_method == "INBOUND_MAN")
								...
								document.getElementById("DiaLControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_pause_OFF.gif\" border=0 alt=\" Paus \"><a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADready');\"><IMG SRC=\"../agc/images/vdc_LB_resume.gif\" border=0 alt=\"Återuppta\"></a><BR><a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Ring nästa nummer\"></a>";
								}
							else
								{
					...
					//	CalL_ScripT_id									= MDnextResponse_array[37];
						...
						document.getElementById("MainStatuSSpan").innerHTML = " Ringer: " + status_display_number + " UID: " + MDnextCID + " &nbsp; " + man_status;
						...
						if (LeaDPreVDispO == 'CALLBK')
							{
							document.getElementById("CusTInfOSpaN").innerHTML = " <B> PREVIOUS SAMTALBACK </B>";
							...
							document.getElementById("CBcommentsBoxB").innerHTML = "<b>Återkomst:</b>" + CBcallback_time;
...
// Send the Manual Dial Skip
	function ManualDialSkip()
		...
		if (manual_dial_in_progress==1)
			{
			alert('DU KAN INTE SKIPPA EN SAMTALBACK ELLER MANUELL UPPRINGNING, DU MÅSTE RINGA LEADET');
			}
		else
			{
			if (dial_method == "INBOUND_MAN")
				...
				xmlhttp.onreadystatechange = function() 
					...
					//	alert(xmlhttp.responseText);
						...
						if (MDSnextCID == "LEAD NOT REVERTED")
							{
							alert("Leadet omvändes ej, ett fel uppstod:\n" + MDSnextResponse);
							}
						else
							{
...
// Send the Manual Dial Only - dial the previewed lead
	function ManualDialOnly(taskaltnum)
		...
		if (dialed_label == 'ALT')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT RING NUMMER: ALT </B>";}
		if (dialed_label == 'ADDR3')
			{document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT RING NUMMER: PERSONNUMMER </B>";}
		var REGalt_dial = new RegExp("X","g");
		if (dialed_label.match(REGalt_dial))
			{
			document.getElementById("CusTInfOSpaN").innerHTML = " <B> ALT RING NUMMER: " + dialed_label + "</B>";
			document.getElementById("EAcommentsBoxA").innerHTML = "<b>Prefuix och telefonnummer: </b>" + EAphone_code + " " + EAphone_number;

			var EAactive_link = '';
			if (EAalt_phone_active == 'Y') 
				{EAactive_link = "<a href=\"#\" onclick=\"alt_phone_change('" + EAphone_number + "','" + EAalt_phone_count + "','" + document.vicidial_form.lead_id.value + "','N');\">Ändra detta nummer till INAKTIV</a>";}
			else
		...
		if (xmlhttp) 
			...
			//		alert(xmlhttp.responseText);
					...
					if (MDnextCID == " SAMTAL NOT PLACED")
						{
						alert("samtalet kopplades ej, ett fel uppstod:\n" + MDOnextResponse);
						}
					else
						{
						...
						if (alt_dial_status_display=='0')
							{
							document.getElementById("MainStatuSSpan").innerHTML = " Ringer: " + status_display_number + " UID: " + MDnextCID + " &nbsp; väntar på Ringsignal...";
	...
	function AutoDial_ReSume_PauSe(taskaction,taskagentlog,taskwrapup,taskstatuschange,temp_reason)
		...
		if (taskaction == 'VDADready')
			...
			if (dial_method == "INBOUND_MAN")
				...
				document.getElementById("DiaLControl").innerHTML = "<a href=\"#\" onclick=\"AutoDial_ReSume_PauSe('VDADpause');\"><IMG SRC=\"../agc/images/vdc_LB_pause.gif\" border=0 alt=\" Paus \"></a><IMG SRC=\"../agc/images/vdc_LB_resume_OFF.gif\" border=0 alt=\"Återuppta\"></a><BR><a href=\"#\" onclick=\"ManualDialNext('','','','','','0');\"><IMG SRC=\"../agc/images/vdc_LB_dialnextnumber.gif\" border=0 alt=\"Ring nästa nummer\"></a>";
				}
			else
				{
				...
				xmlhttprequestcheckauto.onreadystatechange = function() 
					...
					//	alert(xmlhttprequestcheckauto.responseText);
						...
						//	DialLog("start");
							...
							document.getElementById("MainStatuSSpan").innerHTML = " Inkommande: " + dial_display_number + " UID: " + CIDcheck + " &nbsp; " + VDIC_fronter; 
							...
							if (VDIC_data_VDIG[1].length > 0)
								...
								document.getElementById("MainStatuSSpan").innerHTML = " Inkommande: " + dial_display_number + " Group- " + VDIC_data_VDIG[1] + " &nbsp; " + VDIC_fronter; 
								}
...
// refresh the content of the web form URL
	function WebFormRefresH(taskrefresh,submittask,force_webvars_refresh) 
		...
		if (taskrefresh == 'OUT')
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormRefresH('IN');\"><IMG SRC=\"../agc/images/vdc_LB_webform.gif\" border=0 alt=\"Webformulär\"></a>\n";
			}
		else 
			{
			document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormRefresH('OUT');\"><IMG SRC=\"../agc/images/vdc_LB_webform.gif\" border=0 alt=\"Webformulär\"></a>\n";
			}
		}
...
// refresh the content of the second web form URL
	function WebFormTwoRefresH(taskrefresh,submittask) 
		...
		if (enable_second_webform > 0)
			...
			if (taskrefresh == 'OUT')
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOver=\"WebFormTwoRefresH('IN');\"><IMG SRC=\"../agc/images/vdc_LB_webform_two.gif\" border=0 alt=\"Webformulär 2\"></a>\n";
				}
			else 
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<a href=\"" + TEMP_VDIC_web_form_address_two + "\" target=\"" + web_form_target + "\" onMouseOut=\"WebFormTwoRefresH('OUT');\"><IMG SRC=\"../agc/images/vdc_LB_webform_two.gif\" border=0 alt=\"Webformulär 2\"></a>\n";
				}
			}
		}
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup(dispowindow,hotkeysused,altdispo,nodeletevdac) 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			if( document.images ) { document.images['livecall'].src = image_livecall_OFF.src;}
			document.getElementById("WebFormSpan").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_webform_OFF.gif\" border=0 alt=\"Webformulär\">";
			if (enable_second_webform > 0)
				{
				document.getElementById("WebFormSpanTwo").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_webform_two_OFF.gif\" border=0 alt=\"Webformulär 2\">";
				}
			document.getElementById("ParkControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_parkcall_OFF.gif\" border=0 alt=\"Parkera samtal\">";
			document.getElementById("HangupControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_hangupcustomer_OFF.gif\" border=0 alt=\"Lägg på kund\">";
			document.getElementById("XferControl").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_transferconf_OFF.gif\" border=0 alt=\"Vidarekoppla - Konferens\">";
			document.getElementById("LocalCloser").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_localcloser_OFF.gif\" border=0 alt=\"LOKAL CLOSER\" style=\"vertical-align:middle\">";
			document.getElementById("DialBlindTransfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_XB_blindtransfer_OFF.gif\" border=0 alt=\"Ring blindnummer\" style=\"vertical-align:middle\">";
...
// Send Hangup command for 3rd party call connected to the conference now to Manager
	function xfercall_send_hangup() 
		...
		//	document.getElementById("Leave3WayCall").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_leave3waycall_OFF.gif\" border=0 alt=\"Lämna 3-partssamtal\">";

			document.getElementById("DialWithCustomer").innerHTML ="<a href=\"#\" onclick=\"SendManualDial('YES');return false;\"><IMG SRC=\"../agc/images/vdc_XB_dialwithcustomer.gif\" border=0 alt=\"Ring med kund\" style=\"vertical-align:middle\"></a>";

			document.getElementById("ParkCustomerDial").innerHTML ="<a href=\"#\" onclick=\"xfer_park_dial();return false;\"><IMG SRC=\"../agc/images/vdc_XB_parkcustomerdial.gif\" border=0 alt=\"Parkera kunduppringning\" style=\"vertical-align:middle\"></a>";

			document.getElementById("HangupXferLine").innerHTML ="<IMG SRC=\"../agc/images/vdc_XB_hangupxferline_OFF.gif\" border=0 alt=\"Lägg på xfer linje\" style=\"vertical-align:middle\">";
...
// Generate the Call Disposition Chooser panel
	function DispoSelectContent_create(taskDSgrp,taskDSstage)
		...
		var dispo_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td colspan=2><B> SAMTAL DISPOSITION</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=DispoSelectA>";
		var loop_ct = 0;
...
// Generate the Paus Code Chooser panel
	function PauseCodeSelectContent_create()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
			{
			alert("DU MÅSTE VARA I PAUSLÄGE FÖR ATT SKRIVA IN EN PAUSKOD NÄR RINGERN STÅR I AUTODIAL");
			}
		else
			{
			HidEGenDerPulldown();
...
// open web form, then submit disposition
	function WeBForMDispoSelect_submit()
		...
		if (DispoChoice.length < 1) {alert("Du måste välja utfall");}
		else
			{
...
// Generate the Closer In Group Chooser panel
	function CloserSelectContent_create()
		...
		if ( (VU_agent_choose_ingroups == '1') && (manager_ingroups_set < 1) )
			{
			var live_CSC_HTML = "<table cellpadding=5 cellspacing=5 width=500><tr><td><B>EJ VALDA GRUPPER</B></td><td><B>VALDA GRUPPER</B></td></tr><tr><td bgcolor=\"#99FF99\" height=300 width=240 valign=top><font class=\"log_text\"><span id=CloserSelectAdd> &nbsp; <a href=\"#\" onclick=\"CloserSelect_change('-----ADD-ALL-----','ADD');return false;\"><B>--- ADD ALL ---</B><BR>";
			var loop_ct = 0;
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				if (alt_dial_status_display==1)
					...
					var logout_content='';
					if (tempreason=='SHIFT')
						{logout_content='Ditt skift är slut eller har ändrats, du är nu urloggad från din session<BR><BR>';}
...
// Grab the call in queue and bring it into the session
	function callinqueuegrab(CQauto_call_id)
		...
		if (CQauto_call_id > 0)
			...
			if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
				{
				alert("DU MÅSTE VARA I PAUSLÄGE FÖR ATT TA SAMTAL FRÅN KÖN");
				}
			else
				{
				var xmlhttp=false;
...
// Visa the groups selection span
	function OpeNGrouPSelectioN()
		...
		if ( (AutoDialWaiting == 1) || (VD_live_customer_call==1) || (alt_dial_active==1) || (MD_channel_look==1) )
			{
			alert("DU MÅSTE VARA I PAUS FÖR ATT BYTA GRUPP");
			}
		else
			{
			if (manager_ingroups_set > 0)
				{
				alert("Manager" + external_igb_set_name + " har valt dina ingruppsval");
				}
			else
				{
				HidEGenDerPulldown();
	...
	function TimerActionRun(taskaction,taskdialalert)
		...
		if (taskaction == 'DiaLAlerT')
			...
			if ( (timer_action_message.length > 0) || (timer_action == 'MESSAGE_ONLY') )
				{
				document.getElementById("TimerContentSpan").innerHTML = "<b>TIMERNOTIFIERING: " + timer_action_seconds + " sekunder<BR><BR>" + timer_action_message + "</b>";
...
// GLOBAL FUNCTIONS
	...
	function HotKeys(HKstate) 
		...
		if ( (HKstate == 'ON') && (HKbutton_allowed == 1) )
			...
			showDiv('HotKeyEntriesBox');
			hot_keys_active = 1;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOut=\"HotKeys('OFF')\"><IMG SRC=\"../agc/images/vdc_XB_hotkeysactive.gif\" border=0 alt=\"SNABBTANGENTER ÄR AKTIVA\"></a>";
			}
		else
			{
			hideDiv('HotKeyEntriesBox');
			hot_keys_active = 0;
			document.getElementById("hotkeysdisplay").innerHTML = "<a href=\"#\" onMouseOver=\"HotKeys('ON')\"><IMG SRC=\"../agc/images/vdc_XB_hotkeysactive_OFF.gif\" border=0 alt=\"SNABBTANGENTER ÄR INAKTIVA\"></a>";
			}
		}
	...
	function ShoWTransferMain(showxfervar,showoffvar)
		...
		if (VU_vicidial_transfers == '1')
			...
			if (showxfervar == 'ON')
				...
				document.getElementById("XferControl").innerHTML = "<a href=\"#\" onclick=\"ShoWTransferMain('OFF','YES');\"><IMG SRC=\"../agc/images/vdc_LB_transferconf.gif\" border=0 alt=\"Vidarekoppla - Konferens\"></a>";
				if (quick_transfer_button_enabled > 0)
					{document.getElementById("QuickXfer").innerHTML = "<IMG SRC=\"../agc/images/vdc_LB_quickxfer_OFF.gif\" border=0 alt=\"SNABBKOPPLA\">";}
				}
			else
			...
			if (showxfervar != 'OFF')
				{
				alert('DU HAR EJ BEHÖRIGHET ATT VIDAREKOPPLA SAMTAL');
				}
			}
		}
...
<?php	echo "Inloggad som användare: $VD_login på Telefon: $SIP_user till kampanj: $VD_campaign&nbsp; \n"; ?>
...
<TD WIDTH=109><IMG SRC="../agc/images/agc_live_call_OFF.gif" NAME=livecall ALT="Livesamtal" WIDTH=109 HEIGHT=30 Border=0></TD>
</TR></TABLE>
</span>



<span style="position:absolute;left:0px;top:0px;z-index:3;" id="VälkommenBoxA">
    <table border=0 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $HKwidth ?>><TR><TD align=center><BR><span id="VälkommenBoxAt">Agent Screen</span></TD></TR></TABLE>
</span>
...
<span id="CBstatusSpan">X AKTIVA ÅTERKOMSTER</span> <BR>
</font></span>
...
<span style="position:absolute;left:<?php echo $SCwidth ?>px;top:49px;z-index:18;" id="SecondSspan"><font class="body_text"> sekunder: 
...
<span style="position:absolute;left:0px;top:0px;z-index:51;" id="CallBacKsLisTBox">
    <table border=1 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> PERSONLIGA ÅTERKOMSTER <?php echo $VD_login ?>:<BR>Klicka på en återkomst nedan för att ringa, numret försvinner då från listan och du får välja nytt utfall.
	<BR>
	<div class="scroll_callback" id="CallBacKsLisT"></div>
	<BR> &nbsp; 
	<a href="#" onclick="CalLBacKsLisTCheck();return false;">Uppdatera</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('CallBacKsLisTBox');return false;">Tillbaka</a>
	</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:52;" id="NeWManuaLDiaLBox">
    <table border=1 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> NYTT MANUELLT UPPRINGT LEAD FÖR <?php echo "$VD_login in campaign $VD_campaign" ?>:<BR><BR>Skriv in informationen för det nya leadet du vill ringa.
	<BR>
	<?php 
	if (eregi("X",$dial_prefix))
		{
		echo "Notera: ett prefix för $dial_prefix kommer att läggas till i början av numret<BR>\n";
		}
	?>
	Note: all new manual dial leads will go into list <?php echo $manual_dial_list_id ?><BR><BR>
	<table><tr>
	<td align=right><font class="body_text"> Landskod </td>
	<td align=left><font class="body_text"><input type=text size=7 maxlength=10 name=MDDiaLCodE class="cust_form" value="1">&nbsp; (Detta är 46 för Sverige)</td>
	</tr><tr>
	...
	<td align=right><font class="body_text"> Sök existerande leads: </td>
	<td align=left><font class="body_text"><input type=checkbox name=LeadLookuP size=1 value="0">&nbsp; (Detta val söker efter telefonnumret i systemet innan ett nytt lead läggs till.)</td>
	</tr><tr>
	...
	<td align=left><font class="body_text"><input type=text size=24 maxlength=20 name=MDDiaLOverridE class="cust_form" value="">&nbsp; (enbart siffror)
	</td>
	</tr></table>
	<BR>
	...
	<a href="#" onclick="NeWManuaLDiaLCalLSubmiT('PREVIEW');return false;">Förhandsgranska samtal</a>
	 &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp;  &nbsp; 
	<a href="#" onclick="hideDiv('NeWManuaLDiaLBox');return false;">Tillbaka</a>
	</TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:157px;top:<?php echo $HTheight ?>px;z-index:32;" id="TransferMain">
	...
	<IMG SRC="../agc/images/vdc_XB_header.gif" border=0 alt="Vidarekoppla - Konferens" style="vertical-align:middle"> &nbsp; &nbsp; &nbsp; &nbsp; <span id="XfeRDiaLGrouPSelecteD"></span> &nbsp; &nbsp; <span id="XfeRCID"></span><BR>
	...
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="LocalCloser"><IMG SRC="../agc/images/vdc_XB_localcloser_OFF.gif" border=0 alt="LOKAL CLOSER" style="vertical-align:middle"></span> &nbsp; &nbsp;
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupXferLine"><IMG SRC="../agc/images/vdc_XB_hangupxferline_OFF.gif" border=0 alt="Lägg på xfer linje" style="vertical-align:middle"></span>
	</TD>
	</TR>
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=consultativexfer size=1 value="0"><font class="body_tiny"> KONSULTERANDE &nbsp;</font>
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="HangupBothLines"><a href="#" onclick="bothcall_send_hangup();return false;"><IMG SRC="../agc/images/vdc_XB_hangupbothlines.gif" border=0 alt="Lägg på båda linjerna" style="vertical-align:middle"></a></span>
	</TD>
	</TR>

	<TR>
	<TD ALIGN=LEFT COLSPAN=2>
	<IMG SRC="../agc/images/vdc_XB_number.gif" border=0 alt="Nummer att ringa" style="vertical-align:middle">
	&nbsp; 
	...
	<TD ALIGN=LEFT>
	<input type=checkbox name=xferoverride size=1 value="0"><font class="body_tiny"> RING OVERRIDE</font>
	</TD>
	<TD ALIGN=LEFT>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="Leave3WayCall"><a href="#" onclick="leave_3way_call('FIRST');return false;"><IMG SRC="../agc/images/vdc_XB_leave3waycall.gif" border=0 alt="Lämna 3-partssamtal" style="vertical-align:middle"></a></span> 
	</TD>
	</TR>

	<TR>
	<TD ALIGN=LEFT COLSPAN=4>
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialBlindTransfer"><IMG SRC="../agc/images/vdc_XB_blindtransfer_OFF.gif" border=0 alt="Ring blindnummer" style="vertical-align:middle"></span>
	&nbsp;
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="DialWithCustomer"><a href="#" onclick="SendManualDial('YES');return false;"><IMG SRC="../agc/images/vdc_XB_dialwithcustomer.gif" border=0 alt="Ring med kund" style="vertical-align:middle"></a></span> 
	&nbsp;
	<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="ParkCustomerDial"><a href="#" onclick="xfer_park_dial();return false;"><IMG SRC="../agc/images/vdc_XB_parkcustomerdial.gif" border=0 alt="Parkera kunduppringning" style="vertical-align:middle"></a></span> 
	&nbsp;
...
<span style="position:absolute;left:0px;top:<?php echo $CQheight ?>px;width:<?php echo $MNwidth ?>;overflow:scroll;z-index:23;background-color:<?php echo $SIDEBAR_COLOR ?>;" id="callsinqueuedisplay"><TABLE CELLPADDING=0 CELLSPACING=0 Border=0><TR><TD width=5 ROWSPAN=2>&nbsp;</TD><TD ALIGN=CENTER><font class="body_text">Samtalskö: &nbsp; </font></TD></TR><TR><TD ALIGN=CENTER><span id="callsinqueuelist">&nbsp;</span></TD></TR></TABLE></span>
...
Tillgängliga agenter koppla: <span id="AgentXferViewSelect"></span></CENTER></font></span>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:34;" id="HotKeyActionBox">
	...
	<TR bgcolor="#FFEEBB"><TD height=70><font class="sh_text"> Leadet utföll som: </font><BR><BR><CENTER>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:35;" id="HotKeyEntriesBox">
    <table border=0 bgcolor="#FFDD99" width=<?php echo $HCwidth ?> height=70>
	<TR bgcolor="#FFEEBB"><TD width=200><font class="sh_text"> Snabbtangenter: </font></td><td colspan=2>
...
<span style="position:absolute;left:5px;top:<?php echo $HTheight ?>px;z-index:36;" id="CBcommentsBox">
	...
	<TR bgcolor="#FFFF66">
	<TD align=left><font class="sh_text"> Informaci�n Anterior Del Servicio repetido: </font></td>
...
<span style="position:absolute;left:0px;top:12px;z-index:39;" id="NoneInSessionBox">
    <table border=1 bgcolor="#CCFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center> Ingen är i din session: <span id="NoneInSessionID"></span><BR>
	<a href="#" onclick="NoneInSessionOK();return false;">Tillbaka</a>
	<BR><BR>
	<a href="#" onclick="NoneInSessionCalL();return false;">Ring agent igen</a>
	</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:40;" id="CustomerGoneBox">
    <table border=1 bgcolor="#CCFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center> Kunden har lagt på: <span id="CustomerGoneChanneL"></span><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:41;" id="WrapupBox">
    <table border=1 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center> Call Wrapup: <span id="WrapupTimer"></span> kvarvarande sekunder till avslut<BR><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:43;" id="AgenTDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center>Din session har avbrytits<BR><a href="#" onclick="LogouT('DISABLED');return false;">LOGGA UT</a><BR><BR><a href="#" onclick="hideDiv('AgenTDisablEBoX');return false;">Tillbaka</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:44;" id="SysteMDisablEBoX">
    <table border=1 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center>Det är problem med tidssynkroniseringen, kontakta din systemadministratör<BR><BR><BR><a href="#" onclick="hideDiv('SysteMDisablEBoX');return false;">Tillbaka</a>
</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:45;" id="LogouTBox">
    <table border=1 bgcolor="#FFFFFF" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center><BR><span id="LogouTBoxLink">LOGGA UT</span></TD></TR></TABLE>
</span>
...
<span style="position:absolute;left:0px;top:0px;z-index:49;" id="DispoSelectBox">
	...
	<BR><BR>
	<a href="#" onclick="WeBForMDispoSelect_submit();return false;">WEB FORM SKICKA</a>
	<BR><BR> &nbsp; 
	</TD></TR></TABLE>
</span>

<span style="position:absolute;left:0px;top:0px;z-index:64;" id="PauseCodeSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> VÄLJ PAUSKOD :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:65;" id="GroupAliasSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> VÄLJ ETT GRUPPALIAS :<BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:50;" id="CallBackSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> Välj ett datum för återkomsten :<span id="CallBackDatE"></span><BR>
	...
	if ($agentonly_callbacks)
		{echo "<input type=checkbox name=CallBackOnlyMe id=CallBackOnlyMe size=1 value=\"0\"> PERSONLIG ÅTERKOMST <BR>";}
	?>
	Kommentar till ÅK: <input type=text name="CallBackCommenTsField" id="CallBackCommenTsField" size=50 maxlength=255><BR><BR>
...
<span style="position:absolute;left:0px;top:0px;z-index:53;" id="CloserSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=<?php echo $CAwidth ?> height=<?php echo $WRheight ?>><TR><TD align=center VALIGN=top> INGÅENDE CLOSER GRUPPVAL <BR>
	...
	if ( ($outbound_autodial_active > 0) and ($disable_blended_checkbox < 1) and ($dial_method != 'INBOUND_MAN') )
		{
		?>
		<input type=checkbox name=CloserSelectBlended size=1 value="0"> MIXAD RINGNING(utgående aktiverat) <BR>
		<?php
		}
	?>
...
<span style="position:absolute;left:0px;top:0px;z-index:55;" id="NothingBox">
	...
	<?php
	if (!$agentonly_callbacks)
		{echo "<input type=checkbox name=CallBackOnlyMe size=1 value=\"0\"> PERSONLIG ÅTERKOMST <BR>";}
	if ( ($outbound_autodial_active < 1) or ($disable_blended_checkbox > 0) or ($dial_method == 'INBOUND_MAN') )
		{echo "<input type=checkbox name=CloserSelectBlended size=1 value=\"0\"> MIXAD RINGNING<BR>";}
	?>
</span>
...
<span style="position:absolute;left:<?php echo $HKwidth ?>px;top:<?php echo $HKheight ?>px;z-index:16;" id="hotkeysdisplay"><a href="#" onMouseOver="HotKeys('ON')"><IMG SRC="../agc/images/vdc_XB_hotkeysactive_OFF.gif" border=0 alt="SNABBTANGENTER ÄR INAKTIVA"></a></span>
<?php } ?>


<span style="position:absolute;left:0px;top:<?php echo $HKheight ?>px;z-index:15;" id="MaiNfooterspan">
<table BGCOLOR="<?php echo $MAIN_COLOR ?>" id="MaiNfooter" width=<?php echo $MNwidth ?>><tr height=32><td height=32><font face="Arial,Helvetica" size=1>Agent web-client version: <?php echo $version ?> &nbsp; &nbsp; SKAPA: <?php echo $build ?> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; Server: <?php echo $server_ip ?>  &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; &nbsp;</font><BR>
...
<span STYLE="background-color: #CCFFCC" id="DiaLControl"><a href="#" onclick="ManualDialNext('','','','','','0');"><IMG SRC="../agc/images/vdc_LB_dialnextnumber_OFF.gif" border=0 alt="Ring nästa nummer"></a></span><BR>
...
<center>
<!-- <a href=\"#\" onclick=\"conf_send_recording('MonitorConf','" + head_conf + "','');return false;\">Spela in</a> -->
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="RecorDControl"><a href="#" onclick="conf_send_recording('MonitorConf',session_id,'');return false;"><IMG SRC="../agc/images/vdc_LB_startrecording.gif" border=0 alt="Starta inspelning"></a></span><BR>
<span id="SpacerSpanA"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: #FFFFFF" id="WebFormSpan"><IMG SRC="../agc/images/vdc_LB_webform_OFF.gif" border=0 alt="Webformulär"></span><BR>
<?php
if ($enable_second_webform > 0)
	{echo "<span STYLE=\"background-color: #FFFFFF\" id=\"WebFormSpanTwo\"><IMG SRC=\"../agc/images/vdc_LB_webform_two_OFF.gif\" border=0 alt=\"Webformulär 2\"></span><BR>\n";}
?>
<span id="SpacerSpanB"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="ParkControl"><IMG SRC="../agc/images/vdc_LB_parkcall_OFF.gif" border=0 alt="Parkera samtal"></span><BR>
<span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="XferControl"><IMG SRC="../agc/images/vdc_LB_transferconf_OFF.gif" border=0 alt="Vidarekoppla - Konferens"></span><BR>
...
<span STYLE="background-color: #FFCCFF" id="HangupControl"><IMG SRC="../agc/images/vdc_LB_hangupcustomer_OFF.gif" border=0 alt="Lägg på kund"></span><BR>
<span id="SpacerSpanD"><IMG SRC="../agc/images/blank.gif" width=145 height=16 border=0></span><BR>
<div class="text_input" id="SendDTMFdiv"><span STYLE="background-color: <?php echo $MAIN_COLOR ?>" id="SendDTMF"><a href="#" onclick="SendConfDTMF(session_id);return false;"><IMG SRC="../agc/images/vdc_LB_senddtmf.gif" border=0 alt="Skicka DTMF" align=bottom></a>  <input type=text size=5 name=conf_dtmf class="cust_form" value="" maxlength=50></div></span><BR>
</center>
</font>
</td>
...
<td colspan=2 align=center> Kundinformation: <span id="CusTInfOSpaN"></span></td>
</tr><tr>
<td align=left colspan=2>
...
<td align=right><font class="body_text">Stat: </td>
<td align=left><font class="body_text"><input type=text size=4 name=state maxlength=2 class="cust_form" value=""></td>
<td align=right><font class="body_text">Postnummer: </td>
<td align=left><font class="body_text"><input type=text size=14 name=postal_code maxlength=10 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> Provins: </td>
<td align=left><font class="body_text"><input type=text size=20 name=province maxlength=50 class="cust_form" value=""></td>
<td align=right><font class="body_text">VendorID: </td>
<td align=left><font class="body_text"><input type=text size=15 name=vendor_lead_code maxlength=20 class="cust_form" value=""></td>
<td align=right><font class="body_text">Kön:</td>
<td align=left><font class="body_text"><span id="GENDERhideFORie"><select size=1 name=gender_list class="cust_form" id=gender_list><option value="U">U - Odefinierad</option><option value="M">M - Male</option><option value="F">F - Female</option></select></span></td>
</tr><tr>
...
<td align=right valign=top><font class="body_text"> Kommentarer:</td>
