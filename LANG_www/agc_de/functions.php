...
function user_authorization($user,$pass,$user_option,$user_update,$bcrypt,$return_hash)
	...
	$LOCK_over = ($STARTtime - 900); # failed login lockout time is 15 minutes(900 Sekunden)
...
function custom_list_fields_values($lead_id,$list_id,$uniqueid,$user)
	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			$CFoutput .= "<input type=hidden name=stage id=stage value=\"ÜBERNEHMEN\">\n";
	...
	if (preg_match("/--A--/",$CFoutput))
		...
		if (preg_match('/--A--TABLEper_call_notes--B--/i',$CFoutput))
			...
			if ($hide_call_log_info!='Y')
				{
				if ($search != 'logfirst')
					{$NOTESout .= "CALL-Protokoll für diesen LEAD:<br>\n";}
...
function lookup_gmt($phone_code,$USarea,$state,$LOCAL_GMT_OFF_STD,$Shour,$Smin,$Ssec,$Smon,$Smday,$Syear,$postalgmt,$postal_code)
	...
	if ( (!$AC_processed) and ($dst_range == 'SSM-FSN') )
		{
		if ($DBX) {print "     Second Sunday März to Vorname Sunday November\n";}
		#**********************************************************************
		# SSM-FSN
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an Second Sunday März to Vorname Sunday November at 2 am.
		#     INPUTS:
		#       mm              INTEGER       Month.
		...
		#                                      (DEFAULT is blank)
		#                                     make calculations based an UTC time, 
		#                                     which means shift at 10:00 UTC in April
		#                                     and 9:00 UTC in Oktober
		#     OUTPUT: 
	...
	if ( (!$AC_processed) and ($dst_range == 'FSA-LSO') )
		{
		if ($DBX) {print "     Vorname Sunday April to Nachname Sunday Oktober\n";}
		#**********************************************************************
		# FSA-LSO
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an first Sunday in April and last Sunday in Oktober at 2 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSM-LSO') )
		{
		if ($DBX) {print "     Nachname Sunday März to Nachname Sunday Oktober\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an last Sunday in März and last Sunday in Oktober at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSO-LSM') )
		{
		if ($DBX) {print "     Nachname Sunday Oktober to Nachname Sunday März\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an last Sunday in Oktober and last Sunday in März at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-LSM') )
		{
		if ($DBX) {print "     Vorname Sunday Oktober to Nachname Sunday März\n";}
		#**********************************************************************
		#   TASMANIA ONLY
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an first Sunday in Oktober and last Sunday in März at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-FSA') )
		{
		if ($DBX) {print "     Sunday in Oktober to Vorname Sunday in April\n";}
		#**********************************************************************
		# FSO-FSA
		#   2008+ AUSTRALIA ONLY (country code 61)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an first Sunday in Oktober and first Sunday in April at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-TSM') )
		{
		if ($DBX) {print "     Vorname Sunday Oktober to Third Sunday März\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an first Sunday in Oktober and third Sunday in März at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSS-FSA') )
		{
		if ($DBX) {print "     Nachname Sunday in September to Vorname Sunday in April\n";}
		#**********************************************************************
		# LSS-FSA
		#   2007+ NEU ZEALAND (country code 64)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based an last Sunday in September and first Sunday in April at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'TSO-LSF') )
		{
		if ($DBX) {print "     Third Sunday Oktober to Nachname Sunday February\n";}
		#**********************************************************************
		# TSO-LSF
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect. Brazil
		#     Based an Third Sunday Oktober to Nachname Sunday February at 1 am.
		#**********************************************************************
