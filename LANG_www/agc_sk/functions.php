...
function user_authorization($user,$pass,$user_option,$user_update,$bcrypt,$return_hash)
	...
	$LOCK_over = ($STARTtime - 900); # failed login lockout time is 15 minutes(900 sekúnd)
	...
	if ($auth < 1)
		...
		if ( (preg_match("/MXAG/",$SShosted_settings)) and ($mvla_total < 1) )
			...
			$vla_set = preg_replace("/.*MXAG|_POSTAVENÉ_|DRA| /",'',$vla_set);
...
function custom_list_fields_values($lead_id,$list_id,$uniqueid,$user)
	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			$CFoutput .= "<input type=hidden name=stage id=stage value=\"POTVRDIŤ\">\n";
	...
	if (preg_match("/--A--/",$CFoutput))
		...
		if (preg_match('/--A--TABLEper_call_notes--B--/i',$CFoutput))
			...
			if ($hide_call_log_info!='Y')
				{
				if ($search != 'logfirst')
					{$NOTESout .= "VÝZVA PRE LOG tohto kábla:<br>\n";}
...
function lookup_gmt($phone_code,$USarea,$state,$LOCAL_GMT_OFF_STD,$Shour,$Smin,$Ssec,$Smon,$Smday,$Syear,$postalgmt,$postal_code)
	...
	if ( (!$AC_processed) and ($dst_range == 'SSM-FSN') )
		{
		if ($DBX) {print "     Second Sunday Marec to Prvý Sunday November\n";}
		#**********************************************************************
		# SSM-FSN
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na Second Sunday Marec to Prvý Sunday November at 2 am.
		#     INPUTS:
		#       mm              INTEGER       Month.
		...
		#                                     which means shift at 10:00 UTC in Apríl
		#                                     and 9:00 UTC in Október
		#     OUTPUT: 
	...
	if ( (!$AC_processed) and ($dst_range == 'FSA-LSO') )
		{
		if ($DBX) {print "     Prvý Sunday Apríl to Posledný Sunday Október\n";}
		#**********************************************************************
		# FSA-LSO
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na first Sunday in Apríl and last Sunday in Október at 2 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSM-LSO') )
		{
		if ($DBX) {print "     Posledný Sunday Marec to Posledný Sunday Október\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na last Sunday in Marec and last Sunday in Október at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSO-LSM') )
		{
		if ($DBX) {print "     Posledný Sunday Október to Posledný Sunday Marec\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na last Sunday in Október and last Sunday in Marec at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-LSM') )
		{
		if ($DBX) {print "     Prvý Sunday Október to Posledný Sunday Marec\n";}
		#**********************************************************************
		#   TASMANIA ONLY
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na first Sunday in Október and last Sunday in Marec at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-FSA') )
		{
		if ($DBX) {print "     Sunday in Október to Prvý Sunday in Apríl\n";}
		#**********************************************************************
		# FSO-FSA
		#   2008+ AUSTRALIA ONLY (country code 61)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na first Sunday in Október and first Sunday in Apríl at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-TSM') )
		{
		if ($DBX) {print "     Prvý Sunday Október to Third Sunday Marec\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na first Sunday in Október and third Sunday in Marec at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSS-FSA') )
		{
		if ($DBX) {print "     Posledný Sunday in September to Prvý Sunday in Apríl\n";}
		#**********************************************************************
		# LSS-FSA
		#   2007+ NOVÁ ZEALAND (country code 64)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based na last Sunday in September and first Sunday in Apríl at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'TSO-LSF') )
		{
		if ($DBX) {print "     Third Sunday Október to Posledný Sunday February\n";}
		#**********************************************************************
		# TSO-LSF
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect. Brazil
		#     Based na Third Sunday Október to Posledný Sunday February at 1 am.
		#**********************************************************************
