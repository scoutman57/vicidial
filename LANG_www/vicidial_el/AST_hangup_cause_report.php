...
$master_dialstatus_array=array("ΑΠΑΝΤΗΣΗ", "BUSY", "NOΑΠΑΝΤΗΣΗ", "CANCEL", "CONGESTION", "CHANUNAVAIL", "DONTCALL", "TORTURE", "INVALIDARGS");
...
if  (preg_match('/\-\-ALL\-\-/',$dialstatuses_string))
	{$MAIN.="<option value=\"--ALL--\" selected>-- ALL ΤΗΛΕΦΩΝΕΙΣΤΕ καθεστώτων --</option>\n";}
else
	{$MAIN.="<option value=\"--ALL--\">-- ALL ΤΗΛΕΦΩΝΕΙΣΤΕ καθεστώτων --</option>\n";}
...
if ($ΕΠΙΒΕΒΑΙΩΣΗ && $server_ip_ct>0) {
	...
	if (mysqli_num_rows($rslt)>0) {
		...
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+--------------+--------------+--------------------------------+\n";
		$carrier_rpt.="| UNIQUE ID            | CALL DATE           | ΔΙΑΚΟΜΙΣΤΗΣ IP       | LEAD ID   | HANGUP CAUSE | DIAL STATUS | CHANNEL                                  | DIAL TIME | ΑΠΑΝΤΗΣΗED TIME | PHONE NUMBER | SIP RESPONSE | SIP REASON                     |\n";
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+--------------+--------------+--------------------------------+\n";
		$CSV_text="\"UNIQUE ID\",\"CALL DATE\",\"ΔΙΑΚΟΜΙΣΤΗΣ IP\",\"LEAD ID\",\"HANGUP CAUSE\",\"DIAL STATUS\",\"CHANNEL\",\"DIAL TIME\",\"ΑΠΑΝΤΗΣΗED TIME\",\"PHONE NUMBER\",\"SIP RESPONSE\",\"SIP REASON\"\n";
		...
		$ll=$lower_limit-1000;
		if ($ll>=1) {
			$carrier_rpt_hf.="<a href=\"$PHP_SELF?ΕΠΙΒΕΒΑΙΩΣΗ=$ΕΠΙΒΕΒΑΙΩΣΗ&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$hangup_causeQS$sip_hangup_causeQS$dial_statusQS&lower_limit=$ll\">[<<< PREV 1000 records]</a>";
		} else {
		...
		if (($lower_limit+1000)<mysqli_num_rows($rpt_rslt)) {
			...
			$carrier_rpt_hf.="<a href=\"$PHP_SELF?ΕΠΙΒΕΒΑΙΩΣΗ=$ΕΠΙΒΕΒΑΙΩΣΗ&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$hangup_causeQS$sip_hangup_causeQS$dial_statusQS&lower_limit=".($lower_limit+1000)."\">[NEXT $max_limit records >>>]</a>";
		} else {
			$carrier_rpt_hf.=sprintf("%23s", " ");
		}
		$carrier_rpt_hf.="\n";
