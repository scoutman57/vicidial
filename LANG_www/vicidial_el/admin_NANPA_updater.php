...
if (strlen($active_voicemail_server)<7)
	{
	echo "ERROR: Admin -> Τοποθετήσεις συστημάτων -> Ενεργή ΦΩΝΗΤΙΚΟ ΤΑΧΥΔΡΟΜΕΙΟ Server δεν έχει ρυθμιστεί\n";
	exit;
	}
...
$list_ct=count($lists);
if ($submit_form=="ΕΠΙΒΕΒΑΙΩΣΗ" && $list_ct>0 && (strlen($vl_field_update)>0 || strlen($cellphone_list_id)>0 || strlen($landline_list_id)>0 || strlen($invalid_list_id)>0) ) 
	{
	for ($i=0; $i<$list_ct; $i++) 
		{
		if ($lists[$i]=="---ALL---") 
...
if (mysqli_num_rows($schedule_rslt)>0 || (mysqli_num_rows($running_rslt)>0)) {
	...
	if (mysqli_num_rows($schedule_rslt)>0) {
		...
		echo "<td align='left' bgcolor='#015B91' width='300'><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE SIZE=2>Λίστες</th>";
		...
		while ($row=mysqli_fetch_array($schedule_rslt)) {
			...
			echo "<td align='center'><FONT FACE=\"ARIAL,HELVETICA\" size='1'><a href='$PHP_SELF?delete_trigger_id=$row[trigger_id]'>ΔΙΑΓΡΑΦΗ</a></font></td>";
			echo "</tr>";
		}
		echo "</table>";
		echo "</td></tr>";
...
if ( ( (mysqli_num_rows($schedule_rslt)>0) or (mysqli_num_rows($running_rslt)>0) ) and ($block_scheduling_while_running==1) ) 
	...
	echo "";
	echo "<tr><td align='center' colspan='5'><input type='submit' value='ΕΠΙΒΕΒΑΙΩΣΗ' name='submit_form'></td></tr>";
	echo "</table>";
