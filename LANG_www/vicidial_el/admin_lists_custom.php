...
if ($action == "ΒΟΗΘΕΙΑ")
	...
	<BR>
	<B>ViciDial Τομέας συνήθειας καταλόγωνs Help</B>
	<BR><BR>

	<A NAME="vicidial_lists_fields-field_label">
	<BR>
	 <B>Ετικέτα τομέων -</B> Αυτό είναι το προσδιοριστικό τομέων βάσεων δεδομένων για αυτόν τον τομέα. Αυτό πρέπει να είναι ένα μοναδικό προσδιοριστικό μέσα στους τομείς συνήθειας για αυτόν τον κατάλογο. Μην χρησιμοποιήστε οποιαδήποτε διαστήματα ή στίξη για αυτόν τον τομέα. ανώτατοι 50 χαρακτήρες, ελάχιστο 2 χαρακτήρων. Μπορείτε επίσης να συμπεριλάβετε τα προεπιλεγμένα ViciDial πεδία σε μια προσαρμοσμένη ρύθμιση στον τομέα, και θα δείτε τους σε κόκκινο χρώμα στη λίστα. Αυτά τα πεδία δεν θα προστεθεί στην προσαρμοσμένη λίστα πίνακα της βάσης δεδομένων, η διεπαφή θα πράκτορα αναφοράς αντί του πίνακα vicidial_list άμεσα. Οι ετικέτες που μπορείτε να χρησιμοποιήσετε για να συμπεριλάβετε τις fieds προεπιλογή είναι - 
...
if ($SScustom_fields_enabled < 1)
	{
	echo "ΛΑΘΟΣ: Οι τομείς συνήθειας δεν είναι ενεργοί σε αυτό το σύστημα\n";
	exit;
	}
...
if ( ($action == "COPY_FIELDS_ΕΠΙΒΕΒΑΙΩΣΗ") and ($list_id > 99) and ($source_list_id > 99) and (strlen($copy_option) > 2) )
	...
	if ($list_id=="$source_list_id")
		{echo "ΛΑΘΟΣ: Δεν μπορείτε να αντιγράψετε τους τομείς στον ίδιο κατάλογο: $list_id|$source_list_id";}
	else
		{
		$table_exists=0;
		...
		if ($source_field_exists < 1)
			{echo "ΛΑΘΟΣ: Ο κατάλογος πηγής δεν έχει κανέναν τομέα συνήθειας\n<BR>";}
		else
			{
			##### REPLACE option #####
			...
			if ($copy_option=='UPDATE')
				...
				if ($table_exists < 1)
					{echo "ΛΑΘΟΣ: Ο πίνακας δεν υπάρχει custom_$list_id\n<BR>";}
				else
					{
					$stmt="SELECT field_id,field_label,field_name,field_description,field_rank,field_help,field_type,field_options,field_size,field_max,field_default,field_cost,field_required,multi_position,name_position,field_order from vicidial_lists_fields where list_id='$source_list_id' order by field_rank,field_order,field_label;";
					$rslt=mysql_query($stmt, $link);
...
if ( ($action == "DELETE_CUSTOM_FIELD_CONFIRMATION") and ($list_id > 99) and ($field_id > 0) and (strlen($field_label) > 0) )
	...
	if ($field_exists < 1)
		{echo "ΛΑΘΟΣ: Ο τομέας δεν υπάρχει\n<BR>";}
	else
		{
		if ($table_exists < 1)
			{echo "ΛΑΘΟΣ: Ο πίνακας δεν υπάρχει custom_$list_id\n<BR>";}
...
if ( ($action == "ADD_CUSTOM_FIELD") and ($list_id > 99) )
	...
	if ( (strlen($field_label)<1) or (strlen($field_name)<2) or (strlen($field_size)<1) )
		...
		if (preg_match("/\|$field_label\|/i",$mysql_reserved_words))
			{echo "ΣΦΑΛΜΑ: Δεν μπορείτε να χρησιμοποιήσετε δεσμευμένες λέξεις για τις ετικέτες τομέα - $list_id|$field_label|$field_name|$field_size\n<BR>";}
		else
			{
			...
			if ( ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') ) and ( (!preg_match("/,/",$field_options)) or (!preg_match("/\n/",$field_options)) or (strlen($field_options)<6) or ($TEST_valid_options < 1) ) )
				{echo "ERROR: Θα πρέπει να εισάγετε τις επιλογές τομέα όταν η προσθήκη ενός SELECT, MULTI, RADIO or CHECKBOX field type  - $list_id|$field_label|$field_type|$field_options\n<BR>";}
			else
				{
				if ($field_exists > 0)
					{echo "ΛΑΘΟΣ: Ο τομέας υπάρχει ήδη για αυτόν τον κατάλογο - $list_id|$field_label\n<BR>";}
				else
					{
					$table_exists=0;
					$linkCUSTOM=mysql_connect("$VARDB_server:$VARDB_port", "$VARDB_custom_user","$VARDB_custom_pass");
...
if ( ($action == "MODIFY_CUSTOM_FIELD_ΕΠΙΒΕΒΑΙΩΣΗ") and ($list_id > 99) and ($field_id > 0) )
	...
	if ($field_exists < 1)
		...
		{
		if ($table_exists < 1)
			{echo "ΛΑΘΟΣ: Ο πίνακας δεν υπάρχει\n<BR>";}
		else
			{
			$TEST_valid_options=0;
			if ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') )
			...
			if ( ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') ) and ( (!preg_match("/,/",$field_options)) or (!preg_match("/\n/",$field_options)) or (strlen($field_options)<6) or ($TEST_valid_options < 1) ) )
				{echo "ERROR: Θα πρέπει να εισάγετε τις επιλογές τομέα κατά την ενημέρωση ενός SELECT, MULTI, RADIO or CHECKBOX field type  - $list_id|$field_label|$field_type|$field_options\n<BR>";}
			else
				{
				### modify field function
...
if ( ($action == "MODIFY_CUSTOM_FIELDS") and ($list_id > 99) )
	...
	while ($fields_to_print > $o) 
		...
		echo "<tr $bgcolor><td align=right>Ετικέτα τομέων $A_field_rank[$o]: </td><td align=left> $LcolorB<B>$A_field_label[$o]</B>$LcolorE $NWB#vicidial_lists_fields-field_label$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Όνομα τομέων $A_field_rank[$o]: </td><td align=left><textarea name=field_name rows=2 cols=60>$A_field_name[$o]</textarea> $NWB#vicidial_lists_fields-field_name$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Περιγραφή τομέων $A_field_rank[$o]: </td><td align=left><input type=text name=field_description size=70 maxlength=100 value=\"$A_field_description[$o]\"> $NWB#vicidial_lists_fields-field_description$NWE </td></tr>\n";
		echo "<tr $bgcolor><td align=right>Βοήθεια τομέων $A_field_rank[$o]: </td><td align=left><textarea name=field_help rows=2 cols=60>$A_field_help[$o]</textarea> $NWB#vicidial_lists_fields-field_help$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Επιλογές τομέων $A_field_rank[$o]: </td><td align=left><textarea name=field_options ROWS=5 COLS=60>$A_field_options[$o]</textarea>  $NWB#vicidial_lists_fields-field_options$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Μέγεθος τομέων $A_field_rank[$o]: </td><td align=left><input type=text name=field_size size=5 maxlength=3 value=\"$A_field_size[$o]\">  $NWB#vicidial_lists_fields-field_size$NWE </td></tr>\n";
		echo "<tr $bgcolor><td align=right>Τομέας ανώτατος $A_field_rank[$o]: </td><td align=left><input type=text name=field_max size=5 maxlength=3 value=\"$A_field_max[$o]\">  $NWB#vicidial_lists_fields-field_max$NWE </td></tr>\n";
		echo "<tr $bgcolor><td align=right>Προεπιλογή τομέων $A_field_rank[$o]: </td><td align=left><input type=text name=field_default size=50 maxlength=255 value=\"$A_field_default[$o]\">  $NWB#vicidial_lists_fields-field_default$NWE </td></tr>\n";
	...
	echo "<option>5</option>\n";
	echo "</select> &nbsp; $NWB#vicidial_lists_fields-field_order$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Ετικέτα τομέων: </td><td align=left><input type=text name=field_label size=20 maxlength=50> $NWB#vicidial_lists_fields-field_label$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Όνομα τομέων: </td><td align=left><textarea name=field_name rows=2 cols=60></textarea> $NWB#vicidial_lists_fields-field_name$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Θέση ονόματος τομέων: </td><td align=left><select size=1 name=name_position>\n";
	echo "<option value=\"LEFT\">LEFT</option>\n";
	echo "<option value=\"TOP\">TOP</option>\n";
	echo "</select>  $NWB#vicidial_lists_fields-name_position$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Περιγραφή τομέων: </td><td align=left><input name=field_description type=text size=70 maxlength=100> $NWB#vicidial_lists_fields-field_description$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Βοήθεια τομέων: </td><td align=left><textarea name=field_help rows=2 cols=60></textarea> $NWB#vicidial_lists_fields-field_help$NWE </td></tr>\n";
	...
	echo "</select>  $NWB#vicidial_lists_fields-field_type$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Επιλογές τομέων: </td><td align=left><textarea name=field_options ROWS=5 COLS=60></textarea>  $NWB#vicidial_lists_fields-field_options$NWE </td></tr>\n";
	...
	echo "<option value=\"VERTICAL\">VERTICAL</option>\n";
	echo "</select>  $NWB#vicidial_lists_fields-multi_position$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Μέγεθος τομέων: </td><td align=left><input type=text name=field_size size=5 maxlength=3>  $NWB#vicidial_lists_fields-field_size$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Τομέας ανώτατος: </td><td align=left><input type=text name=field_max size=5 maxlength=3>  $NWB#vicidial_lists_fields-field_max$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Προεπιλογή τομέων: </td><td align=left><input type=text name=field_default size=50 maxlength=255 value=\"NULL\">  $NWB#vicidial_lists_fields-field_default$NWE </td></tr>\n";
