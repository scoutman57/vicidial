...
$report_name = 'Called Counts ID Λίσταςs Report';
$db_source = 'M';
...
$HEADER.="function LoadΛίστες(FromBox) {\n";
$HEADER.="	if (!FromBox) {alert(\"NO\"); return false;}\n";
$HEADER.="	var selectedΕκστρατείες=\"|\";\n";
...
$HEADER.="			selectedΕκστρατείες += document.getElementById('group').options[i].value+\"|\";\n";
$HEADER.="		} \n";
$HEADER.="	}\n";
$HEADER.="\n";
$HEADER.="	// Clear List menu\n";
$HEADER.="	document.getElementById('list_ids').options.length=0;\n";
$HEADER.="	var new_list = new Επιλογή();\n";
...
$HEADER.="		var campaign_matches = selectedΕκστρατείες.match(campaignID);\n";
...
$MAIN.="<TABLE CELLSPACING=3><TR><TD VALIGN=TOP><input type='checkbox' name='override_date' value='1' checked>All dates<BR><BR>Ημερομηνίες:<BR>";
...
$MAIN.="Λίστες: <font size=1>(προαιρετική, ενδεχομένως αργή)</font><BR>\n";
$MAIN.=$list_options;
...
if (strlen($QUERY_STRING) > 5)
	...
	$CSV_text="";
	$MAIN.="<center>\n";
	$MAIN.="<br><b>CALLED COUNTS WITHIN LIST(S) $list_id_title_str$date_range_title:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<a href=\"$PHP_SELF?DB=$DB$groupQS$list_idQS&query_date=$query_date&end_date=$end_date&override_date=$override_date&ΕΠΙΒΕΒΑΙΩΣΗ=$ΕΠΙΒΕΒΑΙΩΣΗ&file_download=1\">[ΛΗΨΗ]</a></b><br>\n";
	$CSV_text.="CALLED COUNTS WITHIN LIST(S) $list_id_title_str$date_range_title:\n";
	$MAIN.="<TABLE width=700 cellspacing=1>\n";
	$MAIN.="<tr><td align=left><font size=1>ΚΑΤΑΣΤΑΣΗ</td><td align=center><font size=1>ΟΝΟΜΑ ΚΑΤΑΣΤΑΣΗΣ</td>";
	$CSV_text.="\"STATUS\",\"ΟΝΟΜΑ ΚΑΤΑΣΤΑΣΗΣ\",";
	...
	$MAIN.="<td align=center><font size=1>ΥΠΟΣΥΝΟΛΟ</td></tr>\n";
	$CSV_text.="\"SUBTOTAL\"\n";
	...
	$MAIN.="<tr><td align=center colspan=2><b><font size=1>ΣΥΝΟΛΙΚΑ</td>";
	$CSV_text.="\"\",\"TOTAL\",";
	$first = $all_called_first;
	while ($first <= $all_called_last)
		{
