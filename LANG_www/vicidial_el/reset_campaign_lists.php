...
<title>ADMINISTRATION: Εκστρατεία Κατάλογοι Reset
...
if ( ($LOGuser_level >= 9) and $LOGmodify_campaigns>0 and $LOGmodify_lists>0 and ( (preg_match("/Administration Change Log/",$LOGallowed_reports)) or (preg_match("/ALL ΑΝΑΦΟΡΕΣ/",$LOGallowed_reports)) ) )
	{
	...
	echo "<br><B>Επαναφορά μόλυβδο Ονομάζεται-κατάστασης για τις εκστρατείες:</B><BR><BR>\n";
	...
	echo "<option value='Y'>Ενεργό lists only</option>";
	echo "<option value=''>All lists</option>";
	echo "</select>";
	echo "</td>";
	echo "<td align='right'><input type='submit' name='submit_campaign_reset' value='ΕΠΙΒΕΒΑΙΩΣΗ'></td></tr>";
	...
	if (!preg_match("/_ΔΗΜΙΟΥΡΓΙΑ_/",$SShosted_settings))
