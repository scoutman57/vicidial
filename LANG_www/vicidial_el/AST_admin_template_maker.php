...
if ($submit_template=="ΑΠΟΣΤΟΛΗ ΥΠΟΔΕΙΓΜΑ" && $template_id && $template_name && $template_list_id && $standard_fields_layout) 
	{
...
else if ($delete_template=="Διαγραφή προτύπου" && $template_id) 
	{
...
function checkForm(form_name) {
	...
	if (form_name.template_id.value=="") {error_msg+=" - Πρότυπο ID is missing\n";}
	else if (form_name.template_id.value.length<2) {error_msg+=" - Πρότυπο ID needs to be at least 2 characters long<\n";}
	if (form_name.template_name.value=="") {error_msg+=" - Template name is missing\n";}
	if (form_name.template_list_id.value=="") {error_msg+=" - ID Λίστας is missing\n";}
...
<tr height="20"><th bgcolor="#000099"><font class="standard_bold" color="#FFFFFF">Προσαρμοσμένη Listloader τσάι προτύπου<?php echo "$NWB#vicidial_template_maker$NWE"; ?></font></th></tr>
...
?>
	<tr>
		<th width="50%"><font class="standard_bold">Δημιουργία νέου προτύπου</font></th>
		<th width="50%"><font class="standard_bold">Διαγραφή ενός υπάρχοντος προτύπου</font></th>
	<tr valign="top">
		...
		<form id="listloader_file_primer" action="leadloader_template_display.php?form_action=prime_file" method="post" enctype="multipart/form-data" target="file_holder">
			<font class="standard">Δείγμα αρχείου τοποθέτηση πρότυπο:<BR><font size="-2">()</font></font><?php echo "$NWB#template_maker-create_template$NWE"; ?><BR><BR><input type=file name="sample_template_file" value="<?php echo $sample_template_file; ?>" onChange="loadIFrame('prime_file', this.value); this.form.submit();">
		</form>
		</td>
		<td align="left" width='50%'><form action="<?php echo $PHP_SELF; ?>" method="post"><font class="standard">Επιλέξτε το πρότυπο για να διαγράψετε:</font><?php echo "$NWB#template_maker-delete_template$NWE"; ?><BR><select name="template_id" onChange="loadIFrame('hide_new_template_form', '')">
<?php
...
?>
		</select><BR/><BR/><input type="submit" value="Διαγραφή προτύπου" class="red_btn" name="delete_template">
		</form></td>
	</tr>
	<tr>
		<td align=left colspan="2"><font size=1> &nbsp; &nbsp; &nbsp; &nbsp; <a href="admin.php?ADD=100" target="_parent">ΠΙΣΩ ΣΤΗ ΔΙΑΧΕΙΡΙΣΗ</a> &nbsp; &nbsp; &nbsp; &nbsp; <a href="./admin_listloader_fourth_gen.php">Go to Lead Loader</a> &nbsp; &nbsp; </font></td>
	</tr>
	</tr>
</table>
</form>
...
<table border=0 cellpadding=3 cellspacing=0 width="100%" align="center">
	<tr>
		<th colspan="2" bgcolor="#330099"><font class="standard" color="white">Νέα μορφή προτύπου</font></th>
	</tr>
	<tr bgcolor="#D9E6FE">
		<td align="right" width='25%'><font class="standard">Αναγνωριστικό:</font></td>
	...
	<tr bgcolor="#D9E6FE">
		<td align="right" width='25%'><font class="standard">Όνομα:</font></td>
	...
	<tr bgcolor="#D9E6FE">
		<td align="right" width='25%'><font class="standard">Περιγραφή Πρότυπο:</font></td>
	...
	<tr bgcolor="#D9E6FE">
		<td width='25%' align="right"><font class="standard">Λίστα πρότυπο ID θα φορτώσει σε:</font></td>
		<td width='75%'>
			<select id='template_list_id' name='template_list_id' onChange="ΕμφάνισηTemplateFields(this.value)">
	...
	<tr bgcolor="#D9E6FE">
		<td width='25%' align="right"><font class="standard">Καθεστώτων to dedupe against (optional):</font></td>
		<td width='75%'>
	...
	<tr bgcolor="#D9E6FE">
		<th colspan='2'><input type='submit' name='submit_template' onClick="return checkForm(this.form)" value='ΑΠΟΣΤΟΛΗ ΥΠΟΔΕΙΓΜΑ'></th>
	</tr>
<tr bgcolor="#D9E6FE"><td align="center" colspan=2>
<table border=0 cellpadding=3 cellspacing=1 width="100%" align="center">
	<tr>
		<th colspan="2" bgcolor="#330099"><font class="standard" color="white">Ορισμός στηλών για πεδία αρχείου<BR/><font size='-2'>(Επιλέγοντας μια διαφορετική λίστα θα επαναφέρετε στήλες)</font></font><?php echo "$NWB#template_maker-assign_columns$NWE"; ?></th>
	</tr>
	<tr valign="top">
		<td border="1" align="center" bgcolor="#D9E6FE" width="50%"><font class="standard">Πρότυπο Πεδίο</font></td>
		<td border="0" align="center" bgcolor="#FED9D9" width="50%"><font class="standard">προσαρμοσμένου πεδίου</font></td>
	</tr>
	<tr valign="top">
		<td colspan="2" align="center">
		<span id="field_display" name="field_display"><font class="standard_bold" color='red'>**Επιλέξτε ένα ID της λίστας από το drop down μενού για να δείξει παραπάνω στήλες**</font></span>
		</td>
	</tr>
</table>
</tr></td>
