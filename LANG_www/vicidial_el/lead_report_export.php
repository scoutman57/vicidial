...
$report_name = 'Εξαγωγή οδηγεί Έκθεση';
$db_source = 'M';
$file_exported=0;
...
if ($run_export > 0)
	...
	if ($RUNcampaign > 0)
		...
		if ($outbound_to_print < 1)
			{
			echo "Υπάρχουν no outbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ($RUNgroup > 0)
		...
		if ( ($inbound_to_print < 1) and ($outbound_calls < 1) )
			{
			echo "Υπάρχουν no inbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ( ($outbound_to_print > 0) or ($inbound_to_print > 0) )
		...
		while ($k > $i)
			...
			if ( ($custom_fields_enabled > 0) and ($custom_fields=='YES') )
				...
				$stmt="ΕΜΦΑΝΙΣΗ ΠΙΝΑΚΑΣS LIKE \"custom_$CF_list_id\";";
				if ($DB>0) {echo "$stmt";}
				$rslt=mysql_query($stmt, $link);
		...
		{
		echo "Υπάρχουν no calls during this time period for these parameters\n";
		exit;
		}
	}
##### END RUN THE EXPORT AND OUTPUT FLAT DATA FILE #####
	...
	echo "<CENTER><BR>\n";
	echo "<FONT SIZE=3 FACE=\"Arial,Helvetica\"><B>Εξαγωγή οδηγεί Έκθεση</B></FONT><BR>\n";
	echo "<FONT SIZE=2 FACE=\"Arial,Helvetica\">Η παρούσα έκθεση τραβά μολύβδου πληροφορίες για κλήσεις στο επιλεγμένο εύρος ημερομηνιών. Ο μόλυβδος εξάγεται μόνο μία φορά δεν έχει σημασία πόσες κλήσεις αντιμετωπίστηκαν. Η τρέχουσα κατάσταση μόλυβδος χρησιμοποιείται.</FONT><BR><BR>\n";
	...
	if ($custom_fields_enabled > 0)
		...
		echo "<B>προσαρμοσμένου πεδίουs:</B><BR>\n";
