		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST BEL TIME",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
					if (eregi("DOWN LAST BEL TIME",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if (!$CBleadIDset)
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "Telefoon and Post Code Time Zone Mismatch! ";
...
if ($ACTION == 'updateDISPO')
	...
	### BEGIN Call Opmerkingen Logging ###
...
if ($ACTION == 'AGENTSview')
	...
	while ($agents_count > $loop_count)
		...
		if ($comments=='AgentXferViewSelect') 
			{
			$AXVSuserSORTEER[$j] =	"$full_name$US$j";
			$AXVSuser[$j] =			$user;
	...
	if ($comments=='AgentXferViewSelect') 
		...
		sort($AXVSuserSORTEER);
		while ($j > $k)
			{
			$order_split = explode("_",$AXVSuserSORTEER[$k]);
			$i = $order_split[1];
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		if ($counts_to_print > 0) 
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; PHONE &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; STATUS &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; LAST BEL &nbsp; </font></TD>";
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Gebeld Since Laatste Reset: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Gebeld Count: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Laatste Local Call Time: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
