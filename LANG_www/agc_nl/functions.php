...
function user_authorization($user,$pass,$user_option,$user_update,$bcrypt,$return_hash)
	...
	$LOCK_over = ($STARTtime - 900); # failed login lockout time is 15 minutes(900 seconden)
...
function custom_list_fields_values($lead_id,$list_id,$uniqueid,$user)
	...
	if ($tablecount_to_print > 0) 
		...
		if ($fieldscount_to_print > 0) 
			...
			$CFoutput .= "<input type=hidden name=stage id=stage value=\"OPSLAAN\">\n";
	...
	if (preg_match("/--A--/",$CFoutput))
		...
		if (preg_match('/--A--TABLEper_call_notes--B--/i',$CFoutput))
			...
			if ($hide_call_log_info!='Y')
				{
				if ($search != 'logfirst')
					{$NOTESout .= "Gesprekslog voor deze klant:<br>\n";}
...
function lookup_gmt($phone_code,$USarea,$state,$LOCAL_GMT_OFF_STD,$Shour,$Smin,$Ssec,$Smon,$Smday,$Syear,$postalgmt,$postal_code)
	...
	if ( (!$AC_processed) and ($dst_range == 'SSM-FSN') )
		{
		if ($DBX) {print "     Second Sunday Maart to Eerste Sunday November\n";}
		#**********************************************************************
		# SSM-FSN
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op Second Sunday Maart to Eerste Sunday November at 2 am.
		#     INPUTS:
		#       mm              INTEGER       Month.
		...
		#                                      (DEFAULT is blank)
		#                                     make calculations based op UTC time, 
	...
	if ( (!$AC_processed) and ($dst_range == 'FSA-LSO') )
		{
		if ($DBX) {print "     Eerste Sunday April to Laatste Sunday Oktober\n";}
		#**********************************************************************
		# FSA-LSO
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op first Sunday in April and last Sunday in Oktober at 2 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSM-LSO') )
		{
		if ($DBX) {print "     Laatste Sunday Maart to Laatste Sunday Oktober\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op last Sunday in Maart and last Sunday in Oktober at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSO-LSM') )
		{
		if ($DBX) {print "     Laatste Sunday Oktober to Laatste Sunday Maart\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op last Sunday in Oktober and last Sunday in Maart at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-LSM') )
		{
		if ($DBX) {print "     Eerste Sunday Oktober to Laatste Sunday Maart\n";}
		#**********************************************************************
		#   TASMANIA ONLY
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op first Sunday in Oktober and last Sunday in Maart at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-FSA') )
		{
		if ($DBX) {print "     Sunday in Oktober to Eerste Sunday in April\n";}
		#**********************************************************************
		# FSO-FSA
		#   2008+ AUSTRALIA ONLY (country code 61)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op first Sunday in Oktober and first Sunday in April at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'FSO-TSM') )
		{
		if ($DBX) {print "     Eerste Sunday Oktober to Third Sunday Maart\n";}
		#**********************************************************************
		#     This is s 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op first Sunday in Oktober and third Sunday in Maart at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'LSS-FSA') )
		{
		if ($DBX) {print "     Laatste Sunday in September to Eerste Sunday in April\n";}
		#**********************************************************************
		# LSS-FSA
		#   2007+ NIEUW ZEALAND (country code 64)
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect.
		#     Based op last Sunday in September and first Sunday in April at 1 am.
		#**********************************************************************
	...
	if ( (!$AC_processed) and ($dst_range == 'TSO-LSF') )
		{
		if ($DBX) {print "     Third Sunday Oktober to Laatste Sunday February\n";}
		#**********************************************************************
		# TSO-LSF
		#     This is returns 1 if Daylight Savings Time is in effect and 0 if 
		#       Standard time is in effect. Brazil
		#     Based op Third Sunday Oktober to Laatste Sunday February at 1 am.
		#**********************************************************************
