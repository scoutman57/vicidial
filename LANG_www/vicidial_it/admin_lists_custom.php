...
if ($action == "AIUTO")
	...
	<BR>
	<B>ViciDial Elenca Campo personalizzatos Help</B>
	<BR><BR>

	<A NAME="vicidial_lists_fields-field_label">
	<BR>
	 <B>Campo Etichetta -</B> Questo è l'identificatore di campo del database per questo campo. Questo deve essere un identificatore univoco all'interno dei campi personalizzati per questa lista. Non utilizzare spazi o segni di punteggiatura per questo campo. max 50 caratteri, un minimo di 2 caratteri. È inoltre possibile includere i campi predefiniti ViciDial in un campo personalizzato di configurazione, e li vedrete in rosso nella lista. Questi campi non verranno aggiunti alla tabella di database personalizzato lista, l'interfaccia agente invece riferimento alla tabella vicidial_list direttamente. Le etichette che è possibile utilizzare per includere le fieds di default sono - 
...
if ($SScustom_fields_enabled < 1)
	{
	echo "ERRORE: I campi personalizzati non sono attivi su questo sistema\n";
	exit;
	}
...
if ( ($action == "COPY_FIELDS_INVIA") and ($list_id > 99) and ($source_list_id > 99) and (strlen($copy_option) > 2) )
	...
	if ($list_id=="$source_list_id")
		{echo "ERRORE: Non è possibile copiare i campi alla stessa lista: $list_id|$source_list_id";}
	else
		{
		$table_exists=0;
		...
		if ($source_field_exists < 1)
			{echo "ERROR: nell'elenco Sorgente non ha campi personalizzati\n<BR>";}
		else
			{
			##### REPLACE option #####
			...
			if ($copy_option=='UPDATE')
				...
				if ($table_exists < 1)
					{echo "ERRORE: La tabella non esiste custom_$list_id\n<BR>";}
				else
					{
					$stmt="SELECT field_id,field_label,field_name,field_description,field_rank,field_help,field_type,field_options,field_size,field_max,field_default,field_cost,field_required,multi_position,name_position,field_order from vicidial_lists_fields where list_id='$source_list_id' order by field_rank,field_order,field_label;";
					$rslt=mysql_query($stmt, $link);
...
if ( ($action == "DELETE_CUSTOM_FIELD_CONFIRMATION") and ($list_id > 99) and ($field_id > 0) and (strlen($field_label) > 0) )
	...
	if ($field_exists < 1)
		{echo "ERRORE: Il campo non esiste\n<BR>";}
	else
		{
		if ($table_exists < 1)
			{echo "ERRORE: La tabella non esiste custom_$list_id\n<BR>";}
...
if ( ($action == "ADD_CUSTOM_FIELD") and ($list_id > 99) )
	...
	if ( (strlen($field_label)<1) or (strlen($field_name)<2) or (strlen($field_size)<1) )
		...
		if (preg_match("/\|$field_label\|/i",$mysql_reserved_words))
			{echo "ERRORE: Non è possibile utilizzare le parole riservate per le etichette dei campi - $list_id|$field_label|$field_name|$field_size\n<BR>";}
		else
			{
			...
			if ( ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') ) and ( (!preg_match("/,/",$field_options)) or (!preg_match("/\n/",$field_options)) or (strlen($field_options)<6) or ($TEST_valid_options < 1) ) )
				{echo "ERROR: Devi inserire opzioni di campo quando si aggiunge un SELECT, MULTI, RADIO or CHECKBOX field type  - $list_id|$field_label|$field_type|$field_options\n<BR>";}
			else
				{
				if ($field_exists > 0)
					{echo "ERRORE: Il campo esiste già per questa lista - $list_id|$field_label\n<BR>";}
				else
					{
					$table_exists=0;
					$linkCUSTOM=mysql_connect("$VARDB_server:$VARDB_port", "$VARDB_custom_user","$VARDB_custom_pass");
...
if ( ($action == "MODIFY_CUSTOM_FIELD_INVIA") and ($list_id > 99) and ($field_id > 0) )
	...
	if ($field_exists < 1)
		...
		{
		if ($table_exists < 1)
			{echo "ERRORE: La tabella non esiste\n<BR>";}
		else
			{
			$TEST_valid_options=0;
			if ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') )
			...
			if ( ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') ) and ( (!preg_match("/,/",$field_options)) or (!preg_match("/\n/",$field_options)) or (strlen($field_options)<6) or ($TEST_valid_options < 1) ) )
				{echo "ERROR: Devi inserire opzioni di campo quando si aggiorna un SELECT, MULTI, RADIO or CHECKBOX field type  - $list_id|$field_label|$field_type|$field_options\n<BR>";}
			else
				{
				### modify field function
...
if ( ($action == "MODIFY_CUSTOM_FIELDS") and ($list_id > 99) )
	...
	while ($fields_to_print > $o) 
		...
		echo "<tr $bgcolor><td align=right>Campo Etichetta $A_field_rank[$o]: </td><td align=left> $LcolorB<B>$A_field_label[$o]</B>$LcolorE $NWB#vicidial_lists_fields-field_label$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Nome del campo $A_field_rank[$o]: </td><td align=left><textarea name=field_name rows=2 cols=60>$A_field_name[$o]</textarea> $NWB#vicidial_lists_fields-field_name$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Campo Descrizione $A_field_rank[$o]: </td><td align=left><input type=text name=field_description size=70 maxlength=100 value=\"$A_field_description[$o]\"> $NWB#vicidial_lists_fields-field_description$NWE </td></tr>\n";
		echo "<tr $bgcolor><td align=right>Guida campi $A_field_rank[$o]: </td><td align=left><textarea name=field_help rows=2 cols=60>$A_field_help[$o]</textarea> $NWB#vicidial_lists_fields-field_help$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Opzioni dei campi $A_field_rank[$o]: </td><td align=left><textarea name=field_options ROWS=5 COLS=60>$A_field_options[$o]</textarea>  $NWB#vicidial_lists_fields-field_options$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Dimensione campo $A_field_rank[$o]: </td><td align=left><input type=text name=field_size size=5 maxlength=3 value=\"$A_field_size[$o]\">  $NWB#vicidial_lists_fields-field_size$NWE </td></tr>\n";
		...
		echo "<tr $bgcolor><td align=right>Campo di default $A_field_rank[$o]: </td><td align=left><input type=text name=field_default size=50 maxlength=255 value=\"$A_field_default[$o]\">  $NWB#vicidial_lists_fields-field_default$NWE </td></tr>\n";
	...
	echo "<option>5</option>\n";
	echo "</select> &nbsp; $NWB#vicidial_lists_fields-field_order$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Campo Etichetta: </td><td align=left><input type=text name=field_label size=20 maxlength=50> $NWB#vicidial_lists_fields-field_label$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Nome del campo: </td><td align=left><textarea name=field_name rows=2 cols=60></textarea> $NWB#vicidial_lists_fields-field_name$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Nome del campo Posizione: </td><td align=left><select size=1 name=name_position>\n";
	echo "<option value=\"LEFT\">LEFT</option>\n";
	echo "<option value=\"TOP\">TOP</option>\n";
	echo "</select>  $NWB#vicidial_lists_fields-name_position$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Campo Descrizione: </td><td align=left><input name=field_description type=text size=70 maxlength=100> $NWB#vicidial_lists_fields-field_description$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Guida campi: </td><td align=left><textarea name=field_help rows=2 cols=60></textarea> $NWB#vicidial_lists_fields-field_help$NWE </td></tr>\n";
	...
	echo "</select>  $NWB#vicidial_lists_fields-field_type$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Opzioni dei campi: </td><td align=left><textarea name=field_options ROWS=5 COLS=60></textarea>  $NWB#vicidial_lists_fields-field_options$NWE </td></tr>\n";
	...
	echo "<option value=\"VERTICAL\">VERTICAL</option>\n";
	echo "</select>  $NWB#vicidial_lists_fields-multi_position$NWE </td></tr>\n";
	echo "<tr $bgcolor><td align=right>Dimensione campo: </td><td align=left><input type=text name=field_size size=5 maxlength=3>  $NWB#vicidial_lists_fields-field_size$NWE </td></tr>\n";
	...
	echo "<tr $bgcolor><td align=right>Campo di default: </td><td align=left><input type=text name=field_default size=50 maxlength=255 value=\"NULL\">  $NWB#vicidial_lists_fields-field_default$NWE </td></tr>\n";
