...
$campaign_span_txt="Campagnas:<BR>";
$campaign_span_txt.="<SELECT SIZE=5 NAME=group[] multiple>";
if  (preg_match('/\-\-ALL\-\-/',$group_string))
	{$campaign_span_txt.="<option value=\"--ALL--\" selected>-- ALL CAMPAGNE --</option>";}
else
	{$campaign_span_txt.="<option value=\"--ALL--\">-- ALL CAMPAGNE --</option>";}
$o=0;
	...
	if ($snapshot_time && $report_source=="SNAPSHOT") 
		...
		if ($group_ct>0 && $report_type=="CAMPAIGNS") 
			...
			for ($i=0; $i<$group_ct; $i++) 
				...
				if ($DB) {$HTML_header.= "         CAMPAIGN DIAL esito: |$active_dial_statuses|\n";}
			...
			$inventory_statuses=substr($inventory_statuses, 0, -1);
			if ($DB) {$HTML_header.= "         CAMPAIGN DIAL esito: |$active_dial_statuses|\n";}
			if ($DB) {$HTML_header.= "                  DIAL esito: |$dial_statuses|\n";}
			if ($DB) {$HTML_header.= "     INVENTORY STATUSES RESULTS: |$inventory_statuses|\n";}
			if ($DB) {$HTML_header.= "      INACTIVE STATUSES RESULTS: |$inactive_dial_statuses|\n";}
...
#					$oneoff_stmt="SELECT count(*) from (SELECT lead_id, count(*) as count from vicidial_log where lead_id in (SELECT lead_id from vicidial_list where $full_dialable_SQL and gmt_offset_now='$gmt_row[0]' $filter_SQL) $shift_days_SQL group by lead_id) as count_table where count_table.count>='($call_count_target-1)'";
	...
	$rpt_header.="| Call list | List description               | Campagna | Last call date      | Start Inv | Call Inv | Call Inv | Call Inv | Call Inv | Dial Avg | Pen. %  |$rpt_header_SHIFTS\n";
	$rpt_header.="|           |                                |          |                     |           |  Totale  | No filtr | One-off  | Inattivo|          |         |$rpt_header_SHIFTS_lower\n";
	$rpt_header.="+-----------+--------------------------------+----------+---------------------+-----------+----------+----------+----------+----------+----------+---------+$rpt_header_Border\n";

	$CSV_header="\"$report_name\"\n";
	$CSV_header.="\"Date: ".date("m/d/Y")."\",\"\",\"Time: ".date("H:i a")."\"\n\n";
	$CSV_header.="\"Call list\",\"List description\",\"Campagna\",\"Last call date\",\"Start Inv\",\"Call Inv Total\",\"Call Inv - No filter\",\"Call Inv - One-offs\",\"Call Inv - Inattivodialable statuses\",\"Dial Avg\",\"Pen. %\",".substr($CSV_header_SHIFTS,0,-1)."\n";
	...
	$rpt_footer.="|".sprintf("%76s", "TOTALI")." | ".sprintf("%9s", $total_list_start_inv)." | ".sprintf("%8s", $total_dialable_count)." | ".sprintf("%8s", $total_dialable_count_nofilter)." | ".sprintf("%8s", $total_dialable_count_oneoff)." | ".sprintf("%8s", $total_dialable_count_inactive)." | ".sprintf("%8s", $total_average_call_count)." | ".sprintf("%6s", $total_penetration)."% |";
	$CSV_body.="\"\",\"\",\"\",\"TOTALI\",\"$total_list_start_inv\",\"$total_dialable_count\",\"$total_dialable_count_nofilter\",\"$total_dialable_count_oneoff\",\"$total_dialable_count_inactive\",\"$total_average_call_count\",\"$total_penetration\"";
	$b=0;
	while ($b < count($shift_ary))
		{
...
$HTML_text.="	<tr><td><input type='radio' name='report_type' value='CAMPAIGNS' onClick=\"ToggleSpan('campaign_span', 'list_span')\" $cmp_checked>Campagnas</td></tr>\n";
