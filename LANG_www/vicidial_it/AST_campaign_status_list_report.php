...
$report_name="Campagna Status List Report";
$NOW_DATE = date("Y-m-d");
...
$HTML_text.="<a href=\"$PHP_SELF?DB=$DB&query_date=$query_date&end_date=$end_date&query_date_D=$query_date_D&query_date_T=$query_date_T&end_date_D=$end_date_D&end_date_T=$end_date_T$groupQS&file_download=1&INVIA=$INVIA\">DOWNLOAD</a> |";
...
while($i < $group_ct)
	...
	$ASCII_text.="<B>CAMPAGNA: $group[$i]</B>\n";
	$GRAPH.="<B>CAMPAGNA: $group[$i]</B>\n";
	$CSV_text.="\"CAMPAGNA: $group[$i]\"\n";
	...
	while ($row=mysqli_fetch_row($rslt)) 
		...
		$dispo_ary="";
		$ASCII_text.="<FONT SIZE=2><B>ID Lista #$list_id: $list_name</B>\n";
		$GRAPH.="<FONT SIZE=2><B>ID Lista #$list_id: $list_name</B>\n";
		$CSV_text.="\"ID Lista #$list_id: $list_name\"\n";
		...
		if (mysqli_num_rows($stat_rslt)>0) 
			...
			$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>ID Lista #$list_id: $list_name</caption><tr><th class='thgraph' scope='col'>DISPOSITION</th>";
			...
			$ASCII_text.="+------------------------------------------+--------+------------+-------------+\n";
			$CSV_text.="\"TOTALI:\",\"$total_calls\",\"".sec_convert($total_duration, 'H')."\",\"".sec_convert($total_handle_time, 'H')."\"\n\n";
			...
			$ASCII_text .= "|    TOTALE CHIAMATE: $header_list_count                                   |\n";
			$ASCII_text .= "+--------------------------------------------------------------+\n";
			$ASCII_text .= "| STATUS FLAGS BREAKDOWN:  (and % of total leads in the list)  |\n";
			$ASCII_text .= "|   Risposta Umana:       $HA_count    $HA_percent%                   |\n";
			...
			for ($d=0; $d<count($graph_stats); $d++) 
				...
				$DURATION_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$d][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$graph_stats[$d][2], $max_duration))."' height='16' />".sec_convert($graph_stats[$d][2], 'H')."</td></tr>";
				$HANDLETIME_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$d][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$graph_stats[$d][3], $max_handletime))."' height='16' />".sec_convert($graph_stats[$d][3], 'H')."</td></tr>";
				}
