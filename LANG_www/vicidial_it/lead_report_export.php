...
$report_name = 'Export Leads Rapporto';
$db_source = 'M';
...
if ($run_export > 0)
	...
	if ($RUNcampaign > 0)
		...
		if ($outbound_to_print < 1)
			{
			echo "Esistono no outbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ($RUNgroup > 0)
		...
		if ( ($inbound_to_print < 1) and ($outbound_calls < 1) )
			{
			echo "Esistono no inbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ( ($outbound_to_print > 0) or ($inbound_to_print > 0) )
		...
		while ($k > $i)
			...
			if ( ($custom_fields_enabled > 0) and ($custom_fields=='YES') )
				...
				$stmt="MOSTRA TABELLAS LIKE \"custom_$CF_list_id\";";
				if ($DB>0) {echo "$stmt";}
				$rslt=mysql_query($stmt, $link);
		...
		{
		echo "Esistono no calls during this time period for these parameters\n";
		exit;
		}
	}
##### END RUN THE EXPORT AND OUTPUT FLAT DATA FILE #####
	...
	echo "<CENTER><BR>\n";
	echo "<FONT SIZE=3 FACE=\"Arial,Helvetica\"><B>Export Leads Rapporto</B></FONT><BR>\n";
	echo "<FONT SIZE=2 FACE=\"Arial,Helvetica\">Questo rapporto raccoglie le informazioni di piombo per le chiamate effettuate nell'intervallo di date selezionato. Un cavo è esportato solo una volta, non importa quante chiamate sono state gestite. Lo stato attuale piombo viene utilizzato.</FONT><BR><BR>\n";
	...
	if ($custom_fields_enabled > 0)
		...
		echo "<B>campo personalizzatos:</B><BR>\n";
