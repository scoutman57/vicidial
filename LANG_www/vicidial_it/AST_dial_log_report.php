...
$report_name='Comporre Log Report';
...
if ($INVIA && $query_date) {
		...
		$MAIN.="--- DIAL LOG RECORDS FOR $query_date, $query_date_D TO $query_date_T $server_rpt_string, $HC_rpt_string\n --- RECORDS #$lower_limit-$upper_limit               <a href=\"$PHP_SELF?INVIA=$INVIA&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$sip_hangup_causeQS&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">[DOWNLOAD]</a>\n";
		...
		$ll=$lower_limit-1000;
		if ($ll>=1) {
			$dial_log_rpt_hf.="<a href=\"$PHP_SELF?INVIA=$INVIA&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$sip_hangup_causeQS&lower_limit=$ll\">[<<< PREV 1000 records]</a>";
		} else {
		...
		if (($lower_limit+1000)<mysqli_num_rows($rslt)) {
			...
			$dial_log_rpt_hf.="<a href=\"$PHP_SELF?INVIA=$INVIA&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$sip_hangup_causeQS&lower_limit=".($lower_limit+1000)."\">[NEXT $max_limit records >>>]</a>";
		} else {
			$dial_log_rpt_hf.=sprintf("%23s", " ");
		}
		$dial_log_rpt_hf.="\n";
