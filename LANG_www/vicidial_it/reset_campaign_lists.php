...
<title>ADMINISTRATION: Campagna Liste Reset
...
if ( ($LOGuser_level >= 9) and $LOGmodify_campaigns>0 and $LOGmodify_lists>0 and ( (preg_match("/Administration Change Log/",$LOGallowed_reports)) or (preg_match("/ALL REPORT/",$LOGallowed_reports)) ) )
	{
	...
	echo "<br><B>Ripristina-Lead Chiamato-Status per campagne:</B><BR><BR>\n";
	...
	echo "<option value='Y'>Attivo lists only</option>";
	echo "<option value=''>All lists</option>";
	echo "</select>";
	echo "</td>";
	echo "<td align='right'><input type='submit' name='submit_campaign_reset' value='INVIA'></td></tr>";
