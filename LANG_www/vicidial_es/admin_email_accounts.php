...
if( (strlen($PHP_AUTH_USER)<2) or (strlen($PHP_AUTH_PW)<2) or (!$auth))
	...
	if ($auth>0)
		...
		if ($admin_viewable_groupsALL > 0)
			{$UUgroups_list .= "<option value=\"---ALL---\">Todos los grupos de usuarios de administrador</option>\n";}
...
<title>ADMINISTRATION: Marcador campos cuenta de correo electrónico
<?php 
		...
		if ($ENVIAR=="ENVIAR") 
			...
			if ($add_copy_disabled > 0)
				...
				if (mysql_affected_rows($link)==0) 
					...
					{
					$message="CUENTA NUEVA $email_account_id CREADO CON ÉXITO";
					$eact="";
	...
	if (mysql_num_rows($rslt)>0) 
		...
		if ($add_copy_disabled > 0)
			...
			if (mysql_affected_rows($link)==0) 
				...
				{
				$message="CUENTA NUEVA $new_account_id ÉXITO DE COPIADO $source_email_account<BR/>";
				$eact="";
...
if ($eact == "COPY")
	...
	if ($accounts_to_print>0) 
		...
		if ($error_msg) {echo "CUENTA NO COPIADO<BR/>Los datos que ha enviado tiene los siguientes errores:<BR/>$error_msg";}
		echo "<br>Copie nueva cuenta desde una cuenta existente<form action='$PHP_SELF' method='GET'>\n";
		echo "<input type=hidden name=DB value=\"$DB\">\n";
		echo "<input type=hidden name=action value=COPY_EMAIL_ENVIAR>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>ID de la cuenta de copia del: </td><td align=left><select size=1 name=source_email_account>\n";
		echo "$accounts_list";
		echo "</select></td></tr>\n";
		...
		echo "<TABLE><TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		echo "<br><br>*** No hay cuentas existentes para copiar desde ***\n";
		echo "</TD></TR></TABLE>\n";
		}
	}
### END copy fields to a list form
	...
	if ( ($LOGemails_modify==1) )
		...
		echo "<TR><TD>\n";
		echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
		if ($error_msg) {echo "CUENTA NO INSERTADA<BR/>Los datos que ha enviado tiene los siguientes errores:<BR/>$error_msg";}
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Correo electrónico Nombre de cuenta: </td><td align=left><input type=text name=email_account_name size=40 maxlength=100 value='$email_account_name'>$NWB#vicidial_email_accounts-email_account_name$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Activo: </td><td align=left><select size=1 name=active><option>Y</option><option SELECTED>N</option></select>$NWB#vicidial_email_accounts-email_account_active$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Descripción de la cuenta: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#vicidial_email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Tipo de cuenta: </td><td align=left><select size=1 name=email_account_type><option>INBOUND</option></select>$NWB#vicidial_email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Grupo de usuarios de administración: </td><td align=left><select size=1 name=user_group>\n";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Todos los grupos de usuarios de administrador</option>\n";
		echo "</select>$NWB#vicidial_email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Protocolo de cuenta de correo electrónico: </td><td align=left><select size=1 name=protocol><option SELECTED>IMAP</option><option>POP3</option></select>$NWB#vicidial_email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail Dirección de respuesta: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#vicidial_email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Cuenta de correo electrónico del servidor: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#vicidial_email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Correo electrónico de cuentas de usuario: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#vicidial_email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Contraseña: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#vicidial_email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">Dentro de la ID de grupo</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
		echo "</select>$NWB#vicidial_email_accounts-in_group$NWE</td></tr>\n";

		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=1000\">Por defecto Lista ID</a>: </td><td align=left><input type=text name=default_list_id size=20 maxlength=255 value='$default_list_id'>$NWB#vicidial_email_accounts-default_list_id$NWE</td></tr>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>En Grupo Agente Método de búsqueda:</td><td align=left><select size=1 name=agent_search_method><option value=\"LB\">LB - Load Balanced</option><option value=\"LO\">LO - Load Balanced Overflow</option><option value=\"SO\">SO - Server Only</option><option SELECTED>$agent_search_method</option></select>$NWB#vicidial_email_accounts-agent_search_method$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>-En la lista Grupo de Identificación:</td><td align=left><input type=text name=list_id size=14 maxlength=14 value=\"$list_id\">$NWB#vicidial_email_accounts-ingroup_list_id$NWE</td></tr>\n";
	...
	if ( ($LOGemails_modify==1) )
		...
		if ($eact=="DELETE" && $email_account_id)  
			{
			echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
			echo "<br><B>CONFIRMAR LA SUPRESIÓN DE ENTRADA CUENTA DE CORREO ELECTRÓNICO $email_account_id</B><BR>\n";
			echo "<a href='$PHP_SELF?eact=DELETE&email_account_id=$email_account_id&confirm_deletion=yes'>Haga clic para borrar la cuenta $email_account_id</a>\n";
			echo "</font>";
			}
		$stmt="SELECT campaign_id,campaign_name from vicidial_campaigns $whereLOGallowed_campaignsSQL order by campaign_id;";
		$rslt=mysql_query($stmt, $link);
		...
		if ($error_msg) {echo "CUENTA NO ACTUALIZADO<BR/>Los datos que ha enviado tiene los siguientes errores:<BR/>$error_msg";}
		echo "<br>Actualizar un correo electrónico existente CUENTA DE ENTRADA<form action='$PHP_SELF' method='GET'>\n";
		echo "<center><TABLE width=$section_width cellspacing=3>\n";
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Descripción de la cuenta: </td><td align=left><input type=text name=email_account_description size=70 maxlength=255 value='$email_account_description'>$NWB#vicidial_email_accounts-email_account_description$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Tipo de cuenta: </td><td align=left><select size=1 name=email_account_type><option value='$email_account_type' selected>$email_account_type</option><option>INBOUND</option></select>$NWB#vicidial_email_accounts-email_account_type$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Grupo de usuarios de administración: </td><td align=left><select size=1 name=user_group>\n";
		echo "<option value='$user_group' selected>$user_group</option>";
		echo "$UUgroups_list";
		echo "<option SELECTED value=\"---ALL---\">Todos los grupos de usuarios de administrador</option>\n";
		echo "</select>$NWB#vicidial_email_accounts-admin_user_group$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Protocolo de cuenta de correo electrónico: </td><td align=left><select size=1 name=protocol><option>IMAP</option><option>POP3</option><option SELECTED>$protocol</option></select>$NWB#vicidial_email_accounts-protocol$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>E-mail Dirección de respuesta: </td><td align=left><input type=text name=email_replyto_address size=70 maxlength=255 value='$email_replyto_address'>$NWB#vicidial_email_accounts-email_replyto_address$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Cuenta de correo electrónico del servidor: </td><td align=left><input type=text name=email_account_server size=70 maxlength=255 value='$email_account_server'>$NWB#vicidial_email_accounts-email_account_server$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Correo electrónico de cuentas de usuario: </td><td align=left><input type=text name=email_account_user size=30 maxlength=255 value='$email_account_user'>$NWB#vicidial_email_accounts-email_account_user$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right>Email Account Contraseña: </td><td align=left><input type=text name=email_account_pass size=30 maxlength=100 value='$email_account_pass'>$NWB#vicidial_email_accounts-email_account_pass$NWE</td></tr>\n";
		...
		echo "</select>$NWB#vicidial_email_accounts-email_frequency_check_mins$NWE</td></tr>\n";
		echo "<tr bgcolor=#B6D3FC><td align=right><a href=\"admin.php?ADD=3811&group_id=$group_id\">Dentro de la ID de grupo</a>: </td><td align=left><select size=1 name=group_id>";
		echo "$Dgroups_menu";
...
###############
		...
		echo "<tr bgcolor=#B6D3FC><td align=right>Emails Un-manejados: </td><td align=left><B>$unhandled_emails</B></td></tr>\n";
################
		...
		if ($LOGuser_level >= 9)
			{
			echo "<br><br><a href=\"admin.php?ADD=720000000000000&category=EMAIL&stage=$email_account_id\">Haga clic aquí para ver los cambios de administrador para este disco</FONT>\n";
			}
	...
	echo "<FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2>";
	if ($message) {echo "<B>$message</B><BR>";}
	echo "<br>ENTRADA cuenta de correo electrónico LISTADOS:\n";
