...
$HTML_text.="<a href=\"$PHP_SELF?DB=$DB&query_date=$query_date&end_date=$end_date&query_date_D=$query_date_D&query_date_T=$query_date_T&end_date_D=$end_date_D&end_date_T=$end_date_T$groupQS&file_download=1&ENVIAR=$ENVIAR\">DESCARGAR</a> |";
...
while($i < $group_ct)
	...
	while ($row=mysql_fetch_row($rslt)) 
		...
		$dispo_ary="";
		$ASCII_text.="<FONT SIZE=2><B>ID De la Lista #$list_id: $list_name</B>\n";
		$GRAPH.="<FONT SIZE=2><B>ID De la Lista #$list_id: $list_name</B>\n";
		$CSV_text.="\"ID De la Lista #$list_id: $list_name\"\n";
		...
		if (mysql_num_rows($stat_rslt)>0) 
			...
			$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>ID De la Lista #$list_id: $list_name</caption><tr><th class='thgraph' scope='col'>DISPOSITION</th>";
