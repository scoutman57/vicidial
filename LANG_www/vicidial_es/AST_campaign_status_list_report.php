...
$report_name="Campaña Status List Report";
$NOW_DATE = date("Y-m-d");
...
$HTML_text.="<a href=\"$PHP_SELF?DB=$DB&query_date=$query_date&end_date=$end_date&query_date_D=$query_date_D&query_date_T=$query_date_T&end_date_D=$end_date_D&end_date_T=$end_date_T$groupQS&file_download=1&REMITIR=$REMITIR\">DESCARGAR</a> |";
...
while($i < $group_ct)
	...
	$ASCII_text.="<B>CAMPAÑA: $group[$i]</B>\n";
	$GRAPH.="<B>CAMPAÑA: $group[$i]</B>\n";
	$CSV_text.="\"CAMPAÑA: $group[$i]\"\n";
	...
	while ($row=mysqli_fetch_row($rslt)) 
		...
		$dispo_ary="";
		$ASCII_text.="<FONT SIZE=2><B>ID de Lista #$list_id: $list_name</B>\n";
		$GRAPH.="<FONT SIZE=2><B>ID de Lista #$list_id: $list_name</B>\n";
		$CSV_text.="\"ID de Lista #$list_id: $list_name\"\n";
		...
		if (mysqli_num_rows($stat_rslt)>0) 
			...
			$graph_header="<table cellspacing='0' cellpadding='0' class='horizontalgraph'><caption align='top'>ID de Lista #$list_id: $list_name</caption><tr><th class='thgraph' scope='col'>DISPOSITION</th>";
			...
			$ASCII_text .= "|    LLAMADAS TOTALES: $header_list_count                                   |\n";
			$ASCII_text .= "+--------------------------------------------------------------+\n";
			$ASCII_text .= "| STATUS FLAGS BREAKDOWN:  (and % of total leads in the list)  |\n";
			$ASCII_text .= "|   Respuesta Humana:       $HA_count    $HA_percent%                   |\n";
