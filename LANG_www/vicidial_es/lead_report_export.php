...
$report_name = 'Informe de las exportaciones genera';
$db_source = 'M';
...
if ($run_export > 0)
	...
	if ($RUNcampaign > 0)
		...
		if ($outbound_to_print < 1)
			{
			echo "Hay no outbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ($RUNgroup > 0)
		...
		if ( ($inbound_to_print < 1) and ($outbound_calls < 1) )
			{
			echo "Hay no inbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ( ($outbound_to_print > 0) or ($inbound_to_print > 0) )
		...
		while ($k > $i)
			...
			if ( ($custom_fields_enabled > 0) and ($custom_fields=='YES') )
				...
				$stmt="DEMOSTRACIÓN TABLAS LIKE \"custom_$CF_list_id\";";
				if ($DB>0) {echo "$stmt";}
				$rslt=mysql_query($stmt, $link);
		...
		{
		echo "Hay no calls during this time period for these parameters\n";
		exit;
		}
	}
##### END RUN THE EXPORT AND OUTPUT FLAT DATA FILE #####
	...
	echo "<CENTER><BR>\n";
	echo "<FONT SIZE=3 FACE=\"Arial,Helvetica\"><B>Informe de las exportaciones genera</B></FONT><BR>\n";
	echo "<FONT SIZE=2 FACE=\"Arial,Helvetica\">Este informe recoge la información contenida de plomo para las llamadas realizadas en el intervalo de fechas seleccionado. Una ventaja sólo se exporta una vez sin importar cuántas llamadas han sido manipulados. El estado actual de plomo se utiliza.</FONT><BR><BR>\n";
