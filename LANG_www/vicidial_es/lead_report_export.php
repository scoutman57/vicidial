...
$report_name = 'Exportar Informe de Contactos';
$db_source = 'M';
$file_exported=0;
	...
	if ( ($outbound_to_print > 0) or ($inbound_to_print > 0) )
		...
		while ($k > $i)
			...
			if ( ($custom_fields_enabled > 0) and ($custom_fields=='YES') )
				...
				$stmt="MOSTRAR TABLAS LIKE \"custom_$CF_list_id\";";
				if ($DB>0) {echo "$stmt";}
				$rslt=mysql_to_mysqli($stmt, $link);
...
##### END RUN THE EXPORT AND OUTPUT FLAT DATA FILE #####
	...
	echo "<CENTER><BR>\n";
	echo "<FONT SIZE=3 FACE=\"Arial,Helvetica\"><B>Exportar Informe de Contactos</B></FONT><BR>\n";
	echo "<FONT SIZE=2 FACE=\"Arial,Helvetica\">Este informe recoge la información de contactos para las llamadas realizadas en el intervalo de fechas seleccionado. Un contacto sólo se exporta una vez sin importar cuántas llamadas fueron realizadas. El estado actual del contacto es usado.</FONT><BR><BR>\n";
