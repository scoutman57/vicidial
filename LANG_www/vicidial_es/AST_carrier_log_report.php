...
$report_name='Carrier Informe de registro';
...
if ($ENVIAR && $server_ip_ct>0) {
	...
	if (mysql_num_rows($rslt)>0) {
		...
		$MAIN.="--- CARRIER LOG RECORDS FOR $query_date, $query_date_D TO $query_date_T $server_rpt_string, RECORDS #$lower_limit-$upper_limit               <a href=\"$PHP_SELF?ENVIAR=$ENVIAR&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">[DESCARGAR]</a>\n";
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+----------+--------------------------------+--------------+\n";
		$carrier_rpt.="| UNIQUE ID            | CALL DATE           | SERVIDOR IP       | LEAD ID   | HANGUP CAUSE | DIAL STATUS | CHANNEL                                  | DIAL TIME | RESPUESTAED TIME | SIP CODE | SIP HANGUP REASON              | PHONE NUMBER |\n";
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+----------+--------------------------------+--------------+\n";
		$CSV_text="\"UNIQUE ID\",\"CALL DATE\",\"SERVIDOR IP\",\"LEAD ID\",\"HANGUP CAUSE\",\"DIAL STATUS\",\"CHANNEL\",\"DIAL TIME\",\"RESPUESTAED TIME\",\"SIP CODE\",\"SIP HANGUP REASON\",\"PHONE NUMBER\"\n";
		...
		$ll=$lower_limit-1000;
		if ($ll>=1) {
			$carrier_rpt_hf.="<a href=\"$PHP_SELF?ENVIAR=$ENVIAR&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS&lower_limit=$ll\">[<<< PREV 1000 records]</a>";
		} else {
		...
		if (($lower_limit+1000)<mysql_num_rows($rpt_rslt)) {
			...
			$carrier_rpt_hf.="<a href=\"$PHP_SELF?ENVIAR=$ENVIAR&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS&lower_limit=".($lower_limit+1000)."\">[NEXT $max_limit records >>>]</a>";
		} else {
			$carrier_rpt_hf.=sprintf("%23s", " ");
		}
		$carrier_rpt_hf.="\n";
