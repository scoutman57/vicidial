...
if ($lead_id == 'NEW')
	...
	if ($affected_rows > 0)
		...
		echo "El plomo se ha añadido: $lead_id<BR><BR>\n";
		$end_call=0;
		}
	else
		{echo "ERROR: El plomo no se añade, por favor, volver atrás y ver lo que ha introducido<BR><BR>\n";}
	}
else
	{
...
#		echo "<a href=\"$PHP_SELF\">Close this window</a>\n<BR><BR>\n";
	...
	if ($LOGadmin_hide_phone_data != '0')
		{
		if ($DB > 0) {echo "PIELPHONEDATA|$row[11]|$LOGadmin_hide_phone_data|\n";}
		$phone_temp = $row[11];
	...
	if ($LOGadmin_hide_lead_data != '0')
		{
		if ($DB > 0) {echo "PIELLEADDATA|$row[5]|$row[6]|$row[12]|$row[13]|$row[14]|$row[15]|$row[16]|$row[17]|$row[18]|$row[19]|$row[20]|$row[21]|$row[22]|$row[26]|$row[27]|$row[28]|$LOGadmin_hide_lead_data|\n";}
		if (strlen($row[5]) > 0)
	...
	if ($lead_id == 'NEW')
		{echo "<br><b>Añadir una nueva pista</B>\n";}
	else
	...
	echo "<tr><td colspan=2>Lead ID: $lead_id &nbsp; &nbsp; ID De la Lista: $list_id</td></tr>\n";
	...
	if ($lead_id != 'NEW') 
		...
		if ( ($dispo == 'CALLBK') or ($dispo == 'CBHOLD') or ($scheduled_callback == 'Y') )
			...
			{
			echo "<BR>Si desea cambiar esta dado lugar a una devolución de llamada programada, primero cambie la disposición de CBHOLD, a continuación, enviar y usted será capaz de establecer la fecha de devolución de llamada y el tiempo.<BR>\n";
			}
		echo "</TD></TR></TABLE>\n";
		...
		if ($custom_fields_enabled > 0)
			...
			$stmt="DEMOSTRACIÓN TABLAS LIKE \"custom_$CLlist_id\";";
			if ($DB>0) {echo "$stmt";}
			$rslt=mysql_query($stmt, $link);
			...
			if ($tablecount_to_print > 0) 
				...
				if ($fieldscount_to_print > 0) 
					...
					echo "<B>Campos personalizados para ESTE CABLE:</B><BR>\n";
	...
	##### vicidial agent outbound calls for this time period #####
		...
		if ($admin_display > 0)
			...
			echo "<a href=\"./admin.php?ADD=720000000000000&stage=$lead_id&category=LEADS\">Click here to see Lead Modificarchanges to this lead</a>\n";
                ...
                //Exponer link to QC if user has QC permissions
