...
if (isset($_GET["CallBackLeadСостояние"]))				{$CallBackLeadСостояние=$_GET["CallBackLeadСостояние"];}
	elseif (isset($_POST["CallBackLeadСостояние"]))	{$CallBackLeadСостояние=$_POST["CallBackLeadСостояние"];}
		...
		if (strlen($phone_number)>3)
			...
			#	$DB=1;
				if (strlen($dial_statuses)>2)
					...
					if (eregi("UP LAST ВЫЗОВ TIME",$lead_order)){$order_stmt = "order by last_local_call_time desc, $last_order";}
					if (eregi("DOWN LAST ВЫЗОВ TIME",$lead_order)){$order_stmt = "order by last_local_call_time, $last_order";}
		...
		if (!$CBleadIDset)
			...
			#	$post_phone_time_diff_alert_message = "$POSTgmt_offset|$POSTdialable   ---   $PHONEgmt_offset|$PHONEdialable|$USarea";
				...
				if ($PHONEgmt_offset != $POSTgmt_offset)
					{
					$post_phone_time_diff_alert_message .= "телефон and Post Code Time Zone Mismatch! ";
...
if ($ACTION == 'updateDISPO')
	...
	#	fclose($fp);
		...
		if (strlen($CallBackLeadСостояние) > 0) {$log_dispo_choice = $CallBackLeadСостояние;}
	...
	if ( ($dispo_choice == 'CBHOLD') and (strlen($CallBackDatETimE)>10) )
		...
		$comments = eregi_replace(';','',$comments);
		$comments = eregi_replace("\\\\",' ',$comments);
		$stmt="INSERT INTO vicidial_callbacks (lead_id,list_id,campaign_id,status,entry_time,callback_time,user,recipient,comments,user_group,lead_status) values('$lead_id','$list_id','$campaign','ACTIVE','$NOW_TIME','$CallBackDatETimE','$user','$recipient','$comments','$user_group','$CallBackLeadСостояние');";
		if ($DB) {echo "$stmt\n";}
		$rslt=mysql_query($stmt, $link);
			if ($mel > 0) {mysql_error_logging($NOW_TIME,$link,$mel,$stmt,'00154',$user,$server_ip,$session_name,$one_mysql_log);}
		}

	### BEGIN Call Примечания Logging ###
	...
	echo 'Lead ' . $lead_id . ' был изменен на ' . $dispo_choice . " Состояние\nNext agent_log_id:\n" . $agent_log_id . "\n";
	}
...
if ($ACTION == 'AGENTSview')
	...
	while ($agents_count > $loop_count)
		...
		if ($comments=='AgentXferViewSelect') 
			{
			$AXVSuserОРДЕР[$j] =	"$full_name$US$j";
			$AXVSuser[$j] =			$user;
	...
	if ($comments=='AgentXferViewSelect') 
		...
		while ($j > $k)
			{
			$order_split = explode("_",$AXVSuserОРДЕР[$k]);
			$i = $order_split[1];
...
if ($ACTION == 'SEARCHRESULTSview')
	...
	if ($camps_to_print > 0) 
		...
		if ($counts_to_print > 0) 
			...
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; PHONE &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; СОСТОЯНИЕ &nbsp; </font></TD>";
			echo "<TD BGCOLOR=\"#CCCCCC\"><font style=\"font-size:11px;font-family:sans-serif;\"><B> &nbsp; LAST ВЫЗОВ &nbsp; </font></TD>";
...
if ($ACTION == 'LEADINFOview')
	...
	if (strlen($lead_id) < 1)
		...
		if ($info_to_print > 0)
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Вызван Since Фамилия Reset: &nbsp; </td><td ALIGN=left><font size=2>$row[4]</td></tr>";
			...
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Вызван Count: &nbsp; </td><td ALIGN=left><font size=2>$row[24]</td></tr>";
			$INFOout .= "<tr bgcolor=white><td ALIGN=right><font size=2>Фамилия Local Call Time: &nbsp; </td><td ALIGN=left><font size=2>$row[25]</td></tr>";
