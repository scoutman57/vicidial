...
$report_name = 'Dialer Rapport d inventaire';
$db_source = 'M';
...
$campaign_span_txt="Campagnes:<BR>";
	...
	if ($snapshot_time && $report_source=="SNAPSHOT") 
		...
		if ($group_ct>0 && $report_type=="CAMPAIGNS") 
			...
			for ($i=0; $i<$group_ct; $i++) 
				...
				if ($DB) {$HTML_header.= "         CAMPAIGN DIAL STATUTS: |$active_dial_statuses|\n";}
			...
			$inventory_statuses=substr($inventory_statuses, 0, -1);
			if ($DB) {$HTML_header.= "         CAMPAIGN DIAL STATUTS: |$active_dial_statuses|\n";}
			if ($DB) {$HTML_header.= "                  DIAL STATUTS: |$dial_statuses|\n";}
			if ($DB) {$HTML_header.= "     INVENTORY STATUSES RESULTS: |$inventory_statuses|\n";}
			if ($DB) {$HTML_header.= "      INACTIVE STATUSES RESULTS: |$inactive_dial_statuses|\n";}
...
#					$oneoff_stmt="SELECT count(*) from (SELECT lead_id, count(*) as count from vicidial_log where lead_id in (SELECT lead_id from vicidial_list where $full_dialable_SQL and gmt_offset_now='$gmt_row[0]' $filter_SQL) $shift_days_SQL group by lead_id) as count_table where count_table.count>='($call_count_target-1)'";
	...
	$rpt_header.="| Call list | List description               | Campagne | Last call date      | Start Inv | Call Inv | Call Inv | Call Inv | Call Inv | Dial Avg | Pen. %  |$rpt_header_SHIFTS\n";
	$rpt_header.="|           |                                |          |                     |           |  Total  | No filtr | One-off  | Inactif|          |         |$rpt_header_SHIFTS_lower\n";
	$rpt_header.="+-----------+--------------------------------+----------+---------------------+-----------+----------+----------+----------+----------+----------+---------+$rpt_header_BORDER\n";

	$CSV_header="\"$report_name\"\n";
	$CSV_header.="\"Date: ".date("m/d/Y")."\",\"\",\"Time: ".date("H:i a")."\"\n\n";
	$CSV_header.="\"Call list\",\"List description\",\"Campagne\",\"Last call date\",\"Start Inv\",\"Call Inv Total\",\"Call Inv - No filter\",\"Call Inv - One-offs\",\"Call Inv - Inactifdialable statuses\",\"Dial Avg\",\"Pen. %\",".substr($CSV_header_SHIFTS,0,-1)."\n";
...
$HTML_text.="	<tr><td><input type='radio' name='report_type' value='CAMPAIGNS' onClick=\"ToggleSpan('campaign_span', 'list_span')\" $cmp_checked>Campagnes</td></tr>\n";
...
$HTML_text.="<font color='BLACK' face='ARIAL,HELVETICA' size='2'> &nbsp; &nbsp; &nbsp; &nbsp; &nbsp; <a href='$PHP_SELF?DB=$DB$groupQS&selected_list=$selected_list&report_type=$report_type&time_setting=$time_setting&report_source=$report_source&snapshot_time=$snapshot_time&file_download=1&VALIDER=$VALIDER'>TÉLÉCHARGER</a> | <a href='./admin.php?ADD=999999'>RAPPORTS</a> </font><BR><BR>\n";
$HTML_text.="<input name='VALIDER' value='VALIDER' type='submit'>\n";
$HTML_text.="</TD>\n";
$HTML_text.="</TR>\n";
$HTML_text.="</TABLE>\n";
