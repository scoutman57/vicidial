...
<title>ADMINISTRATION: Campagne Listes Reset
...
if ( ($LOGuser_level >= 9) and $LOGmodify_campaigns>0 and $LOGmodify_lists>0 and ( (preg_match("/Administration Change Log/",$LOGallowed_reports)) or (preg_match("/ALL RAPPORTS/",$LOGallowed_reports)) ) )
	{
	...
	echo "<br><B>Réinitialiser plomb appelé inscrits pour les campagnes:</B><BR><BR>\n";
	...
	echo "</select>";
	echo "</td>";
	echo "<td align='right'><input type='submit' name='submit_campaign_reset' value='VALIDER'></td></tr>";
	...
	if ($submit_campaign_reset && $reset_lead_called_campaigns) {
		...
		while ($list_id_row=mysqli_fetch_array($list_id_rslt)) {
			...
			echo "<LI>ID DE LA LISTE $list_id - ";
