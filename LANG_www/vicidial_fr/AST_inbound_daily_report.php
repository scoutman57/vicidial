...
#		}
	...
	#			$dtt = $dt[$i];
				...
				if (preg_match('/ABANDON|NOAGENT|QUEUETIMEOUT|AFTERHOURS|MAXCALLS/', $tr[$i])) 
					...
					$FtotREPONSES++;
					print "<!-- $FtotREPONSESspeed+=$qs[$i] //-->\n";
					$FtotREPONSESspeed+=$qs[$i];
					}
	...
	*/
	...
	$AVGREPONSESPEED_graph=$graph_header."<th class='thgraph' scope='col'>AVG REPONSE SPEED</th></tr>";
	...
	while ($d < $TOTintervals)
		...
		$totREPONSESavgspeedTIME[$d] =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESspeeddate[$d], $totREPONSESdate[$d])))));
		$totREPONSESavgTIME[$d] =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESsecdate[$d], $totREPONSESdate[$d])))));
		if (round(MathZDC($totREPONSESspeeddate[$d], $totREPONSESdate[$d]))>$max_avganswerspeed) {$max_avganswerspeed=round(MathZDC($totREPONSESspeeddate[$d], $totREPONSESdate[$d]));}
		$graph_stats[$d][12]=round(MathZDC($totREPONSESspeeddate[$d], $totREPONSESdate[$d]));
		$graph_stats[$d][16]=round(MathZDC($totREPONSESsecdate[$d], $totREPONSESdate[$d]));

		$totREPONSEStalkTIME[$d] =	sprintf("%10s", floor(MathZDC($totREPONSESsecdate[$d], 3600)).date(":i:s", mktime(0, 0, $totREPONSESsecdate[$d])));
		$totREPONSESwrapTIME[$d] =	sprintf("%10s", floor(MathZDC(($totREPONSESdate[$d]*15), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESdate[$d]*15))));
		if (($totREPONSESdate[$d]*15)>$max_totalwraptime) {$max_totalwraptime=($totREPONSESdate[$d]*15);}
		$graph_stats[$d][13]=($totREPONSESdate[$d]*15);
		$graph_stats[$d][14]=($totREPONSESsecdate[$d]+($totREPONSESdate[$d]*15));
		$graph_stats[$d][15]=$totREPONSESsecdate[$d];

		$totREPONSEStotTIME[$d] =	sprintf("%10s", floor(MathZDC(($totREPONSESsecdate[$d]+($totREPONSESdate[$d]*15)), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESsecdate[$d]+($totREPONSESdate[$d]*15)))));
		$totREPONSESdate[$d] =	sprintf("%8s", $totREPONSESdate[$d]);
		...
		if (date("w", strtotime($daySTART[$d]))==0 && date("w", strtotime($daySTART[$d-1]))!=0 && $d>0) 
			...
			$totREPONSESavgspeedTIMEwtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESspeedwtd, $totREPONSESwtd)))));
			$totREPONSESavgTIMEwtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESsecwtd, $totREPONSESwtd)))));
			if (round(MathZDC($totREPONSESspeedwtd, $totREPONSESwtd))>$max_wtd_avganswerspeed) {$max_wtd_avganswerspeed=round(MathZDC($totREPONSESspeedwtd, $totREPONSESwtd));}
			$wtd_graph_stats[$wa][12]=round(MathZDC($totREPONSESspeedwtd, $totREPONSESwtd));
			$wtd_graph_stats[$wa][16]=round(MathZDC($totREPONSESsecwtd, $totREPONSESwtd));
			$totREPONSEStalkTIMEwtd =	sprintf("%10s", floor(MathZDC($totREPONSESsecwtd, 3600)).date(":i:s", mktime(0, 0, $totREPONSESsecwtd)));
			$totREPONSESwrapTIMEwtd =	sprintf("%10s", floor(MathZDC(($totREPONSESwtd*15), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESwtd*15))));
			if (($totREPONSESwtd*15)>$max_wtd_totalwraptime) {$max_wtd_totalwraptime=($totREPONSESwtd*15);}
			$wtd_graph_stats[$wa][13]=($totREPONSESwtd*15);
			$wtd_graph_stats[$wa][14]=($totREPONSESsecwtd+($totREPONSESwtd*15));
			$wtd_graph_stats[$wa][15]=$totREPONSESsecwtd;
			$totREPONSEStotTIMEwtd =	sprintf("%10s", floor(MathZDC(($totREPONSESsecwtd+($totREPONSESwtd*15)), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESsecwtd+($totREPONSESwtd*15)))));
			...
			if (round(MathZDC($totREPONSESsecwtd, $totREPONSESwtd))>$max_wtd_avgtalktime) {$max_wtd_avgtalktime=round(MathZDC($totREPONSESsecwtd, $totREPONSESwtd));}
			if (trim($totREPONSESsecwtd)>$max_wtd_totaltalktime) {$max_wtd_totaltalktime=trim($totREPONSESsecwtd);}
			if (trim($totREPONSESsecwtd+($totREPONSESwtd*15))>$max_wtd_totalcalltime) {$max_wtd_totalcalltime=trim($totREPONSESsecwtd+($totREPONSESwtd*15));}
			...
			$wtd_graph_stats[$wa][2]=trim($totREPONSESwtd);
			...
			$wtd_graph_stats[$wa][6]=trim($totREPONSESavgspeedTIMEwtd);
			$wtd_graph_stats[$wa][7]=trim($totREPONSESavgTIMEwtd);
		...
		if (date("d", strtotime($daySTART[$d]))==1 && $d>0 && date("d", strtotime($daySTART[$d-1]))!=1) 
			...
			$totREPONSESavgspeedTIMEmtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESspeedmtd, $totREPONSESmtd)))));
			$totREPONSESavgTIMEmtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESsecmtd, $totREPONSESmtd)))));
			if (round(MathZDC($totREPONSESspeedmtd, $totREPONSESmtd))>$max_mtd_avganswerspeed) {$max_mtd_avganswerspeed=round(MathZDC($totREPONSESspeedmtd, $totREPONSESmtd));}
			$mtd_graph_stats[$ma][12]=round(MathZDC($totREPONSESspeedmtd, $totREPONSESmtd));
			$mtd_graph_stats[$ma][16]=round(MathZDC($totREPONSESsecmtd, $totREPONSESmtd));
			$totREPONSEStalkTIMEmtd =	sprintf("%10s", floor(MathZDC($totREPONSESsecmtd, 3600)).date(":i:s", mktime(0, 0, $totREPONSESsecmtd)));
			$totREPONSESwrapTIMEmtd =	sprintf("%10s", floor(MathZDC(($totREPONSESmtd*15), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESmtd*15))));
			if (($totREPONSESmtd*15)>$max_mtd_totalwraptime) {$max_mtd_totalwraptime=($totREPONSESmtd*15);}
			$mtd_graph_stats[$ma][13]=($totREPONSESmtd*15);
			$mtd_graph_stats[$ma][14]=($totREPONSESsecmtd+($totREPONSESmtd*15));
			$mtd_graph_stats[$ma][15]=$totREPONSESsecmtd;
			$totREPONSEStotTIMEmtd =	sprintf("%10s", floor(MathZDC(($totREPONSESsecmtd+($totREPONSESmtd*15)), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESsecmtd+($totREPONSESmtd*15)))));
			$totAGENTSmtd=count(array_count_values($AGENTS_mtd_array));
			...
			if (round(MathZDC($totREPONSESsecmtd, $totREPONSESmtd))>$max_mtd_avgtalktime) {$max_mtd_avgtalktime=round(MathZDC($totREPONSESsecmtd, $totREPONSESmtd));}
			if (trim($totREPONSESsecmtd)>$max_mtd_totaltalktime) {$max_mtd_totaltalktime=trim($totREPONSESsecmtd);}
			if (trim($totREPONSESsecmtd+($totREPONSESmtd*15))>$max_mtd_totalcalltime) {$max_mtd_totalcalltime=trim($totREPONSESsecmtd+($totREPONSESmtd*15));}
			$month=date("F", strtotime($dayEND[$d-1]));
			$year=substr($dayEND[$d-1], 0, 4);
			$mtd_graph_stats[$ma][0]="$month $year";
			$mtd_graph_stats[$ma][1]=trim($totCALLSmtd);
			$mtd_graph_stats[$ma][2]=trim($totREPONSESmtd);
			...
			$mtd_graph_stats[$ma][6]=trim($totREPONSESavgspeedTIMEmtd);
			$mtd_graph_stats[$ma][7]=trim($totREPONSESavgTIMEmtd);
			...
			if (date("m", strtotime($daySTART[$d]))==1 || date("m", strtotime($daySTART[$d]))==4 || date("m", strtotime($daySTART[$d]))==7 || date("m", strtotime($daySTART[$d]))==10) # Quarterly line
				...
				$totREPONSESavgspeedTIMEqtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESspeedqtd, $totREPONSESqtd)))));
				$totREPONSESavgTIMEqtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totREPONSESsecqtd, $totREPONSESqtd)))));
				if (round(MathZDC($totREPONSESspeedqtd, $totREPONSESqtd))>$max_qtd_avganswerspeed) {$max_qtd_avganswerspeed=round(MathZDC($totREPONSESspeedqtd, $totREPONSESqtd));}
				$qtd_graph_stats[$qa][12]=round(MathZDC($totREPONSESspeedqtd, $totREPONSESqtd));
				$qtd_graph_stats[$qa][16]=round(MathZDC($totREPONSESsecqtd, $totREPONSESqtd));
				$totREPONSEStalkTIMEqtd =	sprintf("%10s", floor(MathZDC($totREPONSESsecqtd, 3600)).date(":i:s", mktime(0, 0, $totREPONSESsecqtd)));
				$totREPONSESwrapTIMEqtd =	sprintf("%10s", floor(MathZDC(($totREPONSESqtd*15), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESqtd*15))));
				if (($totREPONSESqtd*15)>$max_qtd_totalwraptime) {$max_qtd_totalwraptime=($totREPONSESqtd*15);}
				$qtd_graph_stats[$qa][13]=($totREPONSESqtd*15);
				$qtd_graph_stats[$qa][14]=($totREPONSESsecqtd+($totREPONSESqtd*15));
				$qtd_graph_stats[$qa][15]=$totREPONSESsecqtd;
				$totREPONSEStotTIMEqtd =	sprintf("%10s", floor(MathZDC(($totREPONSESsecqtd+($totREPONSESqtd*15)), 3600)).date(":i:s", mktime(0, 0, ($totREPONSESsecqtd+($totREPONSESqtd*15)))));
				$totAGENTSqtd=count(array_count_values($AGENTS_qtd_array));
				...
				if (round(MathZDC($totREPONSESsecqtd, $totREPONSESqtd))>$max_qtd_avgtalktime) {$max_qtd_avgtalktime=round(MathZDC($totREPONSESsecqtd, $totREPONSESqtd));}
				if (trim($totREPONSESsecqtd)>$max_qtd_totaltalktime) {$max_qtd_totaltalktime=trim($totREPONSESsecqtd);}
				if (trim($totREPONSESsecqtd+($totREPONSESqtd*15))>$max_qtd_totalcalltime) {$max_qtd_totalcalltime=trim($totREPONSESsecqtd+($totREPONSESqtd*15));}
				...
				$qtd_graph_stats[$qa][2]=trim($totREPONSESqtd);
				...
				$qtd_graph_stats[$qa][6]=trim($totREPONSESavgspeedTIMEqtd);
				$qtd_graph_stats[$qa][7]=trim($totREPONSESavgTIMEqtd);
		...
		$totREPONSESspeedwtd+=$totREPONSESspeeddate[$d];
		$totABANDONSwtd+=$totABANDONSdate[$d];
		$totABANDONSsecwtd+=$totABANDONSsecdate[$d];
		$totCALLSmtd+=$totCALLSdate[$d];
		$totREPONSESmtd+=$totREPONSESdate[$d];
		$totREPONSESsecmtd+=$totREPONSESsecdate[$d];
		$totREPONSESspeedmtd+=$totREPONSESspeeddate[$d];
		$totABANDONSmtd+=$totABANDONSdate[$d];
		$totABANDONSsecmtd+=$totABANDONSsecdate[$d];
		$totCALLSqtd+=$totCALLSdate[$d];
		$totREPONSESqtd+=$totREPONSESdate[$d];
		$totREPONSESsecqtd+=$totREPONSESsecdate[$d];
		$totREPONSESspeedqtd+=$totREPONSESspeeddate[$d];
		...
		if (round(MathZDC($totREPONSESsecdate[$d], $totREPONSESdate[$d]))>$max_avgtalktime) 
			{$max_avgtalktime=round(MathZDC($totREPONSESsecdate[$d], $totREPONSESdate[$d]));}

		if (trim($totREPONSESsecdate[$d])>$max_totaltalktime) {$max_totaltalktime=trim($totREPONSESsecdate[$d]);}
		if (trim($totREPONSESsecdate[$d]+($totREPONSESdate[$d]*15))>$max_totalcalltime) {$max_totalcalltime=trim($totREPONSESsecdate[$d]+($totREPONSESdate[$d]*15));}
		$graph_stats[$d][0]="$daySTART[$d] - $dayEND[$d]";
		$graph_stats[$d][1]=trim($totCALLSdate[$d]);
		$graph_stats[$d][2]=trim($totREPONSESdate[$d]);
		$graph_stats[$d][3]=trim($totABANDONSdate[$d]);
		$graph_stats[$d][4]=trim($totABANDONSpctDATE[$d]);
		$graph_stats[$d][5]=trim($totABANDONSavgTIME[$d]);
		$graph_stats[$d][6]=trim($totREPONSESavgspeedTIME[$d]);
		$graph_stats[$d][7]=trim($totREPONSESavgTIME[$d]);
	...
	$totTIME_MS =		sprintf("%9s", $totTIME_MS);
		...
		$FtotREPONSESavgspeedTIME =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($FtotREPONSESspeed, $FtotREPONSES)))));
		$FtotREPONSESavgTIME =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($FtotREPONSESsec, $FtotREPONSES)))));
		$FtotREPONSEStalkTIME =	sprintf("%10s", floor(MathZDC($FtotREPONSESsec, 3600)).date(":i:s", mktime(0, 0, $FtotREPONSESsec)));
		$FtotREPONSESwrapTIME =	sprintf("%10s", floor(MathZDC(($FtotREPONSES*15), 3600)).date(":i:s", mktime(0, 0, ($FtotREPONSES*15))));
		$FtotREPONSEStotTIME =	sprintf("%10s", floor(MathZDC(($FtotREPONSESsec+($FtotREPONSES*15)), 3600)).date(":i:s", mktime(0, 0, ($FtotREPONSESsec+($FtotREPONSES*15)))));
		$FtotREPONSES =	sprintf("%8s", $FtotREPONSES);
		...
		if (date("w", strtotime($daySTART[$d]))>0) 
			...
			if (trim($totREPONSESavgTIMEwtd)>$max_wtd_avgtalktime) {$max_wtd_avgtalktime=trim($totREPONSESavgTIMEwtd);}
			if (trim($totREPONSESsecwtd)>$max_wtd_totaltalktime) {$max_wtd_totaltalktime=trim($totREPONSESsecwtd);}
			if (trim($totREPONSESsecwtd+($totREPONSESwtd*15))>$max_wtd_totalcalltime) {$max_wtd_totalcalltime=trim($totREPONSESsecwtd+($totREPONSESwtd*15));}
			...
			$wtd_AVGREPONSESPEED_graph=preg_replace('/DAILY/', 'WEEK-TO-DATE',$AVGREPONSESPEED_graph);
			...
			for ($q=0; $q<count($wtd_graph_stats); $q++) {
				...
				$wtd_REPONSEED_graph.="  <tr><td class='chart_td$class'>".$wtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$wtd_graph_stats[$q][2], $max_wtd_answered))."' height='16' />".$wtd_graph_stats[$q][2]."</td></tr>";
				...
				$wtd_AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$wtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$wtd_graph_stats[$q][12], $max_wtd_avganswerspeed))."' height='16' />".$wtd_graph_stats[$q][6]."</td></tr>";
		...
		if (date("d", strtotime($daySTART[$d]))!=1) 
			...
			if (round(MathZDC($totREPONSESsecmtd, $totREPONSESmtd))>$max_mtd_avgtalktime)
				{$max_mtd_avgtalktime=round(MathZDC($totREPONSESsecmtd, $totREPONSESmtd));}
			...
			$mtd_AVGREPONSESPEED_graph=preg_replace('/DAILY/', 'MONTH-TO-DATE',$AVGREPONSESPEED_graph);
			...
			for ($q=0; $q<count($mtd_graph_stats); $q++) {
				...
				$mtd_REPONSEED_graph.="  <tr><td class='chart_td$class'>".$mtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$mtd_graph_stats[$q][2], $max_mtd_answered))."' height='16' />".$mtd_graph_stats[$q][2]."</td></tr>";
				...
				$mtd_AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$mtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$mtd_graph_stats[$q][12], $max_mtd_avganswerspeed))."' height='16' />".$mtd_graph_stats[$q][6]."</td></tr>";
				...
				$graph_totREPONSESavgspeedTIMEmtd+=$mtd_graph_stats[$q][6];
				$graph_totREPONSESavgTIMEmtd+=$mtd_graph_stats[$q][7];
				$graph_totREPONSEStalkTIMEmtd+=$mtd_graph_stats[$q][8];
	...
	#			{
			...
			if ( (round(MathZDC($totREPONSESsecqtd, $totREPONSESqtd))>$max_qtd_avgtalktime) )
				{$max_qtd_avgtalktime=round(MathZDC($totREPONSESsecqtd, $totREPONSESqtd));}
			...
			$qtd_AVGREPONSESPEED_graph=preg_replace('/DAILY/', 'QUARTER-TO-DATE',$AVGREPONSESPEED_graph);
			...
			for ($q=0; $q<count($qtd_graph_stats); $q++) {
				...
				$qtd_REPONSEED_graph.="  <tr><td class='chart_td$class'>".$qtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$qtd_graph_stats[$q][2], $max_qtd_answered))."' height='16' />".$qtd_graph_stats[$q][2]."</td></tr>";
				...
				$qtd_AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$qtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$qtd_graph_stats[$q][12], $max_qtd_avganswerspeed))."' height='16' />".$qtd_graph_stats[$q][6]."</td></tr>";
				...
				$graph_totREPONSESavgspeedTIMEqtd+=$qtd_graph_stats[$q][6];
				$graph_totREPONSESavgTIMEqtd+=$qtd_graph_stats[$q][7];
				$graph_totREPONSEStalkTIMEqtd+=$qtd_graph_stats[$q][8];
				$graph_totREPONSESwrapTIMEqtd+=$qtd_graph_stats[$q][9];
	...
	#			}
			...
			for ($q=0; $q<count($graph_stats); $q++) {
				...
				$REPONSEED_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$graph_stats[$q][2], $max_answered))."' height='16' />".$graph_stats[$q][2]."</td></tr>";
				...
				$AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$graph_stats[$q][12], $max_avganswerspeed))."' height='16' />".$graph_stats[$q][6]."</td></tr>";
			...
			$AVGREPONSESPEED_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgspeedTIME)."</th></tr></table>";
			$AVGTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgTIME)."</th></tr></table>";
			$TOTALTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStalkTIME)."</th></tr></table>";
			$TOTALWRAPTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESwrapTIME)."</th></tr></table>";
			$TOTALCALLTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStotTIME)."</th></tr></table>";
			...
			$JS_text.="	var AVGREPONSESPEED_graph=\"$AVGREPONSESPEED_graph\";\n";
			...
			$wtd_AVGREPONSESPEED_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgspeedTIME)."</th></tr></table>";
			$wtd_AVGTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgTIME)."</th></tr></table>";
			$wtd_TOTALTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStalkTIME)."</th></tr></table>";
			$wtd_TOTALWRAPTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESwrapTIME)."</th></tr></table>";
			$wtd_TOTALCALLTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStotTIME)."</th></tr></table>";
			...
			$JS_text.="	var AVGREPONSESPEED_graph=\"$wtd_AVGREPONSESPEED_graph\";\n";
			...
			$mtd_AVGREPONSESPEED_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgspeedTIME)."</th></tr></table>";
			$mtd_AVGTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgTIME)."</th></tr></table>";
			$mtd_TOTALTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStalkTIME)."</th></tr></table>";
			$mtd_TOTALWRAPTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESwrapTIME)."</th></tr></table>";
			$mtd_TOTALCALLTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStotTIME)."</th></tr></table>";
			...
			$JS_text.="	var AVGREPONSESPEED_graph=\"$mtd_AVGREPONSESPEED_graph\";\n";
			...
			$qtd_AVGREPONSESPEED_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgspeedTIME)."</th></tr></table>";
			$qtd_AVGTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESavgTIME)."</th></tr></table>";
			$qtd_TOTALTALKTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStalkTIME)."</th></tr></table>";
			$qtd_TOTALWRAPTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSESwrapTIME)."</th></tr></table>";
			$qtd_TOTALCALLTIME_graph.="<tr><th class='thgraph' scope='col'>TOTAL:</th><th class='thgraph' scope='col'>".trim($FtotREPONSEStotTIME)."</th></tr></table>";
			...
			$JS_text.="	var AVGREPONSESPEED_graph=\"$qtd_AVGREPONSESPEED_graph\";\n";
