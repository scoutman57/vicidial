...
if (!$group)
	...
	*/	$totCALLSdate[$d] =	sprintf("%7s", $totCALLSdate[$d]);
		...
		if ($totREPONSESdate[$d]>0)
			{
			$totREPONSESavgspeedTIME[$d] =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESspeeddate[$d]/$totREPONSESdate[$d]))));
			$totREPONSESavgTIME[$d] =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESsecdate[$d]/$totREPONSESdate[$d]))));
			if (round($totREPONSESspeeddate[$d]/$totREPONSESdate[$d])>$max_avganswerspeed) {$max_avganswerspeed=round($totREPONSESspeeddate[$d]/$totREPONSESdate[$d]);}
			$graph_stats[$d][12]=round($totREPONSESspeeddate[$d]/$totREPONSESdate[$d]);
			$graph_stats[$d][16]=round($totREPONSESsecdate[$d]/$totREPONSESdate[$d]);
			}
		else
			{
			$totREPONSESdate[$d]="0";
			$totREPONSESavgspeedTIME[$d] = " 00:00";
			$totREPONSESavgTIME[$d] = " 00:00";
			$graph_stats[$d][12]=0;
			$graph_stats[$d][16]=0;
			}
		$totREPONSEStalkTIME[$d] =	sprintf("%10s", floor($totREPONSESsecdate[$d]/3600).date(":i:s", mktime(0, 0, $totREPONSESsecdate[$d])));
		$totREPONSESwrapTIME[$d] =	sprintf("%10s", floor(($totREPONSESdate[$d]*15)/3600).date(":i:s", mktime(0, 0, ($totREPONSESdate[$d]*15))));
		...
		$totREPONSEStotTIME[$d] =	sprintf("%10s", floor(($totREPONSESsecdate[$d]+($totREPONSESdate[$d]*15))/3600).date(":i:s", mktime(0, 0, ($totREPONSESsecdate[$d]+($totREPONSESdate[$d]*15)))));
		...
		if (date("w", strtotime($daySTART[$d]))==0 && date("w", strtotime($daySTART[$d-1]))!=0 && $d>0) 
			...
			if ($totREPONSESwtd>0)
				{
				$totREPONSESavgspeedTIMEwtd =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESspeedwtd/$totREPONSESwtd))));
				$totREPONSESavgTIMEwtd =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESsecwtd/$totREPONSESwtd))));
				if (round($totREPONSESspeedwtd/$totREPONSESwtd)>$max_wtd_avganswerspeed) {$max_wtd_avganswerspeed=round($totREPONSESspeedwtd/$totREPONSESwtd);}
				$wtd_graph_stats[$wa][12]=round($totREPONSESspeedwtd/$totREPONSESwtd);
				$wtd_graph_stats[$wa][16]=round($totREPONSESsecwtd/$totREPONSESwtd);
				}
			else
				{
				$totREPONSESavgspeedTIMEwtd = " 00:00";
				$totREPONSESavgTIMEwtd = " 00:00";
				$wtd_graph_stats[$wa][12]=0;
				$wtd_graph_stats[$wa][16]=0;
				}
			$totREPONSEStalkTIMEwtd =	sprintf("%10s", floor($totREPONSESsecwtd/3600).date(":i:s", mktime(0, 0, $totREPONSESsecwtd)));
			$totREPONSESwrapTIMEwtd =	sprintf("%10s", floor(($totREPONSESwtd*15)/3600).date(":i:s", mktime(0, 0, ($totREPONSESwtd*15))));
			...
			if (round($totREPONSESsecwtd/$totREPONSESwtd)>$max_wtd_avgtalktime) {$max_wtd_avgtalktime=round($totREPONSESsecwtd/$totREPONSESwtd);}
		...
		if (date("d", strtotime($daySTART[$d]))==1 && $d>0 && date("d", strtotime($daySTART[$d-1]))!=1) {
			...
			if ($totREPONSESmtd>0)
				{
				$totREPONSESavgspeedTIMEmtd =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESspeedmtd/$totREPONSESmtd))));
				$totREPONSESavgTIMEmtd =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESsecmtd/$totREPONSESmtd))));
				if (round($totREPONSESspeedmtd/$totREPONSESmtd)>$max_mtd_avganswerspeed) {$max_mtd_avganswerspeed=round($totREPONSESspeedmtd/$totREPONSESmtd);}
				$mtd_graph_stats[$ma][12]=round($totREPONSESspeedmtd/$totREPONSESmtd);
				$mtd_graph_stats[$ma][16]=round($totREPONSESsecmtd/$totREPONSESmtd);
				}
			else
				{
				$totREPONSESavgspeedTIMEmtd = " 00:00";
				$totREPONSESavgTIMEmtd = " 00:00";
				$mtd_graph_stats[$ma][12]=0;
				$mtd_graph_stats[$ma][16]=0;
				}
			$totREPONSEStalkTIMEmtd =	sprintf("%10s", floor($totREPONSESsecmtd/3600).date(":i:s", mktime(0, 0, $totREPONSESsecmtd)));
			$totREPONSESwrapTIMEmtd =	sprintf("%10s", floor(($totREPONSESmtd*15)/3600).date(":i:s", mktime(0, 0, ($totREPONSESmtd*15))));
			...
			if (date("m", strtotime($daySTART[$d]))==1 || date("m", strtotime($daySTART[$d]))==4 || date("m", strtotime($daySTART[$d]))==7 || date("m", strtotime($daySTART[$d]))==10) # Quarterly line
				...
				if ($totREPONSESqtd>0)
					{
					$totREPONSESavgspeedTIMEqtd =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESspeedqtd/$totREPONSESqtd))));
					$totREPONSESavgTIMEqtd =	sprintf("%6s", date("i:s", mktime(0, 0, round($totREPONSESsecqtd/$totREPONSESqtd))));
					if (round($totREPONSESspeedqtd/$totREPONSESqtd)>$max_qtd_avganswerspeed) {$max_qtd_avganswerspeed=round($totREPONSESspeedqtd/$totREPONSESqtd);}
					$qtd_graph_stats[$qa][12]=round($totREPONSESspeedqtd/$totREPONSESqtd);
					$qtd_graph_stats[$qa][16]=round($totREPONSESsecqtd/$totREPONSESqtd);
					}
				else
					{
					$totREPONSESavgspeedTIMEqtd = " 00:00";
					$totREPONSESavgTIMEqtd = " 00:00";
					$qtd_graph_stats[$qa][12]=0;
					$qtd_graph_stats[$qa][16]=0;
					}
				$totREPONSEStalkTIMEqtd =	sprintf("%10s", floor($totREPONSESsecqtd/3600).date(":i:s", mktime(0, 0, $totREPONSESsecqtd)));
				$totREPONSESwrapTIMEqtd =	sprintf("%10s", floor(($totREPONSESqtd*15)/3600).date(":i:s", mktime(0, 0, ($totREPONSESqtd*15))));
		...
		if (round($totREPONSESsecdate[$d]/$totREPONSESdate[$d])>$max_avgtalktime) {$max_avgtalktime=round($totREPONSESsecdate[$d]/$totREPONSESdate[$d]);}
	...
	if ($totCALLSsec > 0)
		...
		if ($FtotREPONSES>0) 
			{
			$FtotREPONSESavgspeedTIME =	sprintf("%6s", date("i:s", mktime(0, 0, round($FtotREPONSESspeed/$FtotREPONSES))));
			$FtotREPONSESavgTIME =	sprintf("%6s", date("i:s", mktime(0, 0, round($FtotREPONSESsec/$FtotREPONSES))));
			}
		else 
			{
			$FtotREPONSESavgspeedTIME =	sprintf("%6s", "00:00");
			$FtotREPONSESavgTIME =	sprintf("%6s", "00:00");
			}
		$FtotREPONSEStalkTIME =	sprintf("%10s", floor($FtotREPONSESsec/3600).date(":i:s", mktime(0, 0, $FtotREPONSESsec)));
		$FtotREPONSESwrapTIME =	sprintf("%10s", floor(($FtotREPONSES*15)/3600).date(":i:s", mktime(0, 0, ($FtotREPONSES*15))));
		$FtotREPONSEStotTIME =	sprintf("%10s", floor(($FtotREPONSESsec+($FtotREPONSES*15))/3600).date(":i:s", mktime(0, 0, ($FtotREPONSESsec+($FtotREPONSES*15)))));
		...
		if (date("w", strtotime($daySTART[$d]))>0) 
			...
			for ($q=0; $q<count($wtd_graph_stats); $q++) {
				...
				$wtd_AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$wtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(400*$wtd_graph_stats[$q][12]/$max_wtd_avganswerspeed)."' height='16' />".$wtd_graph_stats[$q][6]."</td></tr>";
		...
		if (date("d", strtotime($daySTART[$d]))!=1) 
			...
			for ($q=0; $q<count($mtd_graph_stats); $q++) {
				...
				$mtd_AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$mtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(400*$mtd_graph_stats[$q][12]/$max_mtd_avganswerspeed)."' height='16' />".$mtd_graph_stats[$q][6]."</td></tr>";
	...
	#			{
				...
				for ($q=0; $q<count($qtd_graph_stats); $q++) {
					...
					$qtd_AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$qtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(400*$qtd_graph_stats[$q][12]/$max_qtd_avganswerspeed)."' height='16' />".$qtd_graph_stats[$q][6]."</td></tr>";
	...
	#			}
			...
			for ($q=0; $q<count($graph_stats); $q++) {
				...
				$AVGREPONSESPEED_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(400*$graph_stats[$q][12]/$max_avganswerspeed)."' height='16' />".$graph_stats[$q][6]."</td></tr>";
