...
if ( (!$vendor_id) and (!$phone)  and (!$lead_id) and (!$log_phone)  and (!$log_lead_id) and ( (strlen($status)<1) and (strlen($list_id)<1) and (strlen($user)<1) ) and ( (strlen($first_name)<1) and (strlen($last_name)<1) )) 
	...
	echo "<TD ALIGN=right>Vendor ID(code vendeur de la fiche): &nbsp; </TD><TD ALIGN=left><input type=text name=vendor_id size=10 maxlength=10></TD>";
	echo "<TD><input type=submit name=submit value=VALIDER></TD>\n";
	echo "</TR><TR>";
	echo "<TD colspan=3 align=center> &nbsp; </TD>";
	echo "</TR><TR bgcolor=#B9CBFD>";

	echo "<TD ALIGN=right>Home Numéro de téléphone: &nbsp; </TD><TD ALIGN=left><input type=text name=phone size=14 maxlength=18></TD>";
	...
	echo "<TD rowspan=3><input type=submit name=submit value=VALIDER></TD>\n";
	echo "</TR><TR bgcolor=#B9CBFD>";
	...
	echo "<TD ALIGN=right>Nom: &nbsp; </TD><TD ALIGN=left><input type=text name=first_name size=15 maxlength=30></TD>";
	echo "<TD rowspan=2><input type=submit name=submit value=VALIDER></TD>\n";
	echo "</TR><TR bgcolor=#B9CBFD>";
	echo "<TD ALIGN=right>Prénom: &nbsp; </TD><TD ALIGN=left><input type=text name=last_name size=15 maxlength=30></TD>";
	echo "</TR>";
	...
	echo "<TD ALIGN=right>Numéro de téléphone Dialed: &nbsp; </TD><TD ALIGN=left><input type=text name=log_phone size=18 maxlength=18></TD>";
	echo "<TD><input type=submit name=submit value=VALIDER></TD>\n";
	echo "</TR><TR>";
	echo "<TD colspan=3 align=center> &nbsp; </TD>";
	echo "</TR><TR bgcolor=#B9CBFD>";
	...
	if ( (strlen($log_lead_id)>0) or (strlen($log_phone)>0) )
		...
		if ( ($results_to_print < 1) and ($results_to_printX < 1) )
			...
			{
			echo "<BR><b>SORTANT LOG RESULTS: $results_to_print</b><BR>\n";
		...
		if ( ($results_to_print < 1) and ($results_to_printX < 1) )
			...
			{
			echo "<BR><b>ENTRANT LOG RESULTS: $results_to_print</b><BR>\n";
			echo "<TABLE BGCOLOR=WHITE CELLPADDING=1 CELLSPACING=0 WIDTH=770>\n";
			echo "<TR BGCOLOR=BLACK>\n";
			echo "<TD ALIGN=LEFT VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>#</B></FONT></TD>\n";
			echo "<TD ALIGN=CENTER VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>LEAD ID</B> &nbsp;</FONT></TD>\n";
		...
		if (strlen($stmtC) > 10)
			...
			if ( ($results_to_print < 1) and ($results_to_printX < 1) )
				...
				{
				echo "<BR><b>ENTRANT DID LOG RESULTS: $results_to_print</b><BR>\n";
				echo "<TABLE BGCOLOR=WHITE CELLPADDING=1 CELLSPACING=0 WIDTH=770>\n";
				echo "<TR BGCOLOR=BLACK>\n";
				echo "<TD ALIGN=LEFT VALIGN=TOP><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE><B>#</B></FONT></TD>\n";
