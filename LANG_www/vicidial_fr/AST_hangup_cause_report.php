...
$master_dialstatus_array=array("REPONSE", "BUSY", "NOREPONSE", "CANCEL", "CONGESTION", "CHANUNAVAIL", "DONTCALL", "TORTURE", "INVALIDARGS");
...
if  (preg_match('/\-\-ALL\-\-/',$dialstatuses_string))
	{$MAIN.="<option value=\"--ALL--\" selected>-- ALL DIAL STATUTS --</option>\n";}
else
	{$MAIN.="<option value=\"--ALL--\">-- ALL DIAL STATUTS --</option>\n";}
...
if ($VALIDER && $server_ip_ct>0) {
	...
	if (mysqli_num_rows($rslt)>0) {
		...
		$MAIN.="--- CARRIER LOG RECORDS FOR $query_date, $query_date_D TO $query_date_T $server_rpt_string, $HC_rpt_string, $DS_rpt_string\n --- RECORDS #$lower_limit-$upper_limit               <a href=\"$PHP_SELF?VALIDER=$VALIDER&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS&lower_limit=$lower_limit&upper_limit=$upper_limit&file_download=1\">[TÉLÉCHARGER]</a>\n";
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+--------------+--------------+--------------------------------+\n";
		$carrier_rpt.="| UNIQUE ID            | CALL DATE           | SERVEUR IP       | LEAD ID   | HANGUP CAUSE | DIAL STATUS | CHANNEL                                  | DIAL TIME | REPONSEED TIME | PHONE NUMBER | SIP RESPONSE | SIP REASON                     |\n";
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+--------------+--------------+--------------------------------+\n";
		$CSV_text="\"UNIQUE ID\",\"CALL DATE\",\"SERVEUR IP\",\"LEAD ID\",\"HANGUP CAUSE\",\"DIAL STATUS\",\"CHANNEL\",\"DIAL TIME\",\"REPONSEED TIME\",\"PHONE NUMBER\",\"SIP RESPONSE\",\"SIP REASON\"\n";
		...
		$ll=$lower_limit-1000;
		if ($ll>=1) {
			$carrier_rpt_hf.="<a href=\"$PHP_SELF?VALIDER=$VALIDER&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$hangup_causeQS$sip_hangup_causeQS$dial_statusQS&lower_limit=$ll\">[<<< PREV 1000 records]</a>";
		} else {
		...
		if (($lower_limit+1000)<mysqli_num_rows($rpt_rslt)) {
			...
			$carrier_rpt_hf.="<a href=\"$PHP_SELF?VALIDER=$VALIDER&DB=$DB&type=$type&query_date=$query_date&query_date_D=$query_date_D&query_date_T=$query_date_T$server_ipQS$hangup_causeQS$sip_hangup_causeQS$dial_statusQS&lower_limit=".($lower_limit+1000)."\">[NEXT $max_limit records >>>]</a>";
		} else {
			$carrier_rpt_hf.=sprintf("%23s", " ");
		}
		$carrier_rpt_hf.="\n";
