	...
	if ($file_layout!="custom") 
		...
		<table align=center width="780" border=0 cellpadding=5 cellspacing=0 bgcolor=#D9E6FE>
		  ...
		  <tr>
			...
			<select name='list_id_override' onchange="PopulateStatuts(this.value)">
...
?>
			</select> <a href='AST_admin_template_maker.php'><font face="arial, helvetica" size=1>générateur de modèle</font></a><?php echo "$NWB#list_loader-template_id$NWE"; ?><BR><a href='#' onClick="TemplateSpecs()"><font face="arial, helvetica" size=1>View template info</font></a></td>
		  <tr>
	...
	<tr bgcolor="#D9E6FE">
		<td width='25%' align="right"><font class="standard">Vérifier le statut de double:</font></td>
		<td width='75%'>
...
if ($OK_to_process) 
	...
	if (count($field_check)>=2) 
		...
		if (strlen($status_dedupe_str)>0) 
			{
			print "<BR>OMETTANT DUPLICATES CONTRE états suivants SEULEMENT: $status_dedupe_str<BR>\n";
			}
...
if (($leadfile) && ($LF_path))
	...
	if ($file_layout=="template"  && $template_id) 
		...
		if (count($field_check)>=2) 
			...
			if (strlen($list_id_override)>0) 
				{
				print "<BR>ID DE LA LISTE OVERRIDE FOR THIS FILE: $list_id_override<BR>";
				}
			if (strlen($phone_code_override)>0) 
				{
				print "<BR>DÉPASSEMENT DE CODE DE TÉLÉPHONE POUR CE DOSSIER: $phone_code_override<BR>\n";
				}
			if (strlen($template_statuses)>0) 
				{
				print "<BR>OMETTANT DUPLICATES CONTRE états suivants SEULEMENT: ".preg_replace('/\'/', '', $template_statuses)."<BR>\n";
				}
			while (!feof($file)) 
				{
				$record++;
