...
if ($download_type == 'systemdnc')
	...
	if ($leads_count < 1)
		{
		echo "Il n'y a pas les numéros de téléphone figurant sur la liste: SYSTEM INTERNAL DNC\n";
		exit;
		}
	}
elseif ($download_type == 'dnc')
	{
	##### Campagne DNC list validation #####
	...
	if ($leads_count < 1)
		{
		echo "Il n'y a pas conduit dans la liste DNC Campagne: $group_id\n";
		exit;
		}
	}
elseif ($download_type == 'fpgn')
	...
	if ($leads_count < 1)
		{
		echo "Il n'y a pas conduit dans ce groupe de téléphone du filtre: $group_id\n";
		exit;
		}
	}
else
	...
	if ($leads_count < 1)
		{
		echo "Il ya no leads in list_id: $list_id\n";
		exit;
		}
	}
...
if ( ($custom_fields_enabled > 0) and ($event_code_type=='LIST') )
	...
	if ($list_id=='ALL-LISTS')
		...
		while ($lists_ct > $u)
			{
			$stmt="MONTRER TABLES LIKE \"custom_$custom_list_id[$u]\";";
			if ($DB>0) {echo "$stmt";}
			$rslt=mysql_query($stmt, $link);
