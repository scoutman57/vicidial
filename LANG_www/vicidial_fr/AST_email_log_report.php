...
$MAIN.="<BR><BR><BR><CENTER><INPUT TYPE=submit NAME=VALIDER VALUE=VALIDER></CENTER>\n";
$MAIN.="</FONT>\n";
...
$MAIN.=$rpt_title;
$MAIN.="Email log results: $group_string          $NOW_TIME        <a href=\"$PHP_SELF?DB=$DB&query_date=$query_date&end_date=$end_date$groupQS&shift=$shift&VALIDER=$VALIDER&file_download=1&date_type=$date_type&email_type=$email_type\">TÉLÉCHARGER</a>\n";
...
if ($email_type=="received") 
	...
	{
	$rpt_border="+---------------------+--------------------------------+----------------------+----------------------------------------------------+---------------------+---------------------+----------------------+----------------------------------------------------+------------+\n";
	$rpt_header="| DATE EMAIL RECEIVED | ".sprintf("%-30s", "ADDRESS FROM")." | ".sprintf("%-20s", "SENDER NAME")." | ".sprintf("%-50s", "MESSAGE (click to view full text)")." | DATE EMAIL VIEWED   | DATE EMAIL REPONSEED | ".sprintf("%-20s", "USER")." | ".sprintf("%-50s", "RESPONSE (click to view full text)")." | ".sprintf("%-10s", "STATUS")." |\n";
	$CSV_text1.="\"DATE EMAIL RECEIVED\",\"ADDRESS FROM\",\"SENDER NAME\",\"MESSAGE\",\"DATE EMAIL VIEWED\",\"DATE EMAIL REPONSEED\",\"USER\",\"RESPONSE\",\"STATUS\"\n";
	if ($date_type=="email_date") 
		{
