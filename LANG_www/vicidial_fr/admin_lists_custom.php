...
if ($SScustom_fields_enabled < 1)
	{
	echo "ERREUR: les champs personnalisés ne sont pas actifs sur ce système\n";
	exit;
	}
...
if ( ($action == "COPY_FIELDS_VALIDER") and ($list_id > 99) and ($source_list_id > 99) and (strlen($copy_option) > 2) )
	...
	if ($list_id=="$source_list_id")
		{echo "ERREUR: Vous ne pouvez pas copier les champs à la même liste: $list_id|$source_list_id";}
	else
		{
		$table_exists=0;
		...
		if ($source_field_exists < 1)
			{echo "ERREUR: la liste Source n'a pas de champs personnalisés\n<BR>";}
		else
			{
			##### REPLACE option #####
			...
			if ($copy_option=='UPDATE')
				...
				if ($table_exists < 1)
					{echo "ERREUR: La table n'existe pas custom_$list_id\n<BR>";}
				else
					{
					$stmt="SELECT field_id,field_label,field_name,field_description,field_rank,field_help,field_type,field_options,field_size,field_max,field_default,field_cost,field_required,multi_position,name_position,field_order from vicidial_lists_fields where list_id='$source_list_id' order by field_rank,field_order,field_label;";
					$rslt=mysql_query($stmt, $link);
...
if ( ($action == "DELETE_CUSTOM_FIELD_CONFIRMATION") and ($list_id > 99) and ($field_id > 0) and (strlen($field_label) > 0) )
	...
	if ($field_exists < 1)
		{echo "ERREUR: Champ n'existe pas\n<BR>";}
	else
		{
		if ($table_exists < 1)
			{echo "ERREUR: La table n'existe pas custom_$list_id\n<BR>";}
...
if ( ($action == "ADD_CUSTOM_FIELD") and ($list_id > 99) )
	...
	if ( (strlen($field_label)<1) or (strlen($field_name)<2) or (strlen($field_size)<1) )
		...
		if (preg_match("/\|$field_label\|/i",$mysql_reserved_words))
			{echo "ERREUR: Vous ne pouvez pas utiliser des mots réservés pour les étiquettes sur le terrain - $list_id|$field_label|$field_name|$field_size\n<BR>";}
		else
			{
			...
			if ( ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') ) and ( (!preg_match("/,/",$field_options)) or (!preg_match("/\n/",$field_options)) or (strlen($field_options)<6) or ($TEST_valid_options < 1) ) )
				{echo "ERROR: Vous devez entrer dans les options sur le terrain lors de l'ajout d'un SELECT, MULTI, RADIO or CHECKBOX field type  - $list_id|$field_label|$field_type|$field_options\n<BR>";}
			else
				{
				if ($field_exists > 0)
					{echo "ERREUR: Champ existe déjà pour cette liste - $list_id|$field_label\n<BR>";}
				else
					{
					$table_exists=0;
					$linkCUSTOM=mysql_connect("$VARDB_server:$VARDB_port", "$VARDB_custom_user","$VARDB_custom_pass");
...
if ( ($action == "MODIFY_CUSTOM_FIELD_VALIDER") and ($list_id > 99) and ($field_id > 0) )
	...
	if ($field_exists < 1)
		...
		if ($table_exists < 1)
			...
			if ( ( ($field_type=='SELECT') or ($field_type=='MULTI') or ($field_type=='RADIO') or ($field_type=='CHECKBOX') ) and ( (!preg_match("/,/",$field_options)) or (!preg_match("/\n/",$field_options)) or (strlen($field_options)<6) or ($TEST_valid_options < 1) ) )
				{echo "ERROR: Vous devez entrer dans les options sur le terrain lors de mettre à jour un SELECT, MULTI, RADIO or CHECKBOX field type  - $list_id|$field_label|$field_type|$field_options\n<BR>";}
			else
				{
				### modify field function
