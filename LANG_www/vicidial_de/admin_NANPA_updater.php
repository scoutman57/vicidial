...
if (strlen($active_voicemail_server)<7)
	{
	echo "ERROR: Admin -> System Einstellungen -> Aktive Voicemail-Server nicht gesetzt\n";
	exit;
	}
...
if (mysqli_num_rows($schedule_rslt)>0 || (mysqli_num_rows($running_rslt)>0)) {
	...
	if (mysqli_num_rows($schedule_rslt)>0) {
		...
		echo "<td align='left' bgcolor='#015B91' width='300'><FONT FACE=\"ARIAL,HELVETICA\" COLOR=WHITE SIZE=2>Listen</th>";
		...
		while ($row=mysqli_fetch_array($schedule_rslt)) {
			...
			echo "<td align='center'><FONT FACE=\"ARIAL,HELVETICA\" size='1'><a href='$PHP_SELF?delete_trigger_id=$row[trigger_id]'>LÖSCHUNG</a></font></td>";
			echo "</tr>";
		}
		echo "</table>";
		echo "</td></tr>";
