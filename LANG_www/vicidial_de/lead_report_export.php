...
$report_name = 'Export Leads Bericht';
$db_source = 'M';
$file_exported=0;
...
if ($run_export > 0)
	...
	if ($RUNcampaign > 0)
		...
		if ($outbound_to_print < 1)
			{
			echo "Es gibt no outbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ($RUNgroup > 0)
		...
		if ( ($inbound_to_print < 1) and ($outbound_calls < 1) )
			{
			echo "Es gibt no inbound calls during this time period for these parameters\n";
			exit;
			}
		else
			{
	...
	if ( ($outbound_to_print > 0) or ($inbound_to_print > 0) )
		...
		while ($k > $i)
			...
			if ( ($custom_fields_enabled > 0) and ($custom_fields=='YES') )
				...
				$stmt="ZeigeTABELLES LIKE \"custom_$CF_list_id\";";
				if ($DB>0) {echo "$stmt";}
				$rslt=mysql_query($stmt, $link);
		...
		{
		echo "Es gibt no calls during this time period for these parameters\n";
		exit;
		}
	}
##### END RUN THE EXPORT AND OUTPUT FLAT DATA FILE #####
	...
	echo "<CENTER><BR>\n";
	echo "<FONT SIZE=3 FACE=\"Arial,Helvetica\"><B>Export Leads Bericht</B></FONT><BR>\n";
	echo "<FONT SIZE=2 FACE=\"Arial,Helvetica\">Dieser Bericht zieht Blei Informationen für Anrufe in den ausgewählten Zeitraum gewählt. Ein Lead ist nur einmal, egal wie viele Anrufe bearbeitet wurden exportiert. Die Stromzuführung Status verwendet wird.</FONT><BR><BR>\n";
