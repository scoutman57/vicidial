...
$campaign_span_txt="Kampagnen:<BR>";
$campaign_span_txt.="<SELECT SIZE=5 NAME=group[] multiple>";
if  (preg_match('/\-\-ALL\-\-/',$group_string))
	{$campaign_span_txt.="<option value=\"--ALL--\" selected>-- ALL KAMPAGNEN --</option>";}
else
	{$campaign_span_txt.="<option value=\"--ALL--\">-- ALL KAMPAGNEN --</option>";}
$o=0;
	...
	if ($snapshot_time && $report_source=="SNAPSHOT") 
		...
		if ($group_ct>0 && $report_type=="CAMPAIGNS") 
			...
			for ($i=0; $i<$group_ct; $i++) 
				...
				if ($DB) {$HTML_header.= "         CAMPAIGN DIAL Status:|$active_dial_statuses|\n";}
			...
			$inventory_statuses=substr($inventory_statuses, 0, -1);
			if ($DB) {$HTML_header.= "         CAMPAIGN DIAL Status:|$active_dial_statuses|\n";}
			if ($DB) {$HTML_header.= "                  DIAL Status:|$dial_statuses|\n";}
			if ($DB) {$HTML_header.= "     INVENTORY STATUSES RESULTS: |$inventory_statuses|\n";}
			if ($DB) {$HTML_header.= "      INACTIVE STATUSES RESULTS: |$inactive_dial_statuses|\n";}
...
#					$oneoff_stmt="SELECT count(*) from (SELECT lead_id, count(*) as count from vicidial_log where lead_id in (SELECT lead_id from vicidial_list where $full_dialable_SQL and gmt_offset_now='$gmt_row[0]' $filter_SQL) $shift_days_SQL group by lead_id) as count_table where count_table.count>='($call_count_target-1)'";
	...
	$rpt_header.="| Call list | List description               | Kampagne | Last call date      | Start Inv | Call Inv | Call Inv | Call Inv | Call Inv | Dial Avg | Pen. %  |$rpt_header_SHIFTS\n";
	$rpt_header.="|           |                                |          |                     |           |  Gesamt  | No filtr | One-off  | Inaktiv|          |         |$rpt_header_SHIFTS_lower\n";
	$rpt_header.="+-----------+--------------------------------+----------+---------------------+-----------+----------+----------+----------+----------+----------+---------+$rpt_header_Border\n";

	$CSV_header="\"$report_name\"\n";
	$CSV_header.="\"Date: ".date("m/d/Y")."\",\"\",\"Time: ".date("H:i a")."\"\n\n";
	$CSV_header.="\"Call list\",\"List description\",\"Kampagne\",\"Last call date\",\"Start Inv\",\"Call Inv Total\",\"Call Inv - No filter\",\"Call Inv - One-offs\",\"Call Inv - Inaktivdialable statuses\",\"Dial Avg\",\"Pen. %\",".substr($CSV_header_SHIFTS,0,-1)."\n";
...
$HTML_text.="	<tr><td><input type='radio' name='report_type' value='CAMPAIGNS' onClick=\"ToggleSpan('campaign_span', 'list_span')\" $cmp_checked>Kampagnen</td></tr>\n";
