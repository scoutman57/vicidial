...
$master_dialstatus_array=array("ANTWORT", "BUSY", "NOANTWORT", "CANCEL", "CONGESTION", "CHANUNAVAIL", "DONTCALL", "TORTURE", "INVALIDARGS");
...
$MAIN.="</TD><TD ROWSPAN=2 VALIGN=TOP>Bediener IP:<BR/>\n";
...
if ($SUBMIT && $server_ip_ct>0) {
	...
	if (mysqli_num_rows($rslt)>0) {
		...
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+--------------+--------------+--------------------------------+\n";
		$carrier_rpt.="| UNIQUE ID            | CALL DATE           | BEDIENERIP       | LEAD ID   | HANGUP CAUSE | DIAL STATUS | CHANNEL                                  | DIAL TIME | ANTWORTED TIME | PHONE NUMBER | SIP RESPONSE | SIP REASON                     |\n";
		$carrier_rpt.="+----------------------+---------------------+-----------------+-----------+--------------+-------------+------------------------------------------+-----------+---------------+--------------+--------------+--------------------------------+\n";
		$CSV_text="\"UNIQUE ID\",\"CALL DATE\",\"BEDIENERIP\",\"LEAD ID\",\"HANGUP CAUSE\",\"DIAL STATUS\",\"CHANNEL\",\"DIAL TIME\",\"ANTWORTED TIME\",\"PHONE NUMBER\",\"SIP RESPONSE\",\"SIP REASON\"\n";
