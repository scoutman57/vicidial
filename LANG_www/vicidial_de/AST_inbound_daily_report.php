...
$MAIN.="<TR><TD align='left'><FONT FACE=\"ARIAL,HELVETICA\" COLOR=BLACK SIZE=2><INPUT TYPE=checkbox NAME=hourly_breakdown VALUE='checked' $hourly_breakdown>Show hourly results<BR><INPUT TYPE=checkbox NAME=show_disposition_statuses VALUE='checked' $show_disposition_statuses>Show disposition statuses<BR><INPUT TYPE=checkbox NAME=ignore_afterhours VALUE='checked' $ignore_afterhours>Ignore after-hours calls</FONT></TD><TD align='right'><a href=\"$PHP_SELF?DB=$DB&query_date=$query_date&end_date=$end_date$selected_group_URLstr&shift=$shift&hourly_breakdown=$hourly_breakdown&show_disposition_statuses=$show_disposition_statuses&SUBMIT=$SUBMIT&file_download=1\">DOWNLOAD</a> | <a href=\"./admin.php?ADD=3111&group_id=$group\">ÄNDERN Sie</a> | <a href=\"./admin.php?ADD=999999\">REPORTS</a></TD></TR>\n";
...
#		}
	...
	#			$dtt = $dt[$i];
				...
				if (preg_match('/ABANDON|NOAGENT|QUEUETIMEOUT|AFTERHOURS|MAXCALLS/', $tr[$i])) 
					...
					$FtotANTWORTS++;
					print "<!-- $FtotANTWORTSspeed+=$qs[$i] //-->\n";
					$FtotANTWORTSspeed+=$qs[$i];
					}
	...
	*/
	...
	$MAIN2="| SHIFT                                     | CALLS   | CALLS    | AGENTS   | CALLS     | ABANDON | ABANDON | ANTWORT | TALK   | TALK       | WRAP       | CALL       |";
	$MAIN3="| DATE-TIME RANGE                           | OFFERED | ANTWORTED | ANTWORTED | ABANDONED | PERCENT | TIME    | SPEED  | TIME   | TIME       | TIME       | TIME       |";
	$CSV_text1="";
	$CSV_text2="";
	$CSV_text3="";
	...
	$CSV_text.="\"\",\"CALLS\",\"CALLS\",\"AGENTS\",\"CALLS\",\"ABANDON\",\"ABANDON\",\"ANTWORT\",\"TALK\",\"TALK\",\"WRAP\",\"CALL\"$CSV_text2\n";
	$CSV_text.="\"SHIFT DATE-TIME RANGE\",\"OFFERED\",\"ANTWORTED\",\"ANTWORTED\",\"ABANDONED\",\"PERCENT\",\"TIME\",\"SPEED\",\"TIME\",\"TIME\",\"TIME\",\"TIME\"$CSV_text3\n";
	...
	$AGENTS_graph=$graph_header."<th class='thgraph' scope='col'>TOTAL AGENTS ANTWORTED </th></tr>";
	...
	while ($d < $TOTintervals)
		...
		$totANTWORTSavgspeedTIME[$d] =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSspeeddate[$d], $totANTWORTSdate[$d])))));
		$totANTWORTSavgTIME[$d] =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSsecdate[$d], $totANTWORTSdate[$d])))));
		if (round(MathZDC($totANTWORTSspeeddate[$d], $totANTWORTSdate[$d]))>$max_avganswerspeed) {$max_avganswerspeed=round(MathZDC($totANTWORTSspeeddate[$d], $totANTWORTSdate[$d]));}
		$graph_stats[$d][12]=round(MathZDC($totANTWORTSspeeddate[$d], $totANTWORTSdate[$d]));
		$graph_stats[$d][16]=round(MathZDC($totANTWORTSsecdate[$d], $totANTWORTSdate[$d]));

		$totANTWORTStalkTIME[$d] =	sprintf("%10s", floor(MathZDC($totANTWORTSsecdate[$d], 3600)).date(":i:s", mktime(0, 0, $totANTWORTSsecdate[$d])));
		$totANTWORTSwrapTIME[$d] =	sprintf("%10s", floor(MathZDC(($totANTWORTSdate[$d]*15), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSdate[$d]*15))));
		...
		$totANTWORTStotTIME[$d] =	sprintf("%10s", floor(MathZDC(($totANTWORTSsecdate[$d]+($totANTWORTSdate[$d]*15)), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSsecdate[$d]+($totANTWORTSdate[$d]*15)))));
		...
		if (date("w", strtotime($daySTART[$d]))==0 && date("w", strtotime($daySTART[$d-1]))!=0 && $d>0) 
			...
			$totANTWORTSavgspeedTIMEwtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSspeedwtd, $totANTWORTSwtd)))));
			$totANTWORTSavgTIMEwtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSsecwtd, $totANTWORTSwtd)))));
			if (round(MathZDC($totANTWORTSspeedwtd, $totANTWORTSwtd))>$max_wtd_avganswerspeed) {$max_wtd_avganswerspeed=round(MathZDC($totANTWORTSspeedwtd, $totANTWORTSwtd));}
			$wtd_graph_stats[$wa][12]=round(MathZDC($totANTWORTSspeedwtd, $totANTWORTSwtd));
			$wtd_graph_stats[$wa][16]=round(MathZDC($totANTWORTSsecwtd, $totANTWORTSwtd));
			$totANTWORTStalkTIMEwtd =	sprintf("%10s", floor(MathZDC($totANTWORTSsecwtd, 3600)).date(":i:s", mktime(0, 0, $totANTWORTSsecwtd)));
			$totANTWORTSwrapTIMEwtd =	sprintf("%10s", floor(MathZDC(($totANTWORTSwtd*15), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSwtd*15))));
			...
			$totANTWORTStotTIMEwtd =	sprintf("%10s", floor(MathZDC(($totANTWORTSsecwtd+($totANTWORTSwtd*15)), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSsecwtd+($totANTWORTSwtd*15)))));
			...
			if (round(MathZDC($totANTWORTSsecwtd, $totANTWORTSwtd))>$max_wtd_avgtalktime) {$max_wtd_avgtalktime=round(MathZDC($totANTWORTSsecwtd, $totANTWORTSwtd));}
			...
			$ASCII_text.="$MAINH\n";
			$ASCII_text.="|                                       WTD | $totCALLSwtd | $totANTWORTSwtd | $totAGENTSwtd | $totABANDONSwtd | $totABANDONSpctwtd%| $totABANDONSavgTIMEwtd | $totANTWORTSavgspeedTIMEwtd | $totANTWORTSavgTIMEwtd | $totANTWORTStalkTIMEwtd | $totANTWORTSwrapTIMEwtd | $totANTWORTStotTIMEwtd |";
			$CSV_text.="\"WTD\",\"$totCALLSwtd\",\"$totANTWORTSwtd\",\"$totAGENTSwtd\",\"$totABANDONSwtd\",\"$totABANDONSpctwtd%\",\"$totABANDONSavgTIMEwtd\",\"$totANTWORTSavgspeedTIMEwtd\",\"$totANTWORTSavgTIMEwtd\",\"$totANTWORTStalkTIMEwtd\",\"$totANTWORTSwrapTIMEwtd\",\"$totANTWORTStotTIMEwtd\"";
			for ($s=0; $s<count($status_array); $s++) {
		...
		if (date("d", strtotime($daySTART[$d]))==1 && $d>0 && date("d", strtotime($daySTART[$d-1]))!=1) 
			...
			$totANTWORTSavgspeedTIMEmtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSspeedmtd, $totANTWORTSmtd)))));
			$totANTWORTSavgTIMEmtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSsecmtd, $totANTWORTSmtd)))));
			if (round(MathZDC($totANTWORTSspeedmtd, $totANTWORTSmtd))>$max_mtd_avganswerspeed) {$max_mtd_avganswerspeed=round(MathZDC($totANTWORTSspeedmtd, $totANTWORTSmtd));}
			$mtd_graph_stats[$ma][12]=round(MathZDC($totANTWORTSspeedmtd, $totANTWORTSmtd));
			$mtd_graph_stats[$ma][16]=round(MathZDC($totANTWORTSsecmtd, $totANTWORTSmtd));
			$totANTWORTStalkTIMEmtd =	sprintf("%10s", floor(MathZDC($totANTWORTSsecmtd, 3600)).date(":i:s", mktime(0, 0, $totANTWORTSsecmtd)));
			$totANTWORTSwrapTIMEmtd =	sprintf("%10s", floor(MathZDC(($totANTWORTSmtd*15), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSmtd*15))));
			...
			$totANTWORTStotTIMEmtd =	sprintf("%10s", floor(MathZDC(($totANTWORTSsecmtd+($totANTWORTSmtd*15)), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSsecmtd+($totANTWORTSmtd*15)))));
			$totAGENTSmtd=count(array_count_values($AGENTS_mtd_array));
			...
			if (round(MathZDC($totANTWORTSsecmtd, $totANTWORTSmtd))>$max_mtd_avgtalktime) {$max_mtd_avgtalktime=round(MathZDC($totANTWORTSsecmtd, $totANTWORTSmtd));}
			...
			$ASCII_text.="$MAINH\n";
			$ASCII_text.="|                                       MTD | $totCALLSmtd | $totANTWORTSmtd | $totAGENTSmtd | $totABANDONSmtd | $totABANDONSpctmtd%| $totABANDONSavgTIMEmtd | $totANTWORTSavgspeedTIMEmtd | $totANTWORTSavgTIMEmtd | $totANTWORTStalkTIMEmtd | $totANTWORTSwrapTIMEmtd | $totANTWORTStotTIMEmtd |";
			$CSV_text.="\"MTD\",\"$totCALLSmtd\",\"$totANTWORTSmtd\",\"$totAGENTSmtd\",\"$totABANDONSmtd\",\"$totABANDONSpctmtd%\",\"$totABANDONSavgTIMEmtd\",\"$totANTWORTSavgspeedTIMEmtd\",\"$totANTWORTSavgTIMEmtd\",\"$totANTWORTStalkTIMEmtd\",\"$totANTWORTSwrapTIMEmtd\",\"$totANTWORTStotTIMEmtd\"";
			for ($s=0; $s<count($status_array); $s++) {
			...
			if (date("m", strtotime($daySTART[$d]))==1 || date("m", strtotime($daySTART[$d]))==4 || date("m", strtotime($daySTART[$d]))==7 || date("m", strtotime($daySTART[$d]))==10) # Quarterly line
				...
				$totANTWORTSavgspeedTIMEqtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSspeedqtd, $totANTWORTSqtd)))));
				$totANTWORTSavgTIMEqtd =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($totANTWORTSsecqtd, $totANTWORTSqtd)))));
				if (round(MathZDC($totANTWORTSspeedqtd, $totANTWORTSqtd))>$max_qtd_avganswerspeed) {$max_qtd_avganswerspeed=round(MathZDC($totANTWORTSspeedqtd, $totANTWORTSqtd));}
				$qtd_graph_stats[$qa][12]=round(MathZDC($totANTWORTSspeedqtd, $totANTWORTSqtd));
				$qtd_graph_stats[$qa][16]=round(MathZDC($totANTWORTSsecqtd, $totANTWORTSqtd));
				$totANTWORTStalkTIMEqtd =	sprintf("%10s", floor(MathZDC($totANTWORTSsecqtd, 3600)).date(":i:s", mktime(0, 0, $totANTWORTSsecqtd)));
				$totANTWORTSwrapTIMEqtd =	sprintf("%10s", floor(MathZDC(($totANTWORTSqtd*15), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSqtd*15))));
				...
				$totANTWORTStotTIMEqtd =	sprintf("%10s", floor(MathZDC(($totANTWORTSsecqtd+($totANTWORTSqtd*15)), 3600)).date(":i:s", mktime(0, 0, ($totANTWORTSsecqtd+($totANTWORTSqtd*15)))));
				$totAGENTSqtd=count(array_count_values($AGENTS_qtd_array));
				...
				if (round(MathZDC($totANTWORTSsecqtd, $totANTWORTSqtd))>$max_qtd_avgtalktime) {$max_qtd_avgtalktime=round(MathZDC($totANTWORTSsecqtd, $totANTWORTSqtd));}
				...
				$ASCII_text.="|                                       QTD | $totCALLSqtd | $totANTWORTSqtd | $totAGENTSqtd | $totABANDONSqtd | $totABANDONSpctqtd%| $totABANDONSavgTIMEqtd | $totANTWORTSavgspeedTIMEqtd | $totANTWORTSavgTIMEqtd | $totANTWORTStalkTIMEqtd | $totANTWORTSwrapTIMEqtd | $totANTWORTStotTIMEqtd |";
				$CSV_text.="\"QTD\",\"$totCALLSqtd\",\"$totANTWORTSqtd\",\"$totAGENTSqtd\",\"$totABANDONSqtd\",\"$totABANDONSpctqtd%\",\"$totABANDONSavgTIMEqtd\",\"$totANTWORTSavgspeedTIMEqtd\",\"$totANTWORTSavgTIMEqtd\",\"$totANTWORTStalkTIMEqtd\",\"$totANTWORTSwrapTIMEqtd\",\"$totANTWORTStotTIMEqtd\"";
				for ($s=0; $s<count($status_array); $s++) {
		...
		if (round(MathZDC($totANTWORTSsecdate[$d], $totANTWORTSdate[$d]))>$max_avgtalktime) 
			{$max_avgtalktime=round(MathZDC($totANTWORTSsecdate[$d], $totANTWORTSdate[$d]));}
		...
		$ASCII_text.="| $daySTART[$d] - $dayEND[$d] | $totCALLSdate[$d] | $totANTWORTSdate[$d] | $totAGENTSday | $totABANDONSdate[$d] | $totABANDONSpctDATE[$d]%| $totABANDONSavgTIME[$d] | $totANTWORTSavgspeedTIME[$d] | $totANTWORTSavgTIME[$d] | $totANTWORTStalkTIME[$d] | $totANTWORTSwrapTIME[$d] | $totANTWORTStotTIME[$d] |";
		$CSV_text.="\"$daySTART[$d] - $dayEND[$d]\",\"$totCALLSdate[$d]\",\"$totANTWORTSdate[$d]\",\"$totAGENTSday\",\"$totABANDONSdate[$d]\",\"$totABANDONSpctDATE[$d]%\",\"$totABANDONSavgTIME[$d]\",\"$totANTWORTSavgspeedTIME[$d]\",\"$totANTWORTSavgTIME[$d]\",\"$totANTWORTStalkTIME[$d]\",\"$totANTWORTSwrapTIME[$d]\",\"$totANTWORTStotTIME[$d]\"";
		for ($s=0; $s<count($status_array); $s++) {
	...
	$totTIME_MS =		sprintf("%9s", $totTIME_MS);
		...
		$FtotANTWORTSavgspeedTIME =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($FtotANTWORTSspeed, $FtotANTWORTS)))));
		$FtotANTWORTSavgTIME =	sprintf("%6s", date("i:s", mktime(0, 0, round(MathZDC($FtotANTWORTSsec, $FtotANTWORTS)))));
		$FtotANTWORTStalkTIME =	sprintf("%10s", floor(MathZDC($FtotANTWORTSsec, 3600)).date(":i:s", mktime(0, 0, $FtotANTWORTSsec)));
		$FtotANTWORTSwrapTIME =	sprintf("%10s", floor(MathZDC(($FtotANTWORTS*15), 3600)).date(":i:s", mktime(0, 0, ($FtotANTWORTS*15))));
		$FtotANTWORTStotTIME =	sprintf("%10s", floor(MathZDC(($FtotANTWORTSsec+($FtotANTWORTS*15)), 3600)).date(":i:s", mktime(0, 0, ($FtotANTWORTSsec+($FtotANTWORTS*15)))));
		...
		if (date("w", strtotime($daySTART[$d]))>0) 
			...
			for ($q=0; $q<count($wtd_graph_stats); $q++) {
				...
				$wtd_ANTWORTED_graph.="  <tr><td class='chart_td$class'>".$wtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$wtd_graph_stats[$q][2], $max_wtd_answered))."' height='16' />".$wtd_graph_stats[$q][2]."</td></tr>";
				...
				$wtd_AVGANTWORTSPEED_graph.="  <tr><td class='chart_td$class'>".$wtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$wtd_graph_stats[$q][12], $max_wtd_avganswerspeed))."' height='16' />".$wtd_graph_stats[$q][6]."</td></tr>";
		...
		if (date("d", strtotime($daySTART[$d]))!=1) 
			...
			if (round(MathZDC($totANTWORTSsecmtd, $totANTWORTSmtd))>$max_mtd_avgtalktime)
				{$max_mtd_avgtalktime=round(MathZDC($totANTWORTSsecmtd, $totANTWORTSmtd));}
			...
			for ($q=0; $q<count($mtd_graph_stats); $q++) {
				...
				$mtd_ANTWORTED_graph.="  <tr><td class='chart_td$class'>".$mtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$mtd_graph_stats[$q][2], $max_mtd_answered))."' height='16' />".$mtd_graph_stats[$q][2]."</td></tr>";
				...
				$mtd_AVGANTWORTSPEED_graph.="  <tr><td class='chart_td$class'>".$mtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$mtd_graph_stats[$q][12], $max_mtd_avganswerspeed))."' height='16' />".$mtd_graph_stats[$q][6]."</td></tr>";
	...
	#			{
			...
			if ( (round(MathZDC($totANTWORTSsecqtd, $totANTWORTSqtd))>$max_qtd_avgtalktime) )
				{$max_qtd_avgtalktime=round(MathZDC($totANTWORTSsecqtd, $totANTWORTSqtd));}
			...
			for ($q=0; $q<count($qtd_graph_stats); $q++) {
				...
				$qtd_ANTWORTED_graph.="  <tr><td class='chart_td$class'>".$qtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$qtd_graph_stats[$q][2], $max_qtd_answered))."' height='16' />".$qtd_graph_stats[$q][2]."</td></tr>";
				...
				$qtd_AVGANTWORTSPEED_graph.="  <tr><td class='chart_td$class'>".$qtd_graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$qtd_graph_stats[$q][12], $max_qtd_avganswerspeed))."' height='16' />".$qtd_graph_stats[$q][6]."</td></tr>";
	...
	#			}
			...
			for ($q=0; $q<count($graph_stats); $q++) {
				...
				$ANTWORTED_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$graph_stats[$q][2], $max_answered))."' height='16' />".$graph_stats[$q][2]."</td></tr>";
				...
				$AVGANTWORTSPEED_graph.="  <tr><td class='chart_td$class'>".$graph_stats[$q][0]."</td><td nowrap class='chart_td value$class'><img src='../vicidial/images/bar.png' alt='' width='".round(MathZDC(400*$graph_stats[$q][12], $max_avganswerspeed))."' height='16' />".$graph_stats[$q][6]."</td></tr>";
	...
	$ASCII_text.="$MAINH\n";
	$ASCII_text.="|                                    TOTALS | $FtotCALLS | $FtotANTWORTS | $FtotAGENTS | $FtotABANDONS | $FtotABANDONSpct%| $FtotABANDONSavgTIME | $FtotANTWORTSavgspeedTIME | $FtotANTWORTSavgTIME | $FtotANTWORTStalkTIME | $FtotANTWORTSwrapTIME | $FtotANTWORTStotTIME |";
	$CSV_text.="\"TOTALS\",\"$FtotCALLS\",\"$FtotANTWORTS\",\"$FtotAGENTS\",\"$FtotABANDONS\",\"$FtotABANDONSpct%\",\"$FtotABANDONSavgTIME\",\"$FtotANTWORTSavgspeedTIME\",\"$FtotANTWORTSavgTIME\",\"$FtotANTWORTStalkTIME\",\"$FtotANTWORTSwrapTIME\",\"$FtotANTWORTStotTIME\"";
	for ($s=0; $s<count($status_array); $s++) {
