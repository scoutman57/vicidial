...
while($one_day_interval > 0)
{
	...
	###############################################################################
		...
		$stmtA = "SELECT callerid,server_ip,channel FROM vicidial_auto_calls where server_ip='$server_ip' order by call_time;";
		$dbhA->query("$stmtA");
		if ($dbhA->has_selected_record)
		{
		$iter=$dbhA->create_record_iterator;
	...
	#			$stmtA = "UPDATE vicidial_auto_calls set status='PAUSED' where callerid='$KLcallerid[$kill_vac]'";
		...
		$stmtA = "DELETE FROM vicidial_auto_calls where call_time < '$XDSQLdate' and status NOT IN('XFER')";
		$dbhA->query("$stmtA");
		my $affected_rows = $dbhA->get_affected_rows_length;
