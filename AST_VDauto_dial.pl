#!/usr/bin/perl
#
# AST_VDauto_dial.pl version 0.5
#
# DESCRIPTION:
...
while($one_day_interval > 0)
{
	...
	###############################################################################
		...
		foreach(@DBIPcampaign)
			...
			$dbhA->query("SELECT auto_dial_level,local_call_time,dial_timeout,dial_prefix,campaign_cid,active FROM vicidial_campaigns where campaign_id='$DBIPcampaign[$user_CIPct]'");
			if ($dbhA->has_selected_record)
				{
				$iter=$dbhA->create_record_iterator;
				   while ( $record = $iter->each)
							...
							$dbhA->query("$stmtA");
							   ...
							   $lead_id_call_list .= "$lead_id|";
								...
								### insert a NEW record to the vicidial_manager table to be processed
									$stmtA = "INSERT INTO vicidial_manager values('','','$SQLdate','NEW','N','$DBIPaddress[$user_CIPct]','','Originate','$VqueryCID','Exten: $answer_transfer_agent','Context: $ext_context','Channel: $local_DEF$Local_out_prefix$phone_code$phone_number$local_AMP$ext_context','Priority: 1','Callerid: $CIDstring','Timeout: $Local_dial_timeout','','','','')";
									$dbhA->query($stmtA);
