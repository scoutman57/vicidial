#!/usr/bin/perl
#
# AST_VDauto_dial.pl version 0.1
#
# DESCRIPTION:
...
while($one_day_interval > 0)
{
	...
	###############################################################################
		...
		foreach(@DBIPcampaign)
			...
			$dbhA->query("SELECT auto_dial_level FROM vicidial_campaigns where campaign_id='$DBIPcampaign[$user_CIPct]'");
			if ($dbhA->has_selected_record)
				{
				$iter=$dbhA->create_record_iterator;
				   while ( $record = $iter->each)
			...
			$dbhA->query("SELECT count(*) FROM vicidial_auto_calls where campaign_id='$DBIPcampaign[$user_CIPct]' and server_ip='$DBIPaddress[$user_CIPct]' and status IN('SENT','RINGING','LIVE','XFER');");
			if ($dbhA->has_selected_record)
				{
				$iter=$dbhA->create_record_iterator;
				   while ( $record = $iter->each)
						...
						$dbhA->query("$stmtA");
						   ...
						   $dbhA->query("$stmtA");
							...
							### insert a NEW record to the vicidial_manager table to be processed
								...
								$event_string = "|     number call dialed|$DBIPcampaign[$user_CIPct]|$VqueryCID|$stmtA|";
								 &event_logger;
	...
	###############################################################################
		...
		$stmtA = "SELECT callerid,server_ip,channel FROM vicidial_auto_calls order by call_time;";
		$dbhA->query("$stmtA");
		if ($dbhA->has_selected_record)
		{
		$iter=$dbhA->create_record_iterator;
		...
		foreach(@KLcallerid)
			...
			$stmtA = "SELECT end_epoch,uniqueid FROM call_log where caller_code='$KLcallerid[$kill_vac]' and server_ip='$KLserver_ip[$kill_vac]' order by start_time desc limit 1;";
			$dbhA->query("$stmtA");
			if ($dbhA->has_selected_record)
				{
				$iter=$dbhA->create_record_iterator;
	...
	#			$stmtA = "UPDATE vicidial_auto_calls set status='PAUSED' where callerid='$KLcallerid[$kill_vac]'";
		...
		$stmtA = "UPDATE vicidial_live_agents set status='PAUSED' where last_update_time < '$PDtsSQLdate'";
		$dbhA->query("$stmtA");
		my $affected_rows = $dbhA->get_affected_rows_length;
		...
		### delete call records that are SENT for over 5 minutes
		$stmtA = "DELETE FROM vicidial_auto_calls where call_time < '$XDSQLdate'";
		$dbhA->query("$stmtA");
		my $affected_rows = $dbhA->get_affected_rows_length;
