+------------------------------------------------------------------------------+
|      Asterisk GUI client - astguiclient - eleventh public release 1.0.5      |
 ...
 ** It may be easier to follow the SCRATCH_INSTALL directions than these 3 **
...
- astGUIclient_1.0.5.pl - the GUI application itself
 ...
 - astVICIDIAL_0.8.pl - the VICIDIAL GUI client auto-dialing application
   + same application run specs as the AST_WINphoneAPP above
   + additionally requires Time::HiRes perl module

 - astLEADLOADER.pl - client GUI for loading leads into the vicidial_list
   + same application run specs as the AST_WINphoneAPP above
 ...
 - call_log.agi - AGI/perl programm for call logging
 - call_logCID.agi - AGI/perl programm for call logging receiving CallerID
...
Zaptel trunks or IAX VOIP trunks and SIP VOIP hard or softphones as extensions,
it could be adapted to other functions, but It was designed for Zap/IAX/SIP 
users. The client programs will run on X and Win32.


TO BE ADDED:
See the TODO.txt file for more info on what we're thinking of working on, and if
you have suggestions please send them.
...
- on astGUIclient you cannot record a VOIP-only conversation unless you go into
  a conference, you also cannot monitor a VOIP-only conversation
- Depending on what version of perl you are running, you may have a conflicting 
  version of the Time::HiRes perl module installed. To fix this, jut go to 
  http://search.cpan.org/~jhi/Time-HiRes-1.65/ and download the version that
  your error message says it's looking for(HiRes.pm).
 ...
 * if using a CVS version 2004-10-07 or newer change the line in extensions.conf
