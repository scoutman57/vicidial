...
### optional variables:
###  - $format - ('text','debug')
###  - $ACTION - ('regCLOSER','manDIALnextCALL','manDIALlookCALL','manDIALlogCALL','userLOGOUT','updateDISPO','VDADpause','VDADready','VDADcheckINCOMING')
###  - $stage - ('start','finish')
...
if ($ACTION == 'VDADcheckINCOMING')
{
	...
	if ($queue_leadID_ct > 0)
		...
		if (eregi("CLOSER",$campaign))
			...
			if (strlen($VDCL_group_web)>5) {echo "$VDCL_group_web|$VDCL_group_name|$VDCL_group_color|$VDCL_fronter_display|\n";}
			else {echo "|$VDCL_group_name|$VDCL_group_color|$VDCL_fronter_display|\n";}
...
if ( ($ACTION == 'VDADpause') || ($ACTION == 'VDADready') )
{
	...
	{
	$stmt="UPDATE vicidial_live_agents set status='$stage',lead_id='',uniqueid=0,callerid='',channel='' where user='$user' and server_ip='$server_ip';";
		if ($format=='debug') {echo "\n<!-- $stmt -->";}
	$rslt=mysql_query($stmt, $link);
	}
