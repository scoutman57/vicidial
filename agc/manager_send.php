...
###  - $PHP_AUTH_PW
### optional variables:
###  - $ACTION - ('Originate','Redirect','Hangup','Command','Monitor','StopMonitor','SysCIDOriginate','RedirectName','RedirectNameVmail','MonitorConf','StopMonitorConf','RedirectXtra')
...
if ( ($ACTION=="MonitorConf") || ($ACTION=="StopMonitorConf") )
{
	...
	if ($ACTION=="MonitorConf")
		...
		$stmt="SELECT channel FROM live_sip_channels where server_ip = '$server_ip' and channel LIKE \"$channel%\";";
			if ($format=='debug') {echo "\n<!-- $stmt -->";}
		$rslt=mysql_query($stmt, $link);
		$rec_count = mysql_num_rows($rslt);
			if ($rec_count>0)
			...
			$stmt="INSERT INTO vicidial_manager values('','','$NOW_TIME','NEW','N','$server_ip','','Hangup','CRHU1234567890123','Channel: $HUchannel','','','','','','','','','');";
				if ($format=='debug') {echo "\n<!-- $stmt -->";}
			$rslt=mysql_query($stmt, $link);
		...
		echo "$ACTION command sent for Channel $channel on $server_ip\nFilename: $filename\nRecording_ID: $recording_id\n RECORDING WILL LAST FOR 60 MINUTES\n";
	}
}
