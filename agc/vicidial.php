...
$VDloginDISPLAY=0;
	...
	if($auth>0)
		...
		if($CAMPactive>0)
			...
			$stmt="SELECT dial_status_a,dial_status_b,dial_status_c,dial_status_d,dial_status_e,park_ext,park_file_name,web_form_address,allow_closers,auto_dial_level,dial_timeout,dial_prefix,campaign_cid,campaign_vdad_exten,campaign_rec_exten,campaign_recording,campaign_rec_filename FROM vicidial_campaigns where campaign_id = '$VD_campaign';";
			$rslt=mysql_query($stmt, $link);
			if ($DB) {echo "$stmt\n";}
			$row=mysql_fetch_row($rslt);
			   $status_A = $row[0];
...
#	echo "hotkeys['$HKhotkey[$h]'] = \"$HKstatus[$h]\";\n";
	$h++;
	}
	?>
...
// Send the Manual Dial Next Number request
	function ManualDialNext()
		...
		if (xmlhttp) 
			{ 
			manDiaLnext_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=manDiaLnextCaLL&conf_exten=" + session_id + "&user=" + user + "&pass=" + pass + "&campaign=" + campaign + "&ext_context=" + ext_context + "&dial_timeout=" + dial_timeout + "&dial_prefix=" + dial_prefix + "&campaign_cid=" + campaign_cid + "&preview=" + man_preview;
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// GLOBAL FUNCTIONS
	...
	function ShoWTransferMain(showxfervar)
		{
		if (showxfervar == 'ON')
			{
			HKbutton_allowed = 0;
...
<!-- <td align=left><font class="body_text"><TEXTAREA NAME=comments ROWS=2 COLS=65 class="cust_form" value=""></TEXTAREA></td> -->
