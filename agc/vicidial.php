...
echo "Phone Login: <INPUT TYPE=TEXT NAME=phone_login SIZE=10 MAXLENGTH=20 VALUE=\"$phone_login\">\n<br>";
echo "Phone Password: <INPUT TYPE=PASSWORD NAME=phone_pass SIZE=10 MAXLENGTH=20 VALUE=\"$phone_pass\"><br>\n";
echo "User Login: <INPUT TYPE=TEXT NAME=VD_login SIZE=10 MAXLENGTH=20 VALUE=\"$VD_login\">\n<br>";
echo "User Password: <INPUT TYPE=PASSWORD NAME=VD_pass SIZE=10 MAXLENGTH=20 VALUE=\"$VD_pass\"><br>\n";
...
echo "<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
echo "Phone Login: <INPUT TYPE=TEXT NAME=phone_login SIZE=10 MAXLENGTH=20 VALUE=\"\">\n<br>";
echo "Phone Password: <INPUT TYPE=PASSWORD NAME=phone_pass SIZE=10 MAXLENGTH=20 VALUE=\"\"><br>\n";
echo "<INPUT TYPE=SUBMIT NAME=SUBMIT VALUE=SUBMIT>\n";
echo "</FORM>\n\n";
echo "</body>\n\n";
echo "</html>\n\n";
...
if (!$authphone)
	{
	echo "<title>VICIDIAL web client: Phone Login</title>\n";
	echo "Sorry, your phone login and password are not active in this system, please try again:\n";
	echo "<FORM ACTION=\"$PHP_SELF\" METHOD=POST>\n";
	echo "<INPUT TYPE=HIDDEN NAME=DB VALUE=\"$DB\">\n";
...
?>
	...
	var DialControl_auto_HTML = "<a href=\"#\" onclick=\"AutoDialPause();\"><IMG SRC=\"./images/vdc_LB_pause.gif\" border=0 alt=\"Pause\"></a> | <a href=\"#\" onclick=\"AutoDialResume();\"><IMG SRC=\"./images/vdc_LB_resume.gif\" border=0 alt=\"Resume\"></a>";
...
// ACTIVE EXTENSIONS LIST REFRESH FUNCTIONS
	...
	function getactiveext(taskwindow) 
		...
		if (getactiveext_window == "MainXFERBox")
			{
	//		extvalue = document.vicidial_form.extension_xfer.value;
			...
			if (getactiveext_window == "LocalDialBox")
				...
				if (check_n>3)
					{
					extvalue = '';
						extvalue = document.vicidial_form.extension.value;
		...
		if (xmlhttp) 
			{ 
			activeext_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ADD=1&order=" + activeext_order + "&format=selectlist&bgcolor=#E6E6E6&selectsize=" + menuheight + "&selectedext=" + extension + "&selectfontsize=" + menufontsize + "&field_name=" + ext_field;
	...
	function busyhangup_send_hangup(taskvar) 
		...
		if (xmlhttp) 
			...
			if (taskvar == 'Trunk')
				...
				var hangupvalue = document.vicidial_form.trunk.value;
				busyhangup_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=Hangup&format=text&channel=" + hangupvalue + "&queryCID=" + queryCID;
				}
			if (taskvar == 'Local')
				{
				var queryCID = "HUagcW" + epoch_sec + user_abb;
				var hangupvalue = document.vicidial_form.local.value;
				busyhangup_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=Hangup&format=text&channel=" + hangupvalue + "&queryCID=" + queryCID;
				}
...
// Send Redirect command for Trunk/Local to Manager
	function busyhangup_send_redirect(taskvar) 
		{
		var xmlhttp=false;
		/*@cc_on @*/
		/*@if (@_jscript_version >= 5)
		...
		if (xmlhttp) 
			...
			if (taskvar == 'Trunk')
				...
				var redirectvalue = document.vicidial_form.trunk.value;
				busyredirect_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=Redirect&format=text&channel=" + redirectvalue + "&queryCID=" + queryCID + "&exten=" + dialplan_number + "&ext_context=" + ext_context + "&ext_priority=1";
				}
			if (taskvar == 'Local')
				{
				var queryCID = "HJagcW" + epoch_sec + user_abb;
				var redirectvalue = document.vicidial_form.local.value;
				busyredirect_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=Redirect&format=text&channel=" + redirectvalue + "&queryCID=" + queryCID + "&exten=" + dialplan_number + "&ext_context=" + ext_context + "&ext_priority=1";
				}
			xmlhttp.open('POST', 'manager_send.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// Send Originate command for local dial to Manager sends phone name where call is going to
	function mainxfer_send_originate(taskvar,taskxferconf) 
		{
		var xmlhttp=false;
		/*@cc_on @*/
		/*@if (@_jscript_version >= 5)
		...
		if (xmlhttp) 
			...
			if (taskvar == 'DIAL')
				...
				var originatedestination = document.vicidial_form.extension_dial.value;
				var localdial_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=OriginateName&format=text&channel=" + originatevalue + "&queryCID=" + queryCID + "&extenName=" + originatedestination + "&ext_context=" + ext_context + "&ext_priority=1";
				}
			if (taskvar == 'VMAIL')
				{
				var queryCID = "LDagcW" + epoch_sec + user_abb;
				var originatedestination = document.vicidial_form.extension_dial.value;
				var localdial_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=OriginateNameVmail&format=text&channel=" + originatevalue + "&queryCID=" + queryCID + "&exten=" + voicemail_dump_exten + "&extenName=" + originatedestination + "&ext_context=" + ext_context + "&ext_priority=1";
				}
			if (taskvar == 'ENTRY')
				{
				var queryCID = "LEagcW" + epoch_sec + user_abb;
				var originatedestination = document.vicidial_form.extension_dial_entry.value;
				var localdial_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=Originate&format=text&channel=" + originatevalue + "&queryCID=" + queryCID + "&exten=" + originatedestination + "&ext_context=" + ext_context + "&ext_priority=1";
				}
			if (taskvar == 'CONF')
				{
...
// Grab the inbound and outbound call logs and display
	function call_logs_display_refresh()
		...
		if (xmlhttp) 
			{ 
			call_logs_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&protocol=" + protocol + "&exten=" + extension;
...
// Grab the current parked calls and display
	function parked_calls_display_refresh()
		...
		if (xmlhttp) 
			{ 
			park_calls_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&protocol=" + protocol + "&exten=" + extension;
...
// Grab the conferences available and if they are registered, then display list
	function conference_list_display_refresh(taskplace)
		...
		if (xmlhttp) 
			{
			conferences_list_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ADD=5&order=asc&format=text";
			xmlhttp.open('POST', 'active_list_refresh.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// filter manual dialstring and pass on to originate call
	function SendManualDial(taskFromConf,taskconfexten)
		...
		if (taskFromConf == 'YES')
			{
			var manual_number = document.vicidial_form.conf_dial.value;
			var manual_string = manual_number.toString();
			var dial_conf_exten = taskconfexten;
			}
		else
			{
			var manual_number = document.vicidial_form.manual_dial.value;
			var manual_string = manual_number.toString();
			}
...
// Send Originate command to manager to place a phone call
	function basic_originate_call(tasknum,taskprefix,taskreverse,taskdialvalue) 
		...
		if (xmlhttp) 
			...
			VMCoriginate_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=Originate&format=text&channel=" + originatevalue + "&queryCID=" + queryCID + "&exten=" + orig_prefix + "" + dialnum + "&ext_context=" + ext_context + "&ext_priority=1";
			xmlhttp.open('POST', 'manager_send.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// Check to see if there are any channels live in the agent's conference meetme room
	function check_for_conf_calls(taskconfnum,taskforce)
		...
		if (xmlhttp) 
			{ 
			checkconf_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass + "&client=vdc&conf_exten=" + taskconfnum;
...
// Covers the following types: XFER, VMAIL, ENTRY, CONF, PARK, FROMPARK, XFERLOCAL, XFERINTERNAL, XFERBLIND
	function mainxfer_send_redirect(taskvar,taskxferconf) 
		...
		if (xmlhttp) 
			...
			if (taskvar == 'CONF')
				{
				if (document.vicidial_form.MainXFERconfXTRA.checked==true)
					{
					var queryCID = "LXvdcW" + epoch_sec + user_abb;
					var redirectdestination = taskxferconf;
					var redirectXTRAvalue = document.vicidial_form.M_XFER_channel.value;
					...
					{
					var queryCID = "LCvdcW" + epoch_sec + user_abb;
					var redirectdestination = taskxferconf;
					xferredirect_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&user=" + user + "&pass=" + pass + "&ACTION=Redirect&format=text&channel=" + redirectvalue + "&queryCID=" + queryCID + "&exten=" + redirectdestination + "&ext_context=" + ext_context + "&ext_priority=1";
					}
				}
...
// Insert or update the vicidial_log entry for a customer call
	function DialLog(taskMDstage)
		...
		if (xmlhttp) 
			...
			"&exten=" + extension + "&channel=" + lastcustchannel + "&start_epoch=" + MDlogEPOCH;
...
// Request lookup of manual dial channel
	function ManualDialCheckChannel()
		...
		if (xmlhttp) 
			{ 
			manDIALlook_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=manDIALlookCALL&conf_exten=" + session_id + "&user=" + user + "&pass=" + pass + "&MDnextCID=" + MDnextCID;
			xmlhttp.open('POST', 'vdc_db_query.php'); 
			xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
			...
			xmlhttp.onreadystatechange = function() 
				...
				//	alert(xmlhttp.responseText);
					...
					if (MDlookCID == "NO")
						{
						MD_ring_seconds++;
						document.getElementById("MainStatusSpan").innerHTML = " Calling: " + document.vicidial_form.phone_number.value + " UID: " + MDnextCID + " &nbsp; Waiting for Ring... " + MD_ring_seconds + " seconds";
				//		alert("channel not found yet:\n" + campaign);
						...
						if ( (MDchannel.match(regMDL)) && (asterisk_version != '1.0.8') && (asterisk_version != '1.0.9') )
							...
							MD_channel_look=0;
							document.getElementById("MainStatusSpan").innerHTML = " Called: " + document.vicidial_form.phone_number.value + " UID: " + MDnextCID + " &nbsp;"; 

							document.getElementById("ParkControl").innerHTML ="<a href=\"#\" onclick=\"mainxfer_send_redirect('PARK','" + MDchannel + "');return false;\"><IMG SRC=\"./images/vdc_LB_parkcall.gif\" border=0 alt=\"Park Call\"></a>";
							...
							document.getElementById("LocalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XFERLOCAL','" + MDchannel + "');return false;\">LOCAL CLOSER</a>";

							document.getElementById("InternalCloser").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XFERINTERNAL','" + MDchannel + "');return false;\">INTERNAL CLOSER</a>";

							document.getElementById("DialBlindTransfer").innerHTML = "<a href=\"#\" onclick=\"mainxfer_send_redirect('XFERBLIND','" + MDchannel + "');return false;\">Dial Blind Transfer</a>";
...
// $VICIDIAL_web_QUERY_STRING =~ s/\`|\~|\:|\;|\#|\'|\"|\{|\}|\(|\)|\*|\^|\%|\$|\!|\%|\r|\t|\n//gi;
						...
						document.getElementById("WebFormSpan").innerHTML = "<a href=\"" + VICIDIAL_web_form_address + web_form_vars + "\" target=\"vdcwebform\"><IMG SRC=\"./images/vdc_LB_webform.gif\" border=0 alt=\"Web Form\"></a>\n";
...
// Send Hangup command for customer call connected to the conference now to Manager
	function dialedcall_send_hangup() 
		...
		//  DEACTIVATE CHANNEL-DEPENDANT BUTTONS AND VARIABLES
			...
			document.getElementById("LocalCloser").innerHTML = "LOCAL CLOSER";
			document.getElementById("InternalCloser").innerHTML = "INTERNAL CLOSER";
			document.getElementById("DialBlindTransfer").innerHTML = "Dial Blind Transfer";
	...
	function DispoSelectContent_create(taskDSgrp,taskDSstage)
		...
		while (loop_ct < VD_statuses_ct)
			...
			if (taskDSgrp == VARstatuses[loop_ct]) 
				{
				dispo_HTML = dispo_HTML + "<font size=3><b><a href=\"#\" onclick=\"DispoSelectContent_create('" + VARstatuses[loop_ct] + "','ADD');return false;\">" + VARstatuses[loop_ct] + " - " + VARstatusnames[loop_ct] + "</b></font><BR><BR>";
				}
			else
				{
				dispo_HTML = dispo_HTML + "<a href=\"#\" onclick=\"DispoSelectContent_create('" + VARstatuses[loop_ct] + "','ADD');return false;\">" + VARstatuses[loop_ct] + " - " + VARstatusnames[loop_ct] + "<BR><BR>";
				}
...
// Update vicidial_list lead record with disposition selection
	function DispoSelect_submit()
		...
		if (DispoChoice.length < 1) {alert("You Must Select a Disposition");}
			...
			if (xmlhttp) 
				{ 
				DSupdate_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=updateDISPO&format=text&user=" + user + "&pass=" + pass + "&dispo_choice=" + DispoChoice + "&lead_id=" + document.vicidial_form.lead_id.value;
				xmlhttp.open('POST', 'vdc_db_query.php'); 
				xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
	...
	function Logout()
		...
		if (MD_channel_look==1)
			...
			if (VD_live_customer_call==1)
				...
				if (xmlhttp) 
					{ 
					VDlogout_query = "server_ip=" + server_ip + "&session_name=" + session_name + "&ACTION=userLOGOUT&format=text&user=" + user + "&pass=" + pass + "&campaign=" + campaign + "&conf_exten=" + session_id;
					xmlhttp.open('POST', 'vdc_db_query.php'); 
					xmlhttp.setRequestHeader('Content-Type','application/x-www-form-urlencoded; charset=UTF-8');
...
// GLOBAL FUNCTIONS
	function start_all_refresh()
		...
		if (VICIDIAL_closer_login_checked==0)
			...
			if (AgentDispoing==1)	{WaitingForNextStep=1;}
...
<span style="position:absolute;left:0px;top:12px;z-index:25;" id="CustomerGoneBox">
    <table border=1 bgcolor="#CCFFFF" width=600 height=500><TR><TD align=center> Customer has hung up: <span id="CustomerGoneChannel"></span><BR>
...
<span style="position:absolute;left:0px;top:12px;z-index:27;" id="DispoSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=600 height=500><TR><TD align=center> DISPOSITION CALL :<span id="DispoSelectPhone"></span><BR>
...
<span style="position:absolute;left:0px;top:12px;z-index:28;" id="CloserSelectBox">
    <table border=1 bgcolor="#CCFFCC" width=600 height=500><TR><TD align=center> CLOSER INBOUND GROUP SELECTION <BR>
...
<span style="position:absolute;left:0px;top:12px;z-index:29;" id="TrunkHangupBox">
	...
	<a href="#" onclick="busyhangup_send_hangup('Trunk');return false;">Hangup Trunk</a> | 
	<a href="#" onclick="busyhangup_send_redirect('Trunk');return false;">Hijack Trunk</a> | 
...
<span style="position:absolute;left:0px;top:12px;z-index:28;" id="LocalHangupBox">
	...
	<a href="#" onclick="busyhangup_send_hangup('Local');return false;">Hangup Local</a> | 
	<a href="#" onclick="busyhangup_send_redirect('Local');return false;">Hijack Local</a> | 
...
<span style="position:absolute;left:80px;top:12px;z-index:43;" id="LocalDialBox">
	...
	<a href="#" onclick="hideLocalDial('LocalDialBox');">Back to MAIN</a> <BR><BR>
	</TD>
	<TD></td></TR></TABLE>
</span>
...
<span style="position:absolute;left:40px;top:12px;z-index:41;" id="ParkDisplayBox">
	...
	<a href="#" onclick="hideParkDisplay('ParkDisplayBox');">Back to MAIN</a> <BR><BR>
	</td></TR></TABLE>
</span>
...
<td align=left><font class="body_text"><input type=text size=3 name=seconds class="cust_form" value="">&nbsp; Channel: <input type=text size=5 name=callchannel class="cust_form" value="">&nbsp; Cust Time: <input type=text size=20 name=custdatetime class="cust_form" value=""></td>
</tr><tr>
<td colspan=2 align=center> Customer Information:</td>
</tr><tr>
<td align=right><font class="body_text"> Title: </td>
<td align=left><font class="body_text"><input type=text size=4 name=title maxlength=4 class="cust_form" value="">&nbsp; First: <input type=text size=10 name=first_name maxlength=30 class="cust_form" value="">&nbsp; MI: <input type=text size=1 name=middle_initial maxlength=1 class="cust_form" value="">&nbsp; Last: <input type=text size=10 name=last_name maxlength=30 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> Address1: </td>
<td align=left><font class="body_text"><input type=text size=30 name=address1 maxlength=100 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> Address2: </td>
<td align=left><font class="body_text"><input type=text size=15 name=address2 maxlength=100 class="cust_form" value="">&nbsp; Address3: <input type=text size=15 name=address3 maxlength=100 class="cust_form" value=""></td>
</tr><tr>
<td align=right><font class="body_text"> City: </td>
<td align=left><font class="body_text"><input type=text size=20 name=city maxlength=50 class="cust_form" value="">&nbsp; State: <input type=text size=2 name=state maxlength=2 class="cust_form" value="">&nbsp; PostCode: <input type=text size=6 name=postal_code maxlength=10 class="cust_form" value=""></td>
</tr><tr>
...
<td align=left><font class="body_text"><input type=text size=10 name=phone_number maxlength=10 class="cust_form" value="">&nbsp; DialCode: <input type=text size=4 name=phone_code maxlength=10 class="cust_form" value="">&nbsp; Alt. Phone: <input type=text size=10 name=alt_phone maxlength=10 class="cust_form" value=""></td>
</tr><tr>
...
<td align=left><font class="body_text"><input type=text size=50 name=comments maxlength=255 class="cust_form" value=""></td>
</tr></table>
</font>
</td>
...
<div id="TransferMain">
	...
	<td align=left>
	<font class="body_text">
	 Transfer - Conference Frame:<BR>
	<span STYLE="background-color: #CCCCCC" id="InternalCloser">INTERNAL CLOSER</span> &nbsp; &nbsp; 
	<span STYLE="background-color: #CCCCCC" id="LocalCloser">LOCAL CLOSER</span> &nbsp; &nbsp; 
	<span STYLE="background-color: #CCCCCC" id="CloserCode">CODE: <input type=text size=1 name=xfercode maxlength=2 class="cust_form"></span> &nbsp; &nbsp; &nbsp; &nbsp; 
	<span STYLE="background-color: #CCCCCC" id="Leave3WayCall">LEAVE 3-WAY CALL</span>
	<BR>
	Number to call: <input type=text size=12 name=xfernumber maxlength=15 class="cust_form"> &nbsp; &nbsp; 
	Seconds: <input type=text size=2 name=xferlength maxlength=4 class="cust_form"> &nbsp; &nbsp; 
	Channel: <input type=text size=12 name=xferchannel maxlength=100 class="cust_form"> &nbsp; &nbsp; 
	<BR>
	<span STYLE="background-color: #CCCCCC" id="DialWithCustomer">Dial With Customer</span> &nbsp; &nbsp; 
	<span STYLE="background-color: #CCCCCC" id="ParkCustomerDial">Park Customer Dial</span> &nbsp; &nbsp; 
	<span STYLE="background-color: #CCCCCC" id="DialBlindTransfer">Dial Blind Transfer</span> &nbsp; &nbsp;
	<span STYLE="background-color: #CCCCCC" id="HangupXferLine">Hangup Xfer Line</span> &nbsp; &nbsp;
	<span STYLE="background-color: #CCCCCC" id="HangupBothLines">Hangup Both Lines</span>
	</font>
	</td>
	</tr></table>
</div>
